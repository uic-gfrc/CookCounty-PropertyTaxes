...
change_table |> filter(first_tif_year > first_incent_year) 
# 407 projects were in a TIF before they received incentive classes
change_table |> filter(first_tif_year < first_incent_year) 
...
ptax_proj_summs <- ptax_proj %>% 
  ...
  mutate(
    land_use3 = case_when(
      sum(land_use2 == "Commercial")==12  ~ "Always Commercial",
      sum(land_use2 == "Industrial")==12  ~ "Always Industrial",     
      TRUE ~ "Changes Land Use At Some Point"
    ))|> ungroup()

table(ptax_proj_summs$land_use3)
```
...
ptax_proj_summs %>% 
 # select(-c(landuse_change, land_use, incent_change) ) %>%
           write_csv("Output/Pin to Project Files/project_summaries_2025_Dec04.csv")
...
proj_panel <- ptax_proj_summs %>% 
   ...
   select(main_keypin, year, fmv, fmv_w, incent_prop2,
          land_use2, land_use3,  #in_tif2, 
          triad, fmv_growth_2011,
         base_year_fmv_2011, reassessed_year, clean_name, fmv_growth_2011_w)  %>% 
...
write_rds(proj_panel, file = "Output/proj_panel3.RDS")
```


```{r read-proj-panel}

proj_panel <- read_rds("Output/proj_panel3.RDS")
...
models <- list(
  ...
  reassess_year = feols(fmv_growth_2011_w ~ land_use2*incent_prop2 + land_use3*incent_prop2 | 
                          main_keypin  + year, 
                        panel.id = c("main_keypin", "year"),                                     
                        vcov = ~clean_name,
                        data = proj_panel),
