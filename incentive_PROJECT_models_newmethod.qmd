...
```{r manual-pin-coding, eval = FALSE}

ptax_proj <- ptax_proj |> 
mutate(main_keypin = case_when(
...
```{r eval=FALSE}
ptax_pins <- ptax_pins |>
  select(year, pin, class, tax_code_num:assess_ratio, comparable_props:loa,
         tax_code_rate, tax_code_distribution_pct, fmv, in_tif, 
         class_group,landuse_change, incent_change, exe_abate,
         incent:av_mailed_pct_change)

ptax_proj_summs <- ptax_pins %>% 
 ...
 # mutate(main_keypin = ifelse(is.na(main_keypin), pin, main_keypin) ) |>
  ...
  summarize(
    ...
    # pin_classes = paste(sort(unique(class)), collapse = ", "),
    land_use = paste(sort(unique(land_use)), collapse = ", "),
    incent_prop = paste(sort(unique(incent_prop.x)), collapse = ", "),
    triad = first(Triad),
    clean_name = first(clean_name),
    reassessed_year = first(reassessed_year),
    exe_abate = sum(exe_abate, na.rm = TRUE),
...
ptax_proj_summs %>% 
  select(-c(landuse_change, land_use, incent_change) ) %>%
           write_csv("Output/project_summaries_2025_Sept8.csv")
...
write_rds(proj_panel, file = "Output/proj_panel.RDS")
```


```{r read-proj-panel}

proj_panel <- read_rds("Output/proj_panel.RDS")
