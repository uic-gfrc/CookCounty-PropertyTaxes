...
Changing to "RevenueCollected
Weighted tax rate was calculated for every tax bill observation for every pin and is equal to the eav X agency_tax_rate. Then all pins within tax codes were summed together to get a tax code level EAV and tax code level tax rate.

> Not sure if the agency_tax_rate calculated in the chunk below is reliable. would the agency_tax_rate be the mean of all agency tax rates for a tax code. I don't think so. The muni_tax_codes data table has all tax codes, tax code rates, agency numbers and agency rates. Use those numbers. 

> Should the weighted tax rate be the SUM of weighted tax rates within a tax code after it is grouped?


> Rerun chunk below for the 4.5 GB file of tax bills. Change the summarized weighted tax bill to a Sum()  instead of a mean() value. Also drop the agency_tax_value in the `bills_pertaxcode` file. when rerunning it. Add count of pins for each tax code too! 
...
# 924 taxing agencies in 2021 
#taxbills_by_agency  

# all munis + Cicero
taxbills_by_agency %>% 
  filter(agency_minor_type == "MUNI" | agency_num == "020060000") %>% 
  select(agency_name, final_tax_to_dist)

MuniLevy <- taxbills_by_agency %>%
    filter(agency_minor_type == "MUNI" | agency_num == "020060000") %>%
  group_by(agency_num) %>% summarize(total_tax_to_district = sum(final_tax_to_dist),
                                     total_revenue = final_tax_to_dist+final_tax_to_tif)
```
...
joined <- exemptions_by_class_per_TC %>% left_join(taxbills_by_Class_per_TC)
...
exemptions_by_class_per_TC <- left_join(exemptions_by_class_per_TC, muni_levies, by = c("agency_name", "agency_num")) %>%
  mutate(in_TIF = ifelse(tax_code_num %in% unique_tif_taxcodes$tax_code_num, 1,0) )


taxbills_by_TC$tax_code <- as.character(taxbills_by_TC$tax_code)
merged <- left_join(exemptions_by_class_per_TC, taxbills_by_TC, by = c("tax_code_num" = "tax_code")) #%>% select(-c(total_final_rate:total_non_cap_ext, year.y:curr_new_prop,pct_burden:total_reduced_levy))
...
summtable <- exemps_byClass_perMuni %>% select(agency_name, major_class_code, taxamount, eav_pre_exe_perClass, Exemp_in_PropClass, EAV_post_exe_PerClass, agency_num, agency_rate, major_class_type)
summtable
...
# class_rates <- as.data.frame(assessment_rates,class)
# class_rates
```
...
exemptions_perMuni <- exemps_byClass_perMuni %>%
  ...
  summarize(taxamount = sum(taxamount),
            eav_pre_exe_perMuni = sum(eav_pre_exe_perClass, na.rm=TRUE),
            TotalExemptions=sum(Exemp_in_PropClass, na.rm=TRUE),
         ResidentialExemp = sum(exe_homeowner +exe_senior+exe_freeze+exe_longtime_homeowner + exe_disabled+ exe_vet_returning +exe_vet_dis, na.rm=TRUE),
...
EAV_and_exemps_byClass_andMuni <- summtable %>% left_join(exemptions_perMuni , by = c("agency_name", "agency_num", "agency_rate"))
...
> Is final tax to district the same as the total_ext for the muni?
