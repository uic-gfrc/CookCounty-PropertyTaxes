...
muni_taxrates <- pin_data2 %>% 
  ...
  summarize(
   # median_eav = round(median(eav)), # includes ALL property types!! 
    #median_av = round(median(av)),  # includes ALL property types!! 
...
muni_taxrates_C2 <- pin_data2 %>% 
  ...
  summarize(
    ...
    C2_tax_code_rate = mean(tax_code_rate, na.rm = TRUE), # Changed from first() to mean() on Nov 1
    C2_final_tax_to_dist = sum(final_tax_to_dist, na.rm = TRUE), # used as LEVY amount!! 
    C2_final_tax_to_tif = sum(final_tax_to_tif, na.rm = TRUE),
    tax_amt_exe = sum(tax_amt_exe, na.rm = TRUE), 
    tax_amt_pre_exe = sum(tax_amt_pre_exe, na.rm = TRUE), 
    tax_amt_post_exe = sum(tax_amt_post_exe, na.rm = TRUE),
    rpm_tif_to_cps = sum(rpm_tif_to_cps, na.rm = TRUE), # not used
  ...
  mutate(C2_total_bill_current = C2_final_tax_to_dist + C2_final_tax_to_tif) %>%
  rename(C2_cur_comp_TC_rate = C2_tax_code_rate) %>%
  mutate(C2_current_taxable_eav = C2_final_tax_to_dist/(C2_cur_comp_TC_rate/100),
         C2_new_taxable_eav = C2_final_tax_to_dist/(C2_cur_comp_TC_rate/100) + C2_HO_exemps) %>%
  mutate(C2_new_comp_TC_rate = (C2_final_tax_to_dist / C2_new_taxable_eav)*100) %>%
  mutate(C2_new_comp_TC_rate = ifelse(is.nan(C2_new_comp_TC_rate), C2_cur_comp_TC_rate, C2_new_comp_TC_rate)) %>%
  select(agency_name, C2_cur_comp_TC_rate, C2_new_comp_TC_rate, C2_current_taxable_eav, C2_new_taxable_eav, everything())
...
muni_taxrates_C2 %>%
  ...
  labs(x = "EAV of Class 2 Property w/ No Bill Change", y = "Median EAV of Municipality", title = "Can homeowners with GHE benefit from removal of the GHE?",
       subtitle = "Yes, if they have a property valued way above other properties in their taxing district",
       caption = "Homeowners that currently claim the GHE are assumed to have their
       taxable EAV of their property increase by 10,000.
       Homeowners with very highly valued properties could see their bill decrease in GHE removal scenario.")


## Correlation is 0.95

cor(nobillchange$median_eav, nobillchange$nobillchange_propertyEAV)
...
pin_data2 %>%
  ...
  geom_text(aes(x=(median(eav)+50000), y = 1500, label = median(eav))) +
   scale_x_continuous(limits = c(-5, 3000000)) +
#   scale_y_continuous(limits = c(0, 1500))+
...
nobillchange %>%
  ...
  ggplot(aes(x=nobillchange_propertyEAV, y = median_eav, label=agency_name)) +
  geom_smooth(method = "lm" )+
 # geom_abline(slope = 0.02474, intercept = 0)+
  geom_text(aes(x = (nobillchange_propertyEAV), y = (median_eav+5000)), size = 1.5)+
  theme_classic() +
  labs(x = "EAV of Property w/ No Bill Change", y = "Median EAV of Municipality", title = "Municipality's Median EAV & EAV Where ")


## Correlation is 0.24.74
...
A home with EAV above 79,946 EAV  would experience a lower tax bill and homes below 29,946 EAV would experience an increase in their taxbill. 
The median EAV for all class 2 properties is 10,515 in Phoenix. 
...
A home with EAV above 5,159,327 EAV  would experience a lower tax bill and homes below 5,159,327 EAV would experience an increase in their taxbill. 
The median EAV for all Class 2 properties is 249,018 EAV in Winnetka.
