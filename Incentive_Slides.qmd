...
![](images/clipboard-213373226.png)

![](images/clipboard-1430145241.png)

![](images/clipboard-593146377.png)
...
alt_rates <- alt_rates %>% 
  mutate(change_noExe = cur_munilevel_rate - muni_rate_noExe,
         ...
         change_vacants = cur_munilevel_rate - muni_rate_vacant
         )
```
...
burden %>% 
  ...
  scale_color_brewer(palette="Paired", labels = c("Current Rate", "If LoA 25%", "If Vacant"), direction = 1)+
...
```{r}
#| label: fig-dotplot-ratechange-4dots
#| fig-height: 6
...
burden %>% 
  filter(incentive == "Incentive" & clean_name != "NA") %>%
  select(clean_name, cur_munilevel_rate, muni_rate_noInc, muni_rate_vacant, muni_rate_noExe) %>% 
  arrange(cur_munilevel_rate) %>%
  pivot_longer(c("cur_munilevel_rate", "muni_rate_noInc", "muni_rate_vacant", "muni_rate_noExe"), 
               names_to = "type", values_to = "tax_rate") %>% 
  inner_join(order) %>%
  ggplot(aes(x = tax_rate, y= reorder(clean_name, cur_munilevel_rate)))+
  geom_line(aes(group = clean_name))+ 
  ...
  scale_color_brewer(palette="Paired", labels = c("Current Rate", "If LoA 25%", "If Vacant", "If No GHE"), direction = 1)+
