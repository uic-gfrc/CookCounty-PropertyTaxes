...
$$\mbox{Taxed FMV} = {\frac{\frac{\frac{\sum{final\_tax\_to\_dist}}{\mbox{tax code rate}} + \mbox{Exemptions + Abatements}}{\mbox{county equalizer}}}}{\mbox{Level of assessment}}}$$
...
# reducedEAV_commerc_incents  = sum(ifelse(class >= 600 & class < 900 & class %in% commercial_classes,
#       (fmv*0.25 - fmv*0.1), 0), na.rm=TRUE),
# 
# reducedEAV_indust_incents  = sum(ifelse(class >= 600 & class < 900 & class %in% industrial_classes,
#       (taxed_av*eq2022/loa)*0.25 - (taxed_av*eq2022/loa), 0), na.rm=TRUE),
# 
# reducedEAV_comind_incents  = sum(ifelse(class >= 600 & class < 900,
#       (taxed_av*eq2022/loa)*0.25 - (taxed_av*eq2022/loa), 0), na.rm=TRUE),
...
# Amount collected by the new tax rate - amount needed by the district
    naive_forgonerev_noExe = rate_noExe/100 * eav_taxed - final_tax_to_dist,
    naive_forgonerev_noAbate = rate_noAbate/100 * eav_taxed - final_tax_to_dist,
    naive_forgonerev_noInc = rate_noInc/100 * eav_taxed - final_tax_to_dist,
    naive_forgonerev_noIndustInc = rate_noIndustInc/100 * eav_taxed - final_tax_to_dist,
    naive_forgonerev_noCommercInc = rate_noCommercInc/100 * eav_taxed - final_tax_to_dist,

    naive_forgonerev_noExe_or_Inc = rate_neither/100 * eav_taxed - final_tax_to_dist,
    naive_forgonerev_noTIFs = rate_noTIFs/100 * eav_taxed - final_tax_to_dist,
    naive_forgonerev_vacant = rate_vacant/100 * eav_taxed - final_tax_to_dist,
    naive_forgonerev_maxTEAV = rate_lowest/100 * eav_taxed - final_tax_to_dist,



    # forgonerev_noExe = rate_noExe/100 * new_TEAV_noExemps - final_tax_to_dist,
    # forgonerev_noAbate = rate_noAbate/100 * new_TEAV_noAbates - final_tax_to_dist,
    # forgonerev_noIncents = rate_noInc/100 * new_TEAV_noIncents - final_tax_to_dist,
    # forgonerev_noCommercIncents = rate_noInc/100 * new_TEAV_noCommercIncents - final_tax_to_dist,
    # 
    # forgonerev_noIndustIncents = rate_noInc/100 * new_TEAV_noIndustIncents - final_tax_to_dist, 
    # 
    # 
    # forgone_noExe_or_Inc = rate_neither/100 * TEAV_neither - final_tax_to_dist,
    # forgonerev_noTIFs = rate_noTIFs/100 * (eav_taxed+forgone_TIF_EAV) - final_tax_to_dist,
    # forgonerev_vacant = rate_vacant/100 * new_TEAV_vacant_props - final_tax_to_dist,
    # forgonerev_maxTEAV = rate_lowest/100 * TEAV_max - final_tax_to_dist
) 
...
cook_ratechange %>% 
select(forgonerev_from_comind_incents, forgonerev_commerc_incents, forgonerev_indust_incents, )
...
```{r calculate-cook-revchange-fromPINlevel}

cook_tc_revchange <- ptax_pins %>%
  # left_join(muni_rate) %>%
  filter(!clean_name  %in% c("Frankfort", "Homer Glen",  "Oak Brook", "East Dundee", "University Park",  "Bensenville", "Hinsdale", "Roselle", "Deer Park", "Deerfield")) %>% 
filter(!agency_num %in% cross_county_lines) %>%
group_by(clean_name, tax_code, class_group) %>%
  summarize(
...
reducedEAV_indust_incents  = new_TEAV_noIndustIncents - eav_taxed,
    ...
    naive_forgonerev_noExe = rate_noExe/100 * eav_taxed,
    naive_forgonerev_noAbate = rate_noAbate/100 * eav_taxed,
    naive_forgonerev_noInc = rate_noInc/100 * eav_taxed,
    naive_forgone_noExe_or_Inc = rate_neither/100 * eav_taxed,
    naive_forgonerev_noTIFs = rate_noTIFs/100 * eav_taxed,
    naive_forgonerev_vacant = rate_vacant/100 * eav_taxed,
    naive_forgonerev_maxTEAV = rate_lowest/100 * eav_taxed
) 


cook_tc_revchange %>% select(tax_code, contains("forgonerev"))
