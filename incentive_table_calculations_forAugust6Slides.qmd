...
bor_pins <- bor %>% 
  ...
  summarize(pin = first(pin),              # grabs first occurrence of unique PIN
            ...
            major_class = list(unique(majorclass)),
            timesappealed = n(),
            years_appealed = list(unique(tax_year))) %>%
  mutate(proj_appellant = paste(project_id, "-", appellant))

# write_csv(bor_pins, "./Output/projects_and_PINs_fromBORappeals.csv")

bor_projects <- bor_pins %>% 
  group_by(proj_appellant) %>%
  arrange(pin) %>%
  mutate()
...
table3<-ptax_pins %>% 
  ...
  select(-av_incent) %>% 
  select(clean_name, Alea_cat, av, fmv, pct_incent, pin_count, project_count, av_adjusted, ) %>% 
...
table3 %>%
  flextable() %>% 
  set_header_labels(clean_name = "Municipality", proptype_pincount = 'PINs', projects = "Project IDs", av = ' Taxable AV') %>%
...
> TO DO: Calculate the percents out of each group's FMV instead of the municipalities FMV? 
