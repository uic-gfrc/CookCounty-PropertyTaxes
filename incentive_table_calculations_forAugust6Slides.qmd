...
# Slide 5 - Main users of Incentive Classes, Muni Level
...
ungroup() %>%
  ...
  select(clean_name, pct_fmv_w_incentclass) |> 
  arrange(desc(pct_fmv_w_incentclass)) %>%
  mutate(pct_fmv_w_incentclass = scales::percent(round(pct_fmv_w_incentclass, digits = 3) )) %>%
    slice(c(1:10, 55:65, 110:119)) %>%
...
filter(clean_name != "Unincorporated") %>%
  ...
  scale_fill_steps2(
    ...
    na.value="gray",
    n =4,
    name = "Percent of Commercial & Industrial\nFMV with Incentive Classification",
    labels = scales::percent
) +
   geom_sf_pattern(data = no_incentives, aes(geometry = geometry), pattern = 'crosshatch', pattern_spacing = 0.015, pattern_density = 0.1, fill = "white", alpha = .5, color = 'gray40')
...
muni_mc_table <- ptax_pins %>%
  ...
  #  fmv_taxed =  sum(taxed_fmv, na.rm=TRUE),
    fmv_incents_inTIFs = sum(ifelse(
class >= 600 & class < 900 & final_tax_to_tif > 0, fmv, 0), na.rm = TRUE),
    ...
    untaxable_value_av = sum(untaxable_value_av, na.rm=TRUE),  ## untaxable for all potential policy choices
    av = sum(av),

    eav_taxed = sum(taxed_av*eq2022), 
    untaxable_value_eav = sum(untaxable_value_eav, na.rm=TRUE),      ## untaxable for all potential policy choices
    eav_max = sum(fmv*loa*eq2022),
    fmv = sum(fmv, na.rm=TRUE),
    pins_in_class = n(),
    all_exemptions = sum(all_exemptions),   # in EAV
    abatements = sum(exe_abate),            # in EAV
    eav_incents = sum(ifelse(class >= 600 & class < 900, eav, 0), na.rm = TRUE),
eav_incents_inTIFs = sum(ifelse(class >= 600 & class < 900 & in_tif == 1, eav, 0), na.rm = TRUE),
    ...
    eav = sum(eav),
naive_rev_forgone = round(sum(naive_rev_forgone, na.rm=TRUE)),
#untaxed_eav_incents = round(sum(untaxable_, na.rm=TRUE)) 
) %>%
  mutate(
...
muni_mc_table %>% 
  ...
  formatCurrency(c('Class FMV', 'Value not Taxable for Levy', 'TIF Increment FMV', 'FMV with Incent.Class.', 'FMV with Incent. Class. in TIFs', 'FMV in TIFs'), digits = 0)
```
...
# Slide 7 - Tax Revenue and Burden Shift
...
#eav_untaxable = sum(eav_untaxable, na.rm = TRUE),
#forgone_AV_incent = sum(forgone_AV_incent, na.rm=TRUE),
naive_rev_forgone = sum(naive_rev_forgone, na.rm=TRUE),
            taxes_paid_forlevy = sum(final_tax_to_dist, na.rm=TRUE),
            potential_taxes = sum(new_TEAV_noIncents * current_rate_avg/100, na.rm=TRUE),
           taxes_reduced = sum((new_TEAV_noIncents-eav_taxed) * current_rate_avg/100, na.rm=TRUE) ) %>% 
arrange(desc(taxes_reduced))
```

# Tax Rates

```{r calculate-munialtrates}
...
across(c(muni_current_rate, starts_with("muni_rate")), round, digits = 2)) %>%
mutate(across(starts_with("change"), abs))
...
(taxed_av*eq2022/loa)*0.25, taxed_av*eq2022)),
    ...
    new_TEAV_noExemps = sum(taxed_av*eq2022 + all_exemptions), # does not include abatements
    new_TEAV_noAbates = sum(taxed_av*eq2022 + abatements), # include only abatements, not other exemption types
  
    # amount of EAV from taxing an additional 15% of the AV if incentive properties didn't exist
    
    # TIF increment above the frozen EAV
    forgone_TIF_EAV = sum(fmv_tif_increment * loa * eq2022, na.rm=TRUE),
#naive_rev_forgone_fromincents = sum(forgone_EAV_incent * tax_code_rate, na.rm=TRUe),  
) %>%
  #cbind(table_cook) %>%
  mutate(
   forgone_EAV_incent =  new_TEAV_noIncents - eav_taxed,
...
cook_ratechange %>% select(rate_current, rate_noInc, change_noInc)
cook_ratechange %>% select(forgone_EAV_incent, forgone_EAV_incent, rate_current, rate_noInc, change_noInc)

```


```{r calculate-muniratechange-fromPINs}
...
hline_top() %>%
hline(i = c(5,10)) %>%
set_header_labels(clean_name = "Municipality", rate_current = "Current Comp.\nTax Rate", rate_noInc = "Tax Rate if\nNo Incent. Class.",
change_noInc = "Tax Rate Difference") %>%
add_header_row(values = c("County Wide", "8.66", "8.47", "0.18"), top = FALSE) %>%
  set_table_properties( layout = "autofit")
...
```{r include = FALSE}
# cook_rates <- muni_mc_table %>% 
#   filter(!clean_name  %in% c("Frankfort", "Homer Glen",  "Oak Brook", "East Dundee", "University Park",  "Bensenville", "Hinsdale", "Roselle", "Deer Park", "Deerfield"))  %>%
# ungroup() %>%
#   left_join(muni_rate) %>%
#   summarise(
#     PC = sum(classgroup_PC),
#     levy = sum(final_tax_to_dist),
#     eav_taxed = sum(eav_taxed),
# 
#     exemptions = sum(all_exemptions), # all exe_ variables except exe_abate
#     abatements = sum(abatements),
#     forgoneTIF = sum(forgone_TIF_EAV),
#     forgone_EAV_incent = sum(forgone_EAV_incent),
#     TEAV_noC6 = sum(new_TEAV_noC6),
#     TEAV_noC7 = sum(new_TEAV_noC7),
#     TEAV_noC8 = sum(new_TEAV_noC8),
#     current_TEAV = sum(eav_taxed),
#     TEAV_noExe = sum(new_TEAV_noExemps),
#     TEAV_noAbate= sum(new_TEAV_noAbates),
#     TEAV_noInc = sum(new_TEAV_noIncents),
#     TEAV_vacant_noInc = sum(new_TEAV_vacant_noIncents),
#     forgone_eav_overlap_incents_inTIFs = sum(forgone_EAV_overlap),
#     cur_munilevel_rate = mean(current_rate_avg), # Was same value for each municpality. Took mean to keep variable in dataframe. (Could have included it in group() command) 
#     eav_max = sum(eav_max)  
#     ) %>%
#   mutate(current_rate = cur_munilevel_rate) %>%
#   mutate(
#     # Absolute maximum TEAV: No Exemptions, no abatements, no TIFS, no Incentive properties
#     # Commercial and industrial assessed at 25%
#     TEAV_max = current_TEAV + exemptions + abatements + forgoneTIF + forgone_EAV_incent,
# 
#     # no exemptions or incentive classifications:
#     TEAV_neither = current_TEAV + exemptions + forgone_EAV_incent,
#     
#     rate_noExe = levy / TEAV_noExe * 100,
#     rate_noAbate = levy / TEAV_noAbate * 100,
#     rate_noInc = levy / TEAV_noInc * 100,
#     rate_neither = levy / TEAV_neither * 100, 
#     rate_noTIFs = levy / (current_TEAV + forgoneTIF) * 100,
#     rate_vacant = levy / TEAV_vacant_noInc* 100,
#     rate_lowest = levy / TEAV_max * 100,
#     rate_noC6 = levy / TEAV_noC6 * 100,
#     rate_noC7 = levy / TEAV_noC7 * 100,
#     rate_noC8 = levy / TEAV_noC8 * 100,
#     rate_avgerror = levy / eav_taxed * 100
#   ) 
# 

# 
# cook_rates <- cook_rates %>% 
#   mutate(change_error_dif = rate_avgerror - current_rate,
#     change_noExe =  rate_noExe - current_rate,
#          change_noInc = rate_noInc - current_rate,
#          change_noC6 = rate_noC6 - current_rate,
#          change_noC7 = rate_noC7 - current_rate,
#          change_noC8 = rate_noC8 - current_rate,
#          
#          change_noTIFs = rate_noTIFs - current_rate,
#          change_neither =  rate_neither - current_rate,
#          change_max = rate_lowest - current_rate,
#          change_noAbate = rate_noAbate- current_rate,
#          change_vacants = rate_vacant - current_rate)  %>%
# #mutate(across(change_noExe:change_vacants, ~ . - change_error_dif))  %>%
# mutate(across(starts_with("change"), round, digits = 2),
# across(c(current_rate, starts_with("rate")), round, digits = 2)) %>%
# mutate(across(starts_with("change"), abs))
...
add_header_row(values = c("County Wide", "8.66", "8.95", "0.19"), top = FALSE) %>%
add_footer("There are 26 municipalities that do not use incentives and have a majority of their taxable EAV within Cook County.", top = FALSE) %>%
  set_table_properties( layout = "autofit")
```
