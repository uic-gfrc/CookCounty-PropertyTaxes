...
ptax_pins <- ptax_pins %>% 
  ...
  mutate(
    ...
    fmv_incents_tif_increment = ifelse(between(class, 600, 899) & final_tax_to_tif > 0 , 
                               ((final_tax_to_tif / (tax_code_rate/100)) / eq2022 ) / loa, 0), 
       untaxed_fmv = fmv - taxed_fmv, 
    untaxed_eav_incents = ifelse(between(class, 600, 899), taxable_eav_fromincents - taxed_eav, 0),
    naive_rev_forgone =  untaxed_eav_incents * tax_code_rate/100,
...
untaxed_eav_incents = round(sum(untaxed_eav_incents, na.rm=TRUE)) ) %>%
  mutate(
...
naive_rev_forgone = sum(naive_rev_forgone, na.rm=TRUE),
           ...
           taxes_reduced = sum((new_TEAV_noIncents-eav_taxed) * current_rate_avg/100, na.rm=TRUE) ) %>% arrange(desc(taxes_reduced))
```
