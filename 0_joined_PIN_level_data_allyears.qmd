...
pin_data <- if (i < 2024) {
  ...
  pin_data <- pin_data |>
    ...
    mutate(# zero_bill = ifelse(eav <= all_exemptions, 1, 0),  ## old way of creating zero bill but has missing vet exemptions data problem
      ...
      has_DV_exemp_adj = ifelse(exe_missing_disvet > 0 | (exe_vet_dis_lt50 + exe_vet_dis_50_69 + exe_vet_dis_ge70 + exe_vet_dis_100 ) > 0, 1, 0),
...
#                                ifelse(in_tif == 1, (taxable_eav-all_exemptions)*tax_code_rate * (1-tif_distribution_pct), "CHECK ME")),
#     final_tax_to_tif = ifelse(in_tif == 1, (taxable_eav-all_exemptions)*tax_code_rate * tif_distribution_pct, 0),
#     
#     final_tax_to_dist_adj = ifelse(in_tif == 0, (taxable_eav_adj-all_exemptions)*tax_code_rate, 
#                                ifelse(in_tif == 1, (taxable_eav-all_exemptions)*tax_code_rate* (1-tif_distribution_pct), "CHECK ME")),
#     final_tax_to_tif_adj = ifelse(in_tif == 1, (taxable_eav_adj-all_exemptions)*tax_code_rate * tif_distribution_pct, 0)) |>
#   
...
#    av = av_clerk, # basic av variable is the final assessed value from clerk data
    ...
    #                            ifelse(in_tif == 1, (taxed_eav_AWM)*tax_code_rate * (1-tif_distribution_pct), 0)),
    # final_tax_to_tif_AWM = ifelse(in_tif == 1, (taxed_eav_AWM-all_exemptions)*tax_code_rate * tif_distribution_pct, 0),
    # -----------------------
...
muni_ratechange <- pin_data |>
  ...
  summarize(
    ...
    eav_tif_increment = sum(tif_increment_eav_bill_allocated, na.rm=TRUE),
    pins_in_tif = sum(in_tif, na.rm = TRUE),
    pins_with_tif_increment = sum(has_tif_increment, na.rm = TRUE),
  ...
  mutate(
    ...
    forgone_EAV_incent = new_TEAV_noIncents - muni_eav_taxed
    ) |>
  mutate(
