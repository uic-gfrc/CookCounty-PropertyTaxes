  ...
  pin_data <- pin_data |>
    ...
    if(i < 2024) {
      left_join(tif_distrib, by ="tax_code_num")|>
        ...
        mutate(distribution_pct_to_tif = is.na(tax_code_distribution_pct), 0, tax_code_distribution_pct)
    } else(i >=2024) {
    left_join(pin_tif_distrib)   |>
      mutate(distribution_pct_to_tif = is.na(pin_distribution_pct), 0, pin_distribution_pct/100) 
      } |>
    ...
    mutate(
      ...
      in_tif = ifelse(tax_code_num %in% tif_distrib$tax_code_num | pin %in% pin_distribution_pct$pin , 1, 0),
