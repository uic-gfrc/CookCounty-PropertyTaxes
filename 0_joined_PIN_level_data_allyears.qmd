...
write_csv(pin_data, file = paste0("./Output/Dont_Upload/0_joined_PIN_data_", i, "_test.csv"))
 ...
 # Tax Rate change -------------------------------------
 # calculates straight to municipality level. Less accurate than using tax code level summaries of change
...
# taxcode_rate_scenarios <- pin_data |> 
#   group_by(tax_code_num, tax_code_eav, tax_code_frozen_eav, tax_code_distribution_pct, clean_name) |>
#   
#   summarize(
#     pincount = n(),
#     pins_withincents = sum(ifelse(class >= 600 & class < 900, 1,0)),
...
#     untaxable_value_av = sum(untaxable_value_av, na.rm=TRUE),
#     av = sum(av_clerk), 
#     eav_taxed = sum(taxed_av*eq_factor, na.rm=T), 
#     final_tax_to_dist_AWM = sum(final_tax_to_dist_AWM, na.rm=TRUE),
#     final_tax_to_tif_AWM = sum(final_tax_to_tif_AWM, na.rm=TRUE),
#     eav_untaxable = sum(untaxable_value_eav, na.rm=TRUE),
#     eav_tif_increment = sum(final_tax_to_tif/tax_code_rate, na.rm=TRUE),
#     eav_max = sum(fmv*loa*eq_factor, na.rm=TRUE),
#     pins_in_tc = n(),
...
#   ) |>
#   
#   mutate(across(.cols = av_taxed:new_TEAV_vacant_noIncents, round)) |>
#  
#    mutate(
#     new_TEAV_noExemps = eav_taxed + all_exemptions, # does not include abatements
#     new_TEAV_noAbates = eav_taxed + abatements, # include only abatements, not other exemption types
...
#     TEAV_neither = eav_taxed + all_exemptions + forgone_EAV_incent,
#     
#     ## revenue collected within tax district (held constant) / new tax base
#     rate_noExe = final_tax_to_dist / new_TEAV_noExemps,
#     rate_noAbate = final_tax_to_dist / new_TEAV_noAbates,
#     rate_noInc = final_tax_to_dist / new_TEAV_noIncents ,
#     rate_neither = final_tax_to_dist / TEAV_neither, 
...
#     # change_lowest = rate_current - rate_lowest
#     )  |>
#   mutate(across(contains("rate_"), ~ ifelse(. <=0, NA, .))) |>
#   
#   mutate(across(contains("rate_"), round, digits = 5)) |>
#   
#   mutate(across(contains("change_"), round, digits = 5)) |> 
#   
#   ungroup() |>
#   select(-c(tax_code_eav, tax_code_frozen_eav, tax_code_distribution_pct))
# 
# write_csv(taxcode_rate_scenarios, paste0("./Output/taxcode_rate_scenarios", i, "_test.csv"))
# 
#  taxcode_taxrates <- taxcode_rate_scenarios |> 
#    select(tax_code_num, current_rate_avg, rate_noExe:rate_current)
# 
#  
 ...
 #    # fmv_industrial = sum(ifelse(class %in% industrial_classes, fmv, 0), na.rm = TRUE),
 #    # fmv_commercial = sum(ifelse(class %in% commercial_classes, fmv, 0), na.rm = TRUE),
 #    rate_current = mean(rate_current, na.rm=TRUE),
 #    avg_C2_bill_noexe = mean(ifelse(between(class,200,299) & all_exemptions == 0, (final_tax_to_dist + final_tax_to_tif), NA), na.rm=TRUE),
 #    avg_C2_bill_withexe = mean(ifelse(between(class,200,299) & all_exemptions > 0, (final_tax_to_dist + final_tax_to_tif), NA), na.rm=TRUE),
 #    av_taxed = sum(taxed_av, na.rm = TRUE),
 #    untaxable_value_av = sum(untaxable_value_av, na.rm=TRUE),
 ...
 #                                           0, taxed_av*eq_factor), na.rm=T),
 #        
 #    new_TEAV_noExemps = sum(taxed_eav + all_exemptions), # does not include abatements
 #    new_TEAV_noAbates = sum(taxed_eav + exe_abate), # include only abatements, not other exemption types
 #    
 # 
 #        # Absolute maximum TEAV: No Exemptions, no abatements, no TIFS, no Incentive properties
 #    # Commercial and industrial assessed at 25%
 # 
 #    # no exemptions or incentive classifications:
 #    forgone_EAV_incent = sum(ifelse(class >=600 & class < 900,
 #                                    (taxed_av*eq_factor/loa)*0.25, taxed_av*eq_factor)),
 # 
 #    TEAV_neither = sum(taxed_eav + all_exemptions + ifelse(class >=600 & class < 900,
 #                                    (taxed_av*eq_factor/loa)*0.25, taxed_av*eq_factor)),
 #    abatements = sum(exe_abate),            # in EAV
 #        eav_taxed = sum(taxed_av*eq_factor, na.rm=T), 
 #        # amount of EAV from taxing an additional 15% of the AV if incentive properties didn't exist
 #    eav_tif_increment = sum(final_tax_to_tif/tax_code_rate, na.rm=TRUE),
 #    TEAV_max = sum(taxed_eav + all_exemptions + exe_abate + eav_tif_increment + forgone_EAV_incent),
 #      
 #    rate_noExe = mean(rate_noExe, na.rm=T),
 #    rate_noAbate = mean(rate_noAbate, na.rm=T),
 #    rate_noInc = mean(rate_noInc, na.rm=T),
 #    rate_neither = mean(rate_neither, na.rm=T),
 #    rate_noTIFs = mean(rate_noTIFs, na.rm=T),
 #    rate_vacant = mean(rate_vacant, na.rm=T),
 #    rate_lowest = mean(rate_lowest, na.rm=T),
 #    rate_current = mean(rate_current, na.rm=T),
 #  
 #  ) |>
 #  mutate(
 #    change_noInc = rate_current - rate_noInc,
...
## taxrates for just exemption scenario. remove for expanded version above soon.
...
#   mutate(cur_comp_TC_rate = final_tax_to_dist/taxed_eav,
#          cur_comp_TC_rate = ifelse(is.na(cur_comp_TC_rate), 0, cur_comp_TC_rate)) |>
#   mutate(current_taxable_eav = taxed_eav,
#          new_taxable_eav = current_taxable_eav + HO_exemps) |>
#   mutate(new_comp_TC_rate = (final_tax_to_dist / new_taxable_eav)) |>
...
#   select(tax_code_num, cur_comp_TC_rate, new_comp_TC_rate, current_taxable_eav, new_taxable_eav, everything())
 ...
 #   select(tax_code_num, cur_comp_TC_rate, new_comp_TC_rate, current_exemptions, HO_exemps, pincount)

# write_csv(taxcodes_current, paste0("./Output/taxcodes_current_", i, "_test.csv"))
