...
pin_data <- pin_data |>
  ...
  left_join(tif_distrib, by ="tax_code_num") |>
    mutate( 
      ...
      final_tax_to_tif = ifelse(tax_code_frozen_eav < tax_code_eav,
...
pin_data <- pin_data |>
  ...
  mutate(# zero_bill = ifelse(eav <= all_exemptions, 1, 0),  ## old way of creating zero bill but has missing vet exemptions data problem
    ...
    in_tif_andpays_revtotif = ifelse(in_tif == 1 & tax_code_eav > tax_code_frozen_eav, 1, 0 )
  )
...
pin_data <- pin_data %>%
  ...
  mutate(
    av = av_clerk,
    final_tax_to_dist_AWM = ifelse(in_tif == 0, (taxed_eav-all_exemptions)*tax_code_rate, 
                               ifelse(in_tif == 1, (taxed_eav-all_exemptions)*tax_code_rate * (1-tax_code_distribution_pct), 0)),
    final_tax_to_tif_AWM = ifelse(in_tif == 1, (taxed_eav-all_exemptions)*tax_code_rate * tax_code_distribution_pct, 0),
    
    final_tax_to_dist_adj_AWM = ifelse(in_tif == 0, (taxed_eav-all_exemptions)*tax_code_rate, 
                               ifelse(in_tif == 1, (taxed_eav-all_exemptions)*tax_code_rate* (1-tax_code_distribution_pct), 0)),
    final_tax_to_tif_adj_AWM = ifelse(in_tif == 1, (taxed_eav-all_exemptions)*tax_code_rate * tax_code_distribution_pct, 0),
...
muni_ratechange <- pin_data |> 
  ...
  summarize(
    ...
    current_rate_avg = mean(tax_code_rate*100),
...
mc_burden <- pin_data |>
  ...
  summarize(group_taxbase= sum(taxed_eav),
            group_taxes_current = sum(taxed_eav * (tax_code_rate)),
            hyp_group_taxbase = sum(taxed_eav + all_exemptions, na.rm = T),
            hyp_group_taxes = sum( (taxed_eav + all_exemptions) * (new_comp_TC_rate), na.rm = T)
     )  |>
...
burden_shift <- pin_data |>
  ...
  summarize(group_taxbase = sum(taxed_eav, na.rm = T),
            group_taxes_current = sum(taxed_eav * (cur_comp_TC_rate), na.rm = T),
            hyp_group_taxbase = sum(taxed_eav + all_exemptions, na.rm = T),
            hyp_group_taxes = sum((taxed_eav + all_exemptions) * (new_comp_TC_rate), na.rm = T))  |>
  ungroup() |>
  mutate(
