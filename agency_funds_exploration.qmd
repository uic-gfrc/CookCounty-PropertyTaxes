...
According to PTAXIM agency data, there are 810 agencies with levies in 2021 and 2022, and 815 in 2023.
...
#| tbl-cap: "**MEMO TABLE - Outliers: Largest Agency Level Tax Rate Change due to Fund 408** Data from PTAXSIM database (agency_fund table)."
...
refund_all |> 
  ...
  filter(total_refunds != 0) |> DT::datatable(rownames = FALSE)
```
...
refund_all |>
  ...
  summarize(refunds = sum(total_refunds, na.rm = T),
             n = n(),
            n_with_refunds = sum(ifelse(total_refunds!= 0, 1, 0)))
```
...
#  filter(!is.na(home_rule_ind) & is.finite(share) ) |>
  ...
  labs(title = "Refunds / Levy Amount",
       caption = "Excludes agencies that had refunds but no levy. 
       Excludes agencies with refund share greater than 30% of their levy.",
...
`Refund_all` had `r paste(count(refund_all))` observations and `adjust_all` had `r paste(count(adjust_all))` observations. _Note that many of the observations in `refund_all` had _
