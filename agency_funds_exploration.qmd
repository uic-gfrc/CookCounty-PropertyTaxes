...
final_df |>
  filter(!is.na(total_recap)) |>
  filter(fund408 != levy_adjustment)
```
...
final_df_2 <- final_df |>
  #filter(year > 2020) |>
 # select(-c(major_type)) |>
  mutate(adjust_eligible = ifelse(adjust_eligible == "Eligible", 1, 0),
         adjust_eligible = ifelse(is.na(adjust_eligible), 0, adjust_eligible))

df_mismatch <- final_df_2 |>
  filter(refunded_coe != total_coe_refunds)

final_df_3 <- final_df_2 |>
  select(year, agency_num, agency_name, )
