...
## Fund 408 data from clerk/treasurer

```{r}
#| label: tbl-fund408-fromtreasurer
#| tbl-cap: "Calculates difference in Fund 408 amounts from PTAXSIM and the values from the `Levy Adjustment Fund 408 Extension 2021-2023.xlsx`"

fund408_fromtreas <- readxl::read_xlsx("PTAB/Levy Adjustment Fund 408 Extension 2021-2023.xlsx") 
...
agency_fund408_sums |> 
  ...
  select(year, agency_name.x, fund408, FundExtension ,agency_num, pct_inCook) |>
  mutate(difference = round(fund408-FundExtension, digits = 0)) |> 
...
adjust_all <- rbind(adjust2021, adjust2022, adjust2023, adjust2024) |>
  ...
  # fix agency names
  mutate(ifelse(agency_number == "070390000", agency_name == "INVERNESS FIRE PROT DIST (FKA PALATINE)"), agency_name) |>
  mutate(ifelse(agency_number == "140020000", agency_name == "BRONZEVILLE EXPANDED MENTAL HEALTH SERV DIST"), agency_name) |>
  mutate(ifelse(agency_number == "020070003", agency_name == "TOWN ELK GROVE ROAD FUND"), agency_name) |>
  mutate(ifelse(agency_number == "042220000", agency_name == "GLENBROOK HIGH SCHOOL DISTRICT 225"), agency_name)


adjust_all |> DT::datatable(rownames = FALSE)
```
...
refund_all |> filter(total_refunds != 0) |> DT::datatable(rownames = FALSE)
```
...
```{r}
adjust_all |>  
  filter(aggregate_refunds != 0 & is.finite(recap_levyshare) ) |>
  ggplot() + 
...
deciles |> filter(probs == 0.95 | probs == .50)
```
...
levyshare_byagency |> filter(year == 2024) |> DT::datatable(rownames = FALSE)
```

> **Values differ between the table above and below due to the year of the levy used in the denominator!**

```{r tbl-mtable1}
#| tbl-cap: "**Alternative Replication for 2024.** Recapture Levy Share for tax year 2024. Uses levy amount from fund 408 / total_final_levy to create recap levyshare. Uses only PTAXSIM data." 
...
```{r tbl-mtable2}
#| tbl-cap: "Uses total_refunds/total_final_levy to create recap levyshare. Total Refunds is from the clerk & treasurers files with refund information and the levy amount is from ptaxsim. 2021-2024 refund values. Excludes agencies that did not have a levy amount."
# using the refund data for all agencies, both homerule and non homerule.

refund_all |> 
  left_join(agency_info, by = c("agency_number" = "agency_num", "year")) |>
  group_by(year) |>
  mutate(recap_levyshare =  abs(total_refunds) / total_final_levy) |>
  arrange(year, desc(recap_levyshare)) |>
...
```{r tbl-mtable3}
#| tbl-cap: "Levy Share for Tax Year 2021. Uses levy amount from fund 408 / total_final_levy to create recap levyshare. Only PTAXSIM data." 

levyshare_byagency |> filter(year == 2021)|> DT::datatable(rownames = FALSE)
```


```{r tbl-mtable4}
#| tbl-cap: "Levy Share for Tax Year 2022. Uses levy amount from fund 408 / total_final_levy to create recap levyshare. Only PTAXSIM data." 

levyshare_byagency |> filter(year == 2022)|> DT::datatable(rownames = FALSE)
```


```{r tbl-mtable5}
#| tbl-cap: "Levy Share for Tax Year 2023. Uses levy amount from fund 408 / total_final_levy to create recap levyshare. Only PTAXSIM data." 

levyshare_byagency |> filter(year == 2023)|> DT::datatable(rownames = FALSE)
```
...
```{r}
#| tbl-cap: "Replicated most recent email and table in Merriman_recapture_memo.docx file."
...
```{r fig-table6}
#| fig-cap: "Uses total_refunds / levy from the refund_all dataframe."
...
refund_all |> 
 ...
 # geom_point(aes(x=year, y = share, color = as.character(home_rule_ind))) + 
  # geom_text_repel(data = . %>% filter(share > 0.2),aes(x=year, y = share, label = agency_name), nudge_y =.05 ) + 
...
refund_all |> 
  ...
  labs(title = "Refunds / Levy Amount",
       caption = "Agencies at 100% were Infinite: They did not have a levy, but they did have a refund.",
       color = "Home Rule", x = "Refund Share of Levy",
       y = "Share of Levy") +
  facet_wrap(vars(year, home_rule_ind))
```
...
refund_all |> 
  ...
  labs(title = "Refunds / Levy Amount",
       ...
       Excludes agencies with refund share greater than 0.3. 
       41 observations total are excluded.",
       color = "Home Rule", x = "Refund Share of Levy",
       y = "Share of Levy") +
  facet_wrap(vars(year, home_rule_ind))
...
aes(x=year, y = share, label = agency_name), nudge_y =.05 ) + 
  scale_y_continuous(limits = c(0, 0.25), breaks = c(0, .1, .25)) + 
  labs(title = "Refunds / Levy Amount",
       caption = "Agencies at 100% were Infinite: They did not have a levy, but they did have a refund.",
       color = "Home Rule", x=element_blank(),
       y = "Share of Levy")
...
#| tbl-cap: "Over 8000 agencies exist during 2024, but only 2,390 are actively taxing properties (i.e. have a levy > $0)."
...
refund_all |> 
  ...
  select(agency_name, share, year, total_refunds, total_final_levy) |>
  DT::datatable(rownames = FALSE)
```
...
#| tbl-caption: "Joined data from the refund files and recapture files from the Clerk and Treasurer."
...
#| tbl-cap: "Final data table with renamed variables."
...
```{r}
#| label: tbl-taxcode-aggregation2022
#| tbl-cap: "Aggregating up to Tax Code Level: 2022"
...
From googleing random news articles and public documents, we know that taxing agencies in Oak Lawn opt out,


Some differences in tables below

- School district 35: Glencoe   
- School District 145:   
- Palatine SD 211: submitted motion to abate recapture amount. That is not seen in the adjust_all file. So it either got voted down OR the adjustment files do not reflect those that submitted abatements.  
...
Add line for average or median that is darker? If zooming in on specific taxing agencies in order to have a refernece point.
...
df |> 
  ...
  labs(title = "Levy Adjustment Variable from Treasurer", x= element_blank(),
       caption = "Ratio is recapture for 1 year / recapture for previous year.")
...
df |> 
  ...
  geom_line(aes(x=as.character(year), y = ratio, color = major_type, alpha = .5, group = agency_number))+
   geom_point(aes(x=as.character(year), y = ratio, color = major_type, alpha = .5) ) +
  labs(title = "Eligible Agencies Only. Ratio = Refund / Levy.", 
