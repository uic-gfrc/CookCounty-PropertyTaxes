...
In 2022, 95% of  taxing agencies had refunds equal to less than 4.6% of their levy. In 2024, 95% of agencies had 2.7% of their levy be refunded. (When using the refund amount / last year's levy amount. Which is not quite correct.)
...
adjust_all |>
 group_by(agency_number) |>
  summarize(n = n()) |>
  filter(n != 4) # 70 agencies did not exist all 4 years
...
`Refund_all` had `r count(refund_all)` observations and `adjust_all` had `r count(adjust_all)` observatins. 
