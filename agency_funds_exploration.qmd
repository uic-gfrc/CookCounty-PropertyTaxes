...
taxing_agencies <- agency_funds |> 
  ...
  summarize(across(c(levy:final_rate, fund408, not_recap), sum, na.rm=TRUE))
...
#taxing_agencies <- left_join(taxing_agencies, taxcodes, by = c("year","agency_num"))
...
recap_all <- rbind(recap2021, recap2022, recap2023) |>
  rename(agency_number = `Agency Number`,
         ...
         recaptured_sp_ptab = `SP/PTAB Recouped`,
         recaptured_coe = `CofE (Over-Assessment)`) |>
  mutate(across(where(is.numeric), round, digits =0)) |>
  filter(!is.na(agency_number) &
           ((recaptured_sp_ptab!=0 & !is.na(recaptured_sp_ptab)) | 
               (recaptured_coe!=0 & !is.na(recaptured_coe))))
```
...
adjust_all <- rbind(adjust2021, adjust2022, adjust2023, adjust2024
  ...
  mutate(prior_levy_adjustment = as.numeric(prior_levy_adjustment)) |>
  mutate(across(.cols=c(total_tax:prior_levy_adjustment), round, digits = 0)) |>
  filter(!is.na(agency_number) &
...
write_csv(recap_all, "output/recap_all.csv")
```
...
adjust_recap_all <- adjust_recap_all |>
  mutate(
    ## NO!!!##
    # total_ptab_refunds = ifelse(is.na(total_ptab_refunds), sp_ptab,
    #                             total_ptab_refunds),
...
final_df <- left_join(taxing_agencies, adjust_recap_all)
```