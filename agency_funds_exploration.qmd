...
> These are descriptive stats for non-homerule taxing agencies that are eligible for recapture! Also fairly certain that the years should be n-1 because the file name indicates the when the file was created, but it is for the previous tax year. 
...
```{r tbl-mtable1}
#| tbl-cap: "Recapture Levy Share for tax year 2024. Uses levy amount from fund 408 / total_final_levy to create recap levyshare. Uses only PTAXSIM data." 
...
```{r tbl-mtable2}
#| tbl-cap: "Uses total_refunds/total_final_levy to create recap levyshare. Total Refunds is from the clerk & treasurers files with refund information and the levy amount is from ptaxsim. 2021-2024 refund values."
...
refund_all |> 
  ...
  filter(recap_levyshare > 0) |> DT::datatable(rownames = FALSE)
...
```{r tbl-table6}
#| tbl-cap: "Uses total_refunds / levy from the refund_all dataframe."
...
eligible |> filter(fund408==0 | is.na(levy_adjustment)) |> distinct(agency_name) |> DT::datatable(rownames = FALSE)

eligible |> filter(levy_adjustment==0 | is.na(levy_adjustment)) |> distinct(agency_name) |> DT::datatable(rownames = FALSE)
...
```{r}
df |> 
  filter(year > 2020 & adjust_eligible == "Eligible" & year < 2024) |> 
  group_by(agency_name, agency_number) |>
  arrange(agency_number, year) |>
  mutate(
  ...
  geom_line(aes(x=as.character(year), y = ratio, color = major_type, alpha = .5, group = agency_number))+
   geom_point(aes(x=as.character(year), y = ratio, color = major_type, alpha = .5) ) +
  labs(title = "Eligible Agencies Only. Ratio = Refund / Levy.", caption = "Looked the same as using Levy Adjustment / Levy", x= element_blank())
