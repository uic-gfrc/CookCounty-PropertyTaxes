...
agency_funds |> filter(fund_num==408) |>
  ...
  ggplot() + 
  geom_line(aes(x=as.character(year), y=final_rate, group=agency_name_short, color = major_type, alpha = .3)) + 
  alea_theme()+
...
#| tbl-cap: "**Observations within the 95% percentile** Data from PTAXSIM database (agency_fund table)."
...
filtered_df <- agency_funds |>
  ...
  select(-c(loss_pct,levy_plus_loss, rate_ceiling, max_levy, prelim_rate, ptell_reduced_levy, ptell_reduced_ind)) |>
...
#| tbl-cap: "**Data Comparison** Calculates difference in Fund 408 amounts from PTAXSIM and the values from the `Levy Adjustment Fund 408 Extension 2021-2023.xlsx`. Fund 408 data from Treasurer (`FundExtension`) is joined to fund 408 data (`fund408`, `total_final_levy`, `total_ext`) in PTAXSIM database (created by CCAO)."
...
> Taxing districts with the largest differences appear seem to generally be border crossers so it might just be a data issue of what is included in ptaxsim vs what is included in the treasurer's files.
...
#| tbl-cap: "**Recaptured levy amounts** `adjust_all` has all levy adjustments from the new recapture legislation since 2021. Variables labeled with `refund` represent the amount that was recaptured due to refunds for eligible taxing agencies"
...
adjust_all <- rbind(adjust2021, adjust2022, adjust2023, adjust2024) |>
  ...
  #und(levy_adjustment/lastyear_levy, digits = 5)) |> 
  # fix agency names
...
refund_all |> select(year,agency_name, refund_levyshare, everything()) |> filter(total_refunds != 0) |> DT::datatable(rownames = FALSE)
```
...
adjust_all |>  
  ...
  facet_wrap(~year) +
  labs(caption = "Excludes observations with levy shares > 25% (only a couple outliers)", x = "Share of Levy Recaptured", y = "# Taxing Agencies")
```
...
#| tbl-cap: "Recapture's share of Agencies' Levies. Uses AWM's method of joining in ptaxsim levy data that corresponds with the recapture amount's year." 
...
In 2022, 95% of actively taxing taxing agencies had refunds equal to less than 4.5% of their levy. In 2021 and 2023, 95% of agencies had 3% of their levy be refunded. 


```{r tbl-mtable3}
#| tbl-cap: "Levy Share for Tax Year 2021. Uses levy adjustment / total_final_levy to create recap levyshare." 

awm_levyshare_byagency |> filter(year == 2021)|> DT::datatable(rownames = FALSE)
```
...
awm_levyshare_byagency |> filter(year == 2022)|> DT::datatable(rownames = FALSE)
```
...
awm_levyshare_byagency |> filter(year == 2023)|> DT::datatable(rownames = FALSE)
```
...
```{r}
#| tbl-cap: "Replicated most recent email and table in Merriman_recapture_memo.docx file. It is NOT actually for refunds, but recaptured refunds."
...
```{r fig-table6, eval=FALSE, include=TRUE}
...
refund_all |> 
 ...
 # geom_point(aes(x=year, y = share, color = as.character(home_rule_ind))) + 
  ...
  scale_y_continuous(limits = c(0, 1.2), breaks = c(0, .25, .5, .75, 1)) + alea_theme() +
...
#| fig-cap: "Histograms of total_refunds / levy from the refund_all dataframe. Excludes 41 observations with shares larger than 30% to increase legibility."
...
aes(x=year, y = share, label = agency_name), nudge_y =.05 ) + 
  alea_theme() +
  scale_y_continuous(limits = c(0, 0.25), breaks = c(0, .1, .25), label = scales::percent) + 
  labs(title = "Refunds / Levy Amount",
       caption = "Agencies at 100% were Infinite: They did not have a levy, but they did have a refund.",
       color = "Home Rule", x=element_blank(),
       y = "Share of Levy")
...
#| tbl-cap: "Over 8000 observations exist for the 3 years of data, but only 2,390 are actively taxing properties (i.e. have a levy > $0)."
...
awm_levyshare_byagency <- refund_all |> 
  #mutate(recap_levyshare =  round(abs(total_refunds) / total_final_levy, digits = 3) ) |>
...
refund_all |>
  ...
  summarize(n = n()) |>
  filter(n != 4) # 216 agencies did not exist all 4 years in the refund data
