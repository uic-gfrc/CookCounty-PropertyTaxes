...
fig_muni_perc_of_comval6C <- muni_comval_percentages %>% 
  ...
  geom_sf(aes(geometry = geometry),
          color = "black",
          lwd = .05)+ 
  labs(title = "% of Commercial & Industrial FMV (per Muni) \nwith Incentive Class 6") +
  theme_void() + 
  theme(axis.ticks = element_blank(), axis.text = element_blank()) +
  scale_fill_steps2(
    high = "brown",
   # nice.breaks = FALSE,
   breaks = c(0, 0.05, .10, .20, 0.4, .60, .8),
    show.limits=TRUE,
    na.value = "gray",
    name = "% of Com&Ind FMV",
    label = scales::percent)
...
fig_muni_perc_of_comvalC7 <- muni_comval_percentages %>% 
  ...
  geom_sf(aes(geometry = geometry),
          color = "black",
          lwd = .05)+ 
  labs(title = "% of Commercial & Industrial FMV (per Muni) that has Incentive Classifications",
       subtitle = "FMV is AV / assessment ratio for property class") +
  theme_void() + 
  theme(axis.ticks = element_blank(), axis.text = element_blank()) +
  scale_fill_steps2(
    high = "brown",
limits = c(0, .6),
breaks = c(0, 0.025, 0.05, .10, .15,  0.4, .60),
    show.limits=TRUE,
    na.value = "gray",
    name = "% of Com&Ind FMV",
    label = scales::percent)
