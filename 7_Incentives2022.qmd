...
projects <- read_csv("Output/pin_project_crosswalk.csv")

theme_set(theme_void()) 


theme_AWM <-   theme_void() + 
...
#no_incentives <- readRDS("no_incentives_formap.rds")
```

# Incentive Properties In Cook County, IL

```{r}
muni_class_sums <- muni_class_sums %>%   
...
muni_class_sums %>% 
  filter(year == 2022) %>%
  group_by(Alea_cat) %>% 
  summarize(fmv = sum(av / assess_ratio),
            cur_TEAV = sum(current_taxable_eav),
            av = sum(av),
            pincount = sum(pins_in_muni))

building_summaries <- muni_class_sums %>% 
  filter(year == 2022) %>%
...
building_summaries2 <- muni_class_sums %>% 
  filter(year == 2022) %>%
  group_by(clean_name, Alea_cat, incent_prop) %>% 
  summarize(cur_TEAV = sum(current_taxable_eav),
            av = sum(av),
            pincount = sum(pins_in_muni))

incentive_pins <- muni_class_sums %>% 
  filter(year == 2022) %>% 
  filter(class >= 600 &  class <= 899)

residential_pins <- muni_class_sums %>% 
  filter(year == 2022)  %>% 
...
# incentive_pins %>% 
#   reframe(IncentiveProp_PINcount = sum(pins_in_muni),
...
muni_ind_com_totalvalue <- muni_class_sums %>% 
  mutate(fmv = av/assess_ratio) %>%
  group_by(clean_name) %>%
  mutate(muni_total_fmv = sum(fmv, na.rm=TRUE)) %>% 
  ungroup() %>%
  filter(class > 499 & class < 900) %>% 
  group_by(clean_name, Alea_cat, incent_prop, muni_total_fmv) %>%
  summarize(fmv = sum(fmv),
    pincount = sum(pins_in_muni),
            TEAV = sum(current_taxable_eav),
            av = sum(av)) %>% 
  group_by(clean_name) %>% 
  mutate(comval_fmv = sum(fmv)) %>%
  ungroup() %>% 
  group_by(clean_name, Alea_cat) %>%
  mutate(type_fmv = sum(fmv)) %>% ungroup() %>%
  group_by(clean_name, incent_prop) %>%
  mutate(incent_fmv = sum(fmv)) 

muni_ind_com_totalvalue


muni_fmv_MC_totals <- muni_class_sums %>% 
  mutate(fmv = av/assess_ratio) %>%
  group_by(clean_name) %>%
  mutate(muni_total_fmv = sum(fmv, na.rm=TRUE)) %>% 
  ungroup() %>%
  filter(class > 499 & class < 900) %>% 
  group_by(clean_name, Alea_cat, incent_prop, major_class_code, muni_total_fmv) %>%
  summarize(fmv = sum(fmv),
    pincount = sum(pins_in_muni),
            TEAV = sum(current_taxable_eav),
            av = sum(av)) %>% 
...
muni_ind_com_counts <- incentive_pins %>% 
  filter(class > 599 & class < 900) %>% 
  group_by(clean_name, Alea_cat) %>%
  summarize(pincount = sum(pins_in_muni),
            TEAV = sum(current_taxable_eav),
            av = sum(av)) %>%
  pivot_wider(id_cols = clean_name, 
              names_from = "Alea_cat", 
              values_from = "pincount") %>%
  select( Municipality = clean_name, everything() ) %>% 
  mutate(Municipality = ifelse(is.na(Municipality), "Unincorporated", Municipality)) %>%
  arrange(Municipality)
```
...
no_commercial_incentives <- muni_ind_com_counts %>% filter(is.na(Commercial)) %>% select(clean_name = Municipality)
no_commercial_incentives


no_industrial_incentives <- muni_ind_com_counts %>% filter(is.na(Industrial)) %>% select(clean_name = Municipality)
no_industrial_incentives

useboth <- muni_ind_com_counts %>% filter(!is.na(Industrial)& !is.na(Commercial)) %>% select(clean_name = Municipality)
useboth

```

99 municipalities use incentives to some extent.

60 use both industrial and commercial incentive property classes. 

- 12 municipalities do not use Industrial incentives (87 use industrial incentives classes)  

- 27 municipalities do not use Commercial incentives (72 use commercial incentive classes.)  

```{r}
# csv read in here is only the 2021 incentive pins! 
# incentive_pins_w_projects <- # read_csv("./Output/7_output_incentive_classes.csv") %>%
#   read_csv("./Output/Dont_Upload/0_joined_PIN_data_2022.csv") %>% 
#   left_join(projects) %>%
...
# use other keypin file for now
ptax_pins <- ptax_pins %>% left_join(projects, by = "pin")
...
class_dict <- read_csv("./Necessary_Files/class_dict_expanded.csv") %>%
  select(class_code, comparable_props, Alea_cat, incent_prop, major_class_code, assess_ratio)

ptax_pins <- ptax_pins %>% 
  left_join(class_dict, by =  c("class" = "class_code")) %>%
  mutate(clean_name = ifelse(is.na(clean_name), "0_Unincorporated", clean_name))

# incentive pins and projects for 2022
incentive_pins_w_projects <- ptax_pins %>% filter(between(class, 600, 899))
```
...
incentive_pins_w_projects %>% 
  group_by(clean_name, keypin) %>%
  summarize(pincount = n(),
            av = sum(av)) %>%
  ungroup() %>% 
  group_by(clean_name) %>%
...
incentive_pins %>% 
  ...
  filter(class > 599 & class < 900) %>% 
  group_by(clean_name, Township, major_class_code) %>%
  summarize(pincount = n()) %>%
  pivot_wider(id_cols = c(clean_name, Township), 
              names_from = "major_class_code", 
              values_from = "pincount")  %>% 
  select( Municipality = clean_name, Township ,"6A", "6B", "6C", "7A", "7B", "8") %>% 
  mutate(Municipality = ifelse(is.na(Municipality), "Unincorporated", Municipality)) %>%
  arrange(Municipality)
...
incentive_pins_w_projects %>% 
  ...
  filter(class > 599 & class < 900) %>% 
  group_by(clean_name, major_class_code) %>%
  summarize(projects =  n_distinct(keypin)) %>%
  pivot_wider(id_cols = c(clean_name), 
              names_from = "major_class_code", 
              values_from = "projects")  %>% 
  select( Municipality = clean_name ,"6A", "6B", "6C", "7A", "7B", "8") %>% 
  mutate(Municipality = ifelse(is.na(Municipality), "Unincorporated", Municipality)) %>%
  arrange(Municipality)
...
nbh_sum <- read_csv("./Output/ptaxsim_nbh_MC_summary_2022.csv") %>%     mutate(nbhd_code = as.character(nbhd_code))

 


incentives_props <- nbh_sum %>% 
  filter(major_class_code %in% c("6A", "6B", "6C", "7A", "7B", "8")) %>%
  mutate(nbhd_code = as.character(nbhd_code))


incentive_props <-  incentives_props %>% 
  full_join(NBHs, by = c("nbhd_code" = "town_nbhd"))
```

## All Incentive Types
### Neighborhood - All Incentive Classes
...
### Municipality - All Incentive Types Together

```{r}
#| fig-height: 6
#| fig-width: 8
# Add in names that merge with Muni shapefile names
muni_pincounts_majorclass <- ptax_pins %>%
  filter(major_class_code != 0) %>%
  # incentive_pins_w_projects %>%
  left_join(nicknames) %>%
  group_by(clean_name) %>% 
  mutate(muni_av = sum(av, na.rm=TRUE)) %>%
  ungroup() %>%
  group_by(clean_name, major_class_code, agency_name, shpfile_name, muni_av, assess_ratio) %>%
  dplyr::summarize(
    fmv = sum(av/assess_ratio),
    ...
    pin_count = n(),
            av = sum(av))  %>%
  mutate(
         mc_fmv_perc = fmv/ (muni_av/assess_ratio),
         av_perc = av / muni_av) %>%
   ungroup() %>% group_by(clean_name) %>%
   mutate(muni_fmv = sum(fmv, na.rm = TRUE))


muni_comval_percentages <- ptax_pins %>% 
  filter(major_class_code != 0) %>%
    mutate(fmv = av / assess_ratio ) %>%
    group_by(clean_name) %>% 
  mutate(muni_av = sum(av, na.rm=TRUE),
  ...
  reframe(clean_name, incent_prop, Alea_cat, major_class_code,agency_name, 
          shpfile_name, muni_av, muni_fmv, assess_ratio, av, fmv, keypin, 
          comind_av = sum(av), comind_fmv = sum(av/assess_ratio), 
          .by = c(clean_name, Alea_cat)) %>%
  left_join(nicknames) %>%
  group_by(clean_name, incent_prop, Alea_cat, major_class_code,  agency_name, shpfile_name, muni_av, muni_fmv, assess_ratio) %>%
  dplyr::summarize(
    project_count = n_distinct(keypin),
    pin_count = n(),
    fmv = sum(av/assess_ratio, na.rm=TRUE),
            av = sum(av, na.rm=TRUE))  %>%
  mutate(mc_fmv_perc = fmv/ (muni_av/assess_ratio),
         av_perc = av / muni_av) %>%
  ungroup() %>%
  group_by(clean_name) %>%
  mutate(comind_fmv = sum(fmv, na.rm=TRUE)) %>%
  mutate(perc_comind = fmv / comind_fmv)

# table(muni_pincounts_majorclass$major_class_code) # Number of Munis with these major classes



fig_muni_perc_of_comval <- muni_comval_percentages %>% 
  filter(major_class_code %in% c("6A", "6B", "6C", "7A", "7B", "8")) %>%
  group_by(shpfile_name)%>%
  summarize(perc_comind = sum(perc_comind, na.rm = TRUE)) %>% 
  filter(!is.na(shpfile_name) ) %>% 
  full_join(muni_shp, by = c("shpfile_name" = "MUNICIPALITY")) %>%
  ggplot(aes(fill = perc_comind)) + 
  geom_sf(aes(geometry = geometry, label = shpfile_name),
          color = "black",
          lwd = .05)+ 
  geom_sf_text(aes(geometry = geometry, label = str_wrap(shpfile_name, 1)), size = 1) + 
  labs(title = "% of Commercial & Industrial FMV (per Muni) \nthat has Incentive Classifications",
       subtitle = "FMV is AV / assessment ratio for property class") +
  theme_void() + 
...
legend.position = "bottom", legend.key.width = unit(2, "cm")) +
  scale_fill_steps2(
    high = "brown",
   # nice.breaks = FALSE,
   breaks = c(0, 0.05, .10, .20, 0.4, .60, .70),
  #  show.limits=TRUE,
    na.value = "gray",
    name = "% of Com&Ind FMV",
    label = scales::percent) +   guides(fill=guide_legend(nrow=1,byrow=TRUE, label.position = "bottom"))


fig_muni_perc_of_comval
```
...
fig_muni_AV_perc <- muni_pincounts_majorclass %>% 
  filter(major_class_code %in% c("6A", "6B", "6C", "7A", "7B", "8")) %>%
  filter(clean_name != "Chicago") %>% 
  group_by(shpfile_name)%>%
  summarize(av_perc = sum(av_perc)) %>% 
  filter(!is.na(shpfile_name) ) %>% 
  full_join(muni_shp, by = c("shpfile_name" = "MUNICIPALITY")) %>%
  ggplot(aes(fill = av_perc)) + 
  geom_sf(aes(geometry = geometry),
          color = "black",
          lwd = .05)+ 
  labs(title = "% AV from Properties with Any Type of Incentive Class") +
  theme_void() + 
  theme(axis.ticks = element_blank(), axis.text = element_blank()) +
  scale_fill_steps2(
    high = "darkblue", low = "orange",
...
```{r muni-alltogether-scaled}
#| fig-height: 6
#| fig-width: 6


fig_muni_FMV_perc <- muni_pincounts_majorclass %>% 
  filter(major_class_code %in% c("6A", "6B", "6C", "7A", "7B", "8")) %>%
  group_by(shpfile_name)%>%
  filter(!is.na(shpfile_name) ) %>% 
  full_join(muni_shp, by = c("shpfile_name" = "MUNICIPALITY")) %>%
  ggplot(aes(fill = mc_fmv_perc)) + 
  geom_sf(aes(geometry = geometry),
          color = "black",
          lwd = .05)+ 
  labs(title = "FMV for Properties with Any Type of Incentive Class",
       subtitle = "FMV is AV / assessment ratio for property class") +
  theme_void() + 
  ...
  scale_fill_steps2(
   ...
   # nice.breaks = FALSE,
  # limits = c(0, .6),
   breaks = c(0, 0.05, .10, .15, .20, 0.4, 0.60, 0.70),
   # show.limits=TRUE,
    na.value = "gray",
    name = "% of FMV",
    label = scales::percent)+ guides(fill=guide_legend(nrow=1,byrow=TRUE, label.position = "bottom")) +
...
fig_muni_C6_AV_perc <- muni_pincounts_majorclass %>% 
  filter(major_class_code %in% c("6A", "6B", "6C")) %>%
 # filter(clean_name != "Chicago") %>% 
  group_by(shpfile_name)%>%
  summarize(av = sum(av_perc)) %>% 
  filter(!is.na(shpfile_name)) %>% 
  full_join(muni_shp, by = c("shpfile_name" = "MUNICIPALITY")) %>%
  ggplot(aes(fill = av)) + 
  geom_sf(aes(geometry = geometry),
          ...
          lwd = .05)+ 
  labs(title = "Properties with Class 6 Incentives",
       subtitle = "Assessed Value in Municipalities") +
  theme_void() + 
  theme(axis.ticks = element_blank(), axis.text = element_blank()) +
  scale_fill_steps2(
    high = "darkblue", low = "orange",
...
fig_muni_C6_FMV_perc <- muni_pincounts_majorclass %>% 
  filter(major_class_code %in% c("6A", "6B", "6C")) %>%
  group_by(shpfile_name)%>%
  summarize(mc_fmv_perc = sum(mc_fmv_perc)) %>% 
  filter(!is.na(shpfile_name)) %>% 
  full_join(muni_shp, by = c("shpfile_name" = "MUNICIPALITY")) %>%
  ggplot(aes(fill = mc_fmv_perc)) + 
  geom_sf(aes(geometry = geometry),
          color = "black",
          lwd = .05)+ 
  labs(title = "Percent of Municipality FMV from Property with Class 6 Incentives") +
  theme_AWM + 
  theme(axis.ticks = element_blank(), axis.text = element_blank()) +
  scale_fill_steps2(
    high = "brown",
...
fig_muni_perc_of_comval6C <- muni_comval_percentages %>% 
  filter(major_class_code %in% c("6A", "6B", "6C")) %>%
  group_by(shpfile_name)%>%
  summarize(perc_comind = sum(perc_comind, na.rm = TRUE)) %>% 
  filter(!is.na(shpfile_name) ) %>% 
  ...
  geom_sf(aes(geometry = geometry),
          color = "black",
          lwd = .05)+ 
  labs(title = "Class 6: % of Commercial & Industrial FMV (per Muni)") +
  theme_void() + 
  theme(axis.ticks = element_blank(), axis.text = element_blank()) +
  scale_fill_steps2(
    high = "brown",
   # nice.breaks = FALSE,
   breaks = c(0, 0.05, .10, .20, 0.4, .60, .80),
    show.limits=TRUE,
   limits = c(0, .8),
    na.value = "gray",
    name = "% of Com&Ind FMV",
...
fig_muni_perc_of_comval6C

```

### Neighborhood by Major Class
...
fig_nbhd_C6_AV <- incentives_props %>% 
  filter(major_class_code %in% c("6A", "6B", "6C") ) %>%
  group_by(nbhd_code) %>%
  summarize(av = sum(nbh_MC_AV,na.rm=TRUE)) %>%
  full_join(NBHs, by = c("nbhd_code" = "town_nbhd")) %>%
  ggplot(aes(fill = av)) + 
  geom_sf(aes(geometry = geometry),
          color = "black",
...
fig_muni_C7_AV_perc <- muni_pincounts_majorclass %>% 
  filter(major_class_code %in% c("7A", "7B", "7C")) %>%
  filter(clean_name != "Chicago") %>% 
  group_by(shpfile_name)%>%
  summarize(av = sum(av_perc)) %>% 
  ...
  geom_sf(aes(geometry = geometry),
          color = "black",
          lwd = .05)+ 
  labs(title = "Percent of Total Municipality Assessed Value \nfrom Properties with Class 7 Incentives",
       subtitle = "in Municipalities (Except Chicago)") +
  theme_void() + 
  theme(axis.ticks = element_blank(), axis.text = element_blank()) +
  scale_fill_steps2(
...
fig_muni_C7_fmv_perc <- muni_pincounts_majorclass %>% 
  filter(major_class_code %in% c("7A", "7B", "7C")) %>%
   group_by(shpfile_name)%>%
   summarize(mc_fmv_perc = sum(mc_fmv_perc)) %>% 
  filter(!is.na(shpfile_name)) %>% 
  ...
  geom_sf(aes(geometry = geometry),
          color = "black",
          lwd = .05)+ 
  labs(title = "Percent of Muni FMV from Properties with Class 7 Incentives") +
  theme_void() + 
  theme(axis.ticks = element_blank(), axis.text = element_blank()) +
  scale_fill_steps2(
   high = "brown",
...
fig_muni_C7_FMV <- muni_pincounts_majorclass %>% 
  filter(major_class_code %in% c("7A", "7B", "7C")) %>%
  filter(clean_name != "Chicago") %>% 
  group_by(shpfile_name)%>%
  summarize(fmv = sum(fmv)) %>% 
  ...
  geom_sf(aes(geometry = geometry),
          color = "black",
          lwd = .05)+ 
  labs(title = "FMV ($) of Properties with Class 7 Incentives",
       subtitle = "in Municipalities (Except Chicago)") +
  theme_void() + 
  theme(axis.ticks = element_blank(), axis.text = element_blank()) +
  scale_fill_steps2(
...
fig_muni_C7_FMV
fig_muni_C7_fmv_perc

fig_muni_perc_of_comvalC7 <- muni_comval_percentages %>% 
  filter(major_class_code %in% c( "7A", "7B")) %>%
  group_by(shpfile_name)%>%
  summarize(perc_comind = sum(perc_comind, na.rm = TRUE)) %>% 
  filter(!is.na(shpfile_name) ) %>% 
  full_join(muni_shp, by = c("shpfile_name" = "MUNICIPALITY")) %>%
  ...
  geom_sf(aes(geometry = geometry),
          color = "black",
          lwd = .05)+ 
  labs(title = "Class 7: % of Commercial & Industrial FMV (per Muni)",
       subtitle = "FMV is AV / assessment ratio for property class") +
  theme_void() + 
  theme(axis.ticks = element_blank(), axis.text = element_blank()) +
  scale_fill_steps2(
    high = "brown",
limits = c(0, .8),
breaks = c(0, 0.05, .10, .20,  0.4, .60, 0.8),
    show.limits=TRUE,
    na.value = "gray",
    name = "% of Com&Ind FMV",
    label = scales::percent)

fig_muni_perc_of_comvalC7
```
...
muni_comval_percentages %>% ungroup() %>% filter(major_class_code == "8") %>% 
  reframe(perc_comind = sum(perc_comind, na.rm=TRUE), 
          mc_fmv_perc = sum(mc_fmv_perc, na.rm=TRUE), .by = clean_name) %>% arrange(desc(perc_comind))
```
...
fig_muni_C8_AV <- muni_pincounts_majorclass %>% 
  ...
  geom_sf(aes(geometry = geometry),
          color = "black",
          lwd = .05)+ 
  labs(title = "AV per Muni from Properties with Class 8 Incentives",
...
fig_muni_C8_avperc <- muni_pincounts_majorclass %>% 
  filter(!is.na(shpfile_name)) %>% 
  filter(major_class_code %in% c("8")) %>%
  full_join(muni_shp, by = c("shpfile_name" = "MUNICIPALITY")) %>%
  ggplot(aes(fill = av_perc)) + 
  ...
  labs(title = "Class 8 Properties",
       subtitle = "Percent of AV for Property with Incentive Classifications") +
  theme_void() + 
  theme(axis.ticks = element_blank(), axis.text = element_blank()) +
  scale_fill_steps2(
    high = "darkblue", low = "orange",
    breaks = c(0, 0.025, 0.05, .1, .2, .4),
    show.limits=TRUE,
    na.value = "gray",
    name = "% Muni AV",
    label = scales::percent
...
fig_muni_C8_fmvperc <- muni_pincounts_majorclass %>% 
  filter(!is.na(shpfile_name)) %>% 
  filter(major_class_code %in% c("8")) %>%
  full_join(muni_shp, by = c("shpfile_name" = "MUNICIPALITY")) %>%
  ggplot(aes(fill = mc_fmv_perc)) + 
  geom_sf(aes(geometry = geometry),color = "black", lwd = .05)+ 
  labs(title = "Class 8 Properties",
       subtitle = "Percent of FMV for Property with Class 8 Classification") +
  theme_void() + 
  theme(axis.ticks = element_blank(), axis.text = element_blank()) +
  scale_fill_steps2(
    high = "brown",
...
fig_muni_perc_of_comvalC8 <- muni_comval_percentages %>% 
  filter(major_class_code %in% c("8")) %>%
  group_by(shpfile_name)%>%
  summarize(perc_comind = sum(perc_comind, na.rm = TRUE)) %>% 
  filter(!is.na(shpfile_name) ) %>% 
  ...
  geom_sf(aes(geometry = geometry),
          color = "black",
          lwd = .05)+  
  labs(title = "Class 8: % of Commercial & Industrial FMV (per Muni)",
       subtitle = "FMV is AV / assessment ratio for property class") +
  theme_void() + 
  theme(axis.ticks = element_blank(), axis.text = element_blank()) +
  scale_fill_steps2(
...
Mostly ignore these. Looking at the number of PINs is not a good way to assess incentive properties or projects. Done originally due to data limitations during initial research. 



```{r class6pincount-nbhd}
#| layout-ncol: 2

fig_nbhd_C6_PC <- incentives_props %>% 
  filter(major_class_code %in% c("6A", "6B", "6C") ) %>%
  group_by(nbhd_code) %>%
  summarize(pin_count= round(sum(nbh_MC_PC)), digits = 0) %>%
  full_join(NBHs, by = c("nbhd_code" = "town_nbhd")) %>%
...
fig_muni_C7_PC <- muni_pincounts_majorclass %>% 
  ...
  labs(title = "Properties with Class 7 Incentives",
       subtitle = "# of PINs with Incentives in Municipalities (Except Chicago)",
         caption = "Chicago has over 200 Class 7 PINs") +
  theme_void() + 
  theme(axis.ticks = element_blank(), axis.text = element_blank()) +
  scale_fill_steps2(
    high = "darkblue", low = "orange",
...
fig_nbhd_C7_PC <- incentives_props %>% 
  filter(major_class_code %in% c("7A", "7B", "7C") ) %>%
  group_by(nbhd_code) %>%
  summarize(pincount = sum(nbh_MC_PC)) %>% 
  full_join(NBHs, by = c("nbhd_code" = "town_nbhd")) %>%
    filter(triad_name != "City") %>%
...
```{r allclasses-pincount}
#| column: page
#| layout-ncol: 2

fig_nbhd_pincount <- incentives_props %>% 
  group_by(nbhd_code) %>%
  summarize(pin_count = sum(nbh_MC_PC)) %>%
  full_join(NBHs, by = c("nbhd_code" = "town_nbhd")) %>%
  ggplot(aes(fill = pin_count)) + 
  geom_sf(aes(geometry = geometry), color = "black") + 
  labs(title = "Number of PINs with Incentive Class in Assessor Neighborhoods", 
  caption = "Count of 14 digit PINs") +
  theme_void() + 
   theme(axis.ticks = element_blank(), axis.text = element_blank()) +
scale_fill_steps2(
...
fig_muni_C8_PC <- muni_pincounts_majorclass %>% 
 ...
 # filter(clean_name != "Chicago") %>% 
  ...
  labs(title = "Properties with Class 8 Incentives",
       ...
       caption = "Outlier is the Amazon in Markham") +
  theme_void() + 
  theme(axis.ticks = element_blank(), axis.text = element_blank()) +
  scale_fill_steps2(
    high = "darkblue", low = "orange",
...
nbh_major_class <- read_csv("./Output/ptaxsim_nbh_MC_summary_2022.csv")
nbh_major_class
...
```{r}
nbh_sum <- read_csv("./Output/ptaxsim_nbh_summary_2022.csv") %>%
    mutate(nbhd_code = as.character(nbhd_code),
           township_code = str_sub(nbhd_code, 1,2))

nbh_MC_sum <- read_csv("./Output/ptaxsim_nbh_MC_summary_2022.csv") %>%
    mutate(nbhd_code = as.character(nbhd_code))

nbh_sum %>% 
  full_join(NBHs, by = c("nbhd_code" = "town_nbhd")) %>% 
  ggplot(aes(fill = nbh_exe_abate/nbh_PC)) + 
  geom_sf(aes(geometry = geometry), color = "black") + 
  labs(title = "Average Abatement/PIN", 
  caption = "Abatements from exe_abate variable") +
  theme_void() + 
   theme(axis.ticks = element_blank(), axis.text = element_blank()) +
scale_fill_steps2(
  high = "darkblue", low = "orange",
  midpoint = median(nbh_sum$nbh_exe_abate/nbh_sum$nbh_PC, na.rm = TRUE),
 nice.breaks = FALSE,
  show.limits=TRUE,
na.value = "gray",
  name = "Exempt EAV",
...
# Add in names that merge with Muni shapefile names
muni_appeals_2021 <- incent_pins_ptax %>% 
  left_join(tc_muninames, by = "tax_code_num") %>%
  ...
  filter(av_mailed > av_clerk & 
                              year == 2021) %>%
    mutate(av_dif =  av_mailed - av_clerk) %>%
  group_by(clean_name, shpfile_name) %>%
  dplyr::summarize(
    pin_count = n(),
    av_dif = sum(av_dif))
...
fig_muni_AV_w_chicago <- muni_appeals_2021 %>% 
  filter(!is.na(shpfile_name) ) %>% 
  full_join(muni_shp, by = c("shpfile_name" = "MUNICIPALITY")) %>%
...
fig_muni_AV_nochicago <- muni_appeals_2021 %>% 
...
```{r}
munilevel <- read.csv("./Output/ptaxsim_muni_level_2006-2021.csv") %>% filter(year == 2021)

muni_appeals_2021 %>% left_join(munilevel) %>% select(clean_name, av_dif, pin_count, cur_comp_muni_rate, current_taxable_eav, MuniLevy = final_tax_to_dist) %>%
  mutate(cf_eav = current_taxable_eav + av_dif*3.003,
         cf_taxrate = MuniLevy / cf_eav,
         rate_change = round(cur_comp_muni_rate-cf_taxrate*100,  digits =4) )%>%
  select(clean_name, rate_change, av_dif, everything()) %>%
  arrange(-rate_change)
```

> In Phoenix during tax year 2021, the tax rate is 2 pct points higher currently than in the counter factual due to incentive properties appealing their already reduced assessed values. 
...
# Add in names that merge with Muni shapefile names
muni_appeals_2022 <- incent_pins_ptax %>% 
  filter(year == 2022) %>%
  left_join(tc_muninames, by = "tax_code_num") %>%
  filter(av_mailed > av_clerk & 
                              year == 2022) %>%
...
fig_muni_AV_w_chicago_2022 <- muni_appeals_2022 %>% 
  full_join(muni_shp, by = c("shpfile_name" = "MUNICIPALITY")) %>%
  ggplot(aes(fill = av_dif) ) + 
  geom_sf(aes(geometry = geometry),
          color = "black",
...
fig_muni_AV_nochicago_2022 <- muni_appeals_2022 %>% 
  filter(clean_name != "Chicago") %>% 
  filter(!is.na(shpfile_name) ) %>% 
  full_join(muni_shp, by = c("shpfile_name" = "MUNICIPALITY")) %>%
  ggplot(aes(fill = av_dif) ) + 
...
incent_pins_ptax %>%  
 ...
 # geom_line(aes(x = year, y = av_clerk
  ...
  theme_minimal() + 
  scale_y_continuous(labels = scales::dollar) +
  labs(title = "All Municipalities:Incentive Properties & Appeals", subtitle = "Mailed AV and Clerk AV", caption = "Black is mailed, green is certified, purple is Board & Clerk AV",
       y = "Assessed Value")
```
