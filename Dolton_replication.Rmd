...
Reran code but I did not change the comments in code chunks so number of pins, tax codes, ect. referenced in text is for Bridgeview. 
...
In general, I never use the simplify=TRUE default in the tax_bill() function. I want the amount that goes to the tif, the district, and the pre and post exemption bill amounts. These are only returned if `simplify=FALSE`so you need to include that in your tax_bill() request.
...
# made this the morning of 6/14 and haven't thought deeply about if 
# percent to tif is meaningful

agency_levies <- bills %>% 
  group_by(agency_name) %>%
  summarize(levy_within_muni = round(sum(final_tax_to_dist, na.rm=TRUE), digits=0),
