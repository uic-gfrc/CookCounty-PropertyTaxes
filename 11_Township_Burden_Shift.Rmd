...
# chi_nbhds <- read_sf("https://data.cityofchicago.org/resource/igwz-8jzy.json")
...
The median amount of EAV from residential parcels is `r scales::percent(median(pct_residential$pct_eav_MC))`.
...
# TS_burden_table <- TS_taxrates %>% 
#   left_join(pct_exempt_MC) %>%
#   left_join(pct_eav_MC) %>%
#   group_by(township_name) %>%
#   summarize(rev_collected_current = sum(MC_nonTIF_EAV_post_exemps * tax_rate_current),
  #        rev_collected_new = sum((MC_nonTIF_EAV_post_exemps + MC_exemptions )*taxrate_new),
  #        burden_current = sum(rev_collected_current/TownshipLevy),
  #        burden_noexemps = sum(rev_collected_new/TownshipLevy), 
  #        burden_change = sum(burden_noexemps- burden_current) ) %>%
  # mutate(burden_current = ifelse(burden_current>1, 1, burden_current)
