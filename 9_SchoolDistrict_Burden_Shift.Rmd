...
taxbills_by_Class_per_TC <- read_csv("2_Summed_Bills_by_Taxcode_and_Class.csv")  %>% 
...
exemptions_by_class_per_TC <- read_csv("3_Exemptions_Details_output-ClassTaxcodeExemps.csv") %>% 
  ...
  mutate(tax_code_num = as.character(tax_code_num)) %>%
  left_join(school_tax_codes) %>%
  full_join(school_names_elem) %>%
...
grouped_exemptions <- exemptions_by_class_per_TC %>% 
  filter(!is.na(agency_name)) %>%
  # group_by(agency_name, major_class_code, major_class_type, ResidentialProps, PropType) %>%
  group_by(PropType, agency_name, agency_num.x)%>%
  summarize(eav = sum(eav),
            exempt_EAV = sum(exempt_EAV, exe_abate, na.rm=TRUE),
            tax_base_current = sum(tax_base_current, na.rm=TRUE),
            tax_base_noexemps = sum(tax_base_noexemps, na.rm=TRUE)) %>% ungroup() %>% 
...
elementary_district_eav <- grouped_exemptions %>%  
  group_by(agency_name, agency_num.x) %>% 
  summarize(school_EAV_includesTIF = sum(eav), # all EAV in the municipality that exists
            school_tax_base_current=sum(tax_base_current), # taxable EAV based on current exemptions
            school_tax_base_noexemps = sum(tax_base_noexemps)) %>%  # taxable EAV pre-exemptions
  ungroup() 

perc_residential <- full_join(grouped_exemptions, elementary_district_eav) %>% 
  filter(PropType == "Residential (C1)") %>% 
  
  mutate(percent_residential = eav / school_EAV_includesTIF)# %>% select()
...
perc_residential %>% 
  ...
  full_join(elem_schools_shp, by =  c("agency_num.x" = "agency_num")) %>%
  ggplot(aes(fill = percent_residential)) + 
  geom_sf(aes(geometry = geometry), color = "black") + 
  labs(title = "Residential EAV /  Total EAV", caption = "The median school district outside of Chicago has 65.7% of its EAV from Class 2 Residential Properties") +
  theme_classic() + 
...
taxcodes_current <- TC_bills_current %>% 
  filter(tax_code %in% school_tax_codes$tax_code_num) %>% 
  left_join(school_tax_codes, by = c("tax_code" = "tax_code_num")) %>%
  left_join(muni_tax_codes) %>%   
  left_join(school_names_elem) %>% #  by = c("tax_code" = "tax_code_num")) %>%
...
#  filter(!agency_num %in% cross_county_lines) %>%
  ...
  summarize(SchoolLevy = sum(final_tax_to_dist, na.rm = TRUE), # amount billed by tax districts with current exemptions in place
  ...
  mutate(tax_rate_current = SchoolLevy/nonTIF_EAV_post_exemps,
         nonTIF_EAV_pre_exemps = nonTIF_EAV_post_exemps + Exempt_EAV,
         taxrate_new = SchoolLevy/nonTIF_EAV_pre_exemps,
         taxrate_change = tax_rate_current-taxrate_new) %>% 
  select(agency_name, taxrate_change, tax_rate_current, taxrate_new, everything()) %>% 
...
write_csv(Current_Taxrates, "elem_schooldist_comprates.csv")
```

Current composite tax rates for each elementary school district are above. Table also includes the new tax rate if there were no exemptions, the levy (aka amount collected by the district from final_tax_to_dist variable), EAV outside of TIFs, amount of exempt EAV, and additional variables.
...
# calculate totals with ONLY EAV outside of TIFs
elem_dist_eav <- grouped_exemptions %>%  
  group_by(agency_name, agency_num.x) %>% 
  summarize(school_EAV_includesTIF = sum(eav), # all EAV in the municipality that exists
            school_tax_base_current=sum(tax_base_current), # taxable EAV based on current exemptions
            school_tax_base_noexemps = sum(tax_base_noexemps)) %>%  # taxable EAV pre-exemptions
...
pct_property_types <- left_join(grouped_exemptions, elem_dist_eav) %>% 
 # filter(ResidentialProps == "Alea_cat") %>% 
  mutate(pct_PropType = eav / school_EAV_includesTIF)# %>% select()
...
burden_table <- pct_property_types %>% 
  ...
  mutate(rev_collected_current = tax_base_current * tax_rate_current,
         ...
         burden_current = rev_collected_current/SchoolLevy,
         burden_noexemps = rev_collected_new/SchoolLevy, 
...
Current_Taxrates %>% 
  mutate(agency_name = ifelse(agency_name == "TOWN CICERO", "CITY OF CICERO", agency_name) ) %>%
  full_join(elem_schools_shp) %>%
  ggplot(aes(fill = tax_rate_current)) + 
  geom_sf(aes(geometry = geometry), color = "black") + 
  labs(title = "Composite Tax Rate within School Districts (Elementary)", 
       caption = "The median school district outside of Chicago has composite tax rate around 12.3%") +
  theme_classic() + 
  theme(axis.ticks = element_blank(), axis.text = element_blank())+# +#+
  scale_fill_steps2(
    high = "darkblue", low = "black",  #  guide = "legend",
