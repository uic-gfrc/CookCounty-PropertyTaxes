...
# create final dataframe used in models

comm_ind_df <- comm_ind |> 

  select(pin, class, year, clean_name, 
         treatment_year, status,
         incent_prop, incent_indicator, base_year_fmv_2011, base_year_fmv_2006, 
         ...
         nonvacant_lag, fmv_growth_2011_w, base_year_fmv_2011_w,
         first_incent_year, in_tif)
...
# Create functions to pass through modelsummary for logit models.
# Converts logged-odds into probabilities for both coefficients AND
# standard errors

logit_to_prob <- function(beta) {
  prob <- exp(beta) / (1 + exp(beta))
  return(prob)
}

logit_se_to_prob_se <- function(beta, se) {
  # Calculate the derivative of the logit function
  derivative <- exp(beta) / (1 + exp(beta))^2
  # Transform the standard error
  se_prob <- se * abs(derivative)
  return(se_prob)
}

# More probability conversion stuff
transform_model <- function(model) {
 # Extract pertinent output
  coefs <- coef(summary(model))
  betas <- coefs[, "Estimate"]
  ses <- coefs[, "Std. Error"]
    # Transform the coefficients and standard errors
  prob_betas <- logit_to_prob(betas)
  prob_ses <- mapply(logit_se_to_prob_se, betas, ses)
  
  # Return a list of transformed values
  list(estimate = prob_betas, std.error = prob_ses)
}

transform_feglm_model <- function(model) {
...
### Transformed Logit Model:
```{r}
#| label: tbl-logitincentDV-noFE
#| tbl-cap-location: margin
#| tbl-cap: "DV is if a parcel has a incentive property or not or not. 0 = did not have incentive class, 1 = had incentive class. Only uses Municipality-Year fixed effects. There are no PIN level fixed effects in the logit models."
#| results: hold

logitted_models <- list(
  
  "Land Use"  = feglm(incent_indicator ~ land_use | clean_name + year, 
                              panel.id = c("clean_name", "year"),   
                              data = comm_ind_df, 
                              family = binomial(link = "logit")
                      ),
  
  "Land Use and FMV Growth" =  feglm(incent_indicator ~ land_use 
                                    + nonvacant | clean_name + year, 
                                           panel.id = c("clean_name", "year"),   
                                           data = comm_ind_df, 
                                           family = binomial(link = "logit")
                                    ),
  
  "Land Use Interact FMV" = feglm(incent_indicator ~ land_use*fmv_growth_2011_w | clean_name + year,
                                    panel.id = c("clean_name", "year"),   
                                    data = comm_ind_df, 
                                    family = binomial(link = "logit")
                                  ),
  
    "Other Variables" = feglm(incent_indicator ~ land_use*fmv_growth_2011_w + building + nonvacant + in_tif | clean_name + year,
                                    panel.id = c("clean_name", "year"),   
                                    data = comm_ind_df, 
                                    family = binomial(link = "logit")
                                  )
...
loggited_statistic_overrides <- lapply(logitted_models, transform_feglm_model)

#ov_override <- transform_feglm_model(ov)

#loggited_statistic_overrides <- lapply(ov, transform_feglm_model)

modelsummary(logitted_models,
             statistic_override = loggited_statistic_overrides,
            # vcov = list(vcovHAC, vcovHAC, vcovHAC, vcovHAC),
             stars = TRUE,
...
```{r}
#| label: tbl-FE_exponentiated_coeffs-noFE
#| tbl-cap: "Odds. Has exponentiated coefficients."
#| results: hold
...
Only includes PINs that that change incent_prop status during the time frame. Any PIN that was never an incentive or always an incentive property are dropped from these models.

Not sure how to interpret results though?

```{r}
#| label: tbl-logitincentDV
#| tbl-cap-location: margin
#| tbl-cap: "DV is if a parcel has a incentive property or not or not. 0 = did not have incentive class, 1 = had incentive class. Sample size decreases a lot due to only being able to use observations that experienced a change in the DV. "
#| results: hold
...
#| tbl-cap: "Comparison of different combinations of Muni, PIN, and Year Fixed Effects. 2012-2022. "
#| warning: false
#| results: hold
...
muni_fe_models <- list(
  ...
  "PIN-Muni FE" = feols(fmv_growth_2011_w ~ incent_prop + land_use | pin + clean_name,
   ...
   "Pin-Year FE" = feols(fmv_growth_2011_w ~ incent_prop+land_use | pin + year ,
     ...
     "Pin-Year FE" = feols(fmv_growth_2011_w ~ incent_prop*land_use + reassessed_year*incent_prop + in_tif| pin + year ,
                        panel.id = c("pin", "year"),
                        data = comm_ind_df %>% filter(year>2011)  ), 
  
  "Muni-Year FE" = feols(fmv_growth_2011_w ~ incent_prop+land_use | clean_name + year ,
                        panel.id = c("pin", "year"),
                        data = comm_ind_df %>% filter(year>2011)  ), 
...
modelsummary(muni_fe_models,
             stars = TRUE,
          #  vcov = list(vcovHAC, vcovHAC, vcovHAC, vcovHAC, vcovHAC, vcovHAC, vcovHAC),
             fmt = function(x) round(x, 2),
             gof_omit = "AIC|BIC|Log.Lik.", 
            output = "flextable")
