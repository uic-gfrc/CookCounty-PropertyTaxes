...
comm_ind <- comm_ind |>
  ...
  ungroup() |>
  group_by(year) |>
         mutate(base_year_fmv_2011_w = DescTools::Winsorize(base_year_fmv_2011, quantile(base_year_fmv_2011, probs = c(0.01,0.99), na.rm=TRUE)), 
...
lose_incent_pins <- comm_ind_raw |> 
  ...
  select(pin, year) %>% mutate(status = "Loses Incentive")

gains_incent_pins <- comm_ind_raw |> 
  filter(incent_prop == "Incentive" & lag(incent_prop) == "Non-Incentive") %>% 
  select(pin, year) %>% mutate(status = "Gains Incentive")

gains_loses <- rbind(lose_incent_pins, gains_incent_pins) %>%
  mutate(treatment_year = as.numeric(year))
...
# comm_ind_raw |> 
#   filter(year == 2022) |> 
#   filter(pin %in% gains_incent_pins$pin) %>% arrange(pin)

comm_ind_raw <- comm_ind_raw %>% 
  left_join(gains_loses, by = "pin") %>%
  rename(year = year.x)
#  group_by(pin) %>%
 # mutate(treatment_year = as.numeric(ifelse(any(!is.na(status)), as.numeric(year), NA)))

table(comm_ind_raw$treatment_year)
table(comm_ind_raw$year)
table(comm_ind_raw$incent_change)
comm_ind_raw <- comm_ind_raw |>
  ungroup() |> 
 # mutate(year = as.numeric(year)) %>%
  mutate(treated = ifelse(treatment_year < year & incent_change == "Changes Sometime", "Pre Treatment", 
                         ifelse(treatment_year > year & incent_change == "Changes Sometime", "Post Treatment", 
                                 ifelse(treatment_year == year & incent_change == "Changes Sometime", "Treatment year",
                                        "Control"))))

table(comm_ind_raw$treated)
table(comm_ind_raw$status)


# feols(fmv_growth_2011 ~ i(year, treated, ref=2016) | clean_name + year, data = comm_ind_raw)
```
...
ols_models <- list(
    "Year Controls" = lm(fmv_growth_2011_w ~ incent_prop*land_use + year + triad*reassessed_year,
                data = comm_ind_raw),
  
  "Year and Muni Controls" = lm(fmv_growth_2011_w ~ incent_prop*land_use
             + clean_name + year,
                data = comm_ind_raw))
...
ols_models <- list(
    "Year Controls" = lm(fmv_growth_2011_w ~  reassessed_year*gain_lose_incent + year,
                data = comm_ind_raw),
  
  "Year and Muni Controls" = lm(fmv_growth_2011_w ~ gain_lose_incent*reassessed_year +
             + clean_name + year,
                data = comm_ind_raw))
...
muni_fe_models <- list(
      ...
      "1" = feols(fmv_growth_2011_w ~ land_use + reassessed_year + l(reassessed_year, 1) + l(reassessed_year, 2) | pin + clean_name + year,
                   vcov = "threeway", panel.id = c("pin", "year"), 
                       data = comm_ind_raw ),
      
      "2" = feols(fmv_growth_2011_w ~ incent_prop + land_use | pin + clean_name + year,
                                     vcov = "threeway", panel.id = c("pin", "year"),
                       data = comm_ind_raw 
                       ),


      "3" = feols(fmv_growth_2011_w ~ incent_prop*land_use + reassessed_year + l(reassessed_year, 1) + l(reassessed_year, 2) | pin + clean_name + year,
                                                       vcov = "threeway", panel.id = c("pin", "year"),

                       data = comm_ind_raw  ),
          "4" = feols(fmv_growth_2011_w ~ incent_prop*land_use + in_tif | pin + clean_name + triad^year,
                                                           vcov = "threeway", panel.id = c("pin", "year"),
...
modelsummary(muni_fe_models,
            ...
            # fmt = function(x) round(x, 2),
            # coef_omit= 'pin|year|clean_name',
             gof_omit = "AIC|BIC|Log.Lik.|RMSE", output = "flextable")
...
muni_fe_new_vars_mods <- list(
     ...
     "1" = feols(fmv_growth_2011_w ~ incent_change_bin*prop_use_change_bin
      ...
      "3" = feols(fmv_growth_2011_w ~ land_use + incent_prop + incent_change_bin*prop_use_change_bin
                 | clean_name + year,  vcov = "twoway",
                   data = comm_ind_raw ),
     
      "4" = feols(fmv_growth_2011_w ~ incent_change_bin + prop_use_change_bin + land_use*incent_prop
                 | clean_name + year,  vcov = "twoway",
                   data = comm_ind_raw
                       )
)
...
```{r}
muni_fe_new_vars_mods <- list(
  "Industrial Subset" = feols(fmv_growth_2011_w ~ incent_change_bin*prop_use_change_bin
                 | clean_name + year,  vcov = "twoway",
                              data = industrial_subset
  ),
  "Commercial Subset" = feols(fmv_growth_2011_w ~ incent_change_bin*prop_use_change_bin
                 | clean_name + year,  vcov = "twoway",
...
```{r}
muni_fe_new_vars_mods <- list(
  "Industrial Subset" = feols(fmv_growth_2011_w ~ land_use*incent_prop + in_tif
                 | clean_name + year,  vcov = "twoway",
                              data = industrial_subset
  ),
  "Commercial Subset" = feols(fmv_growth_2011_w ~ land_use*incent_prop + in_tif
                 | clean_name + year,  vcov = "twoway",
                              data = commercial_subset
  )
)
...
**Pooled OLS: FMV Growth NOT Winsorized:**

```{r pooled_ols}

pooled_ols_models <- list(
  
  "1" = feols(fmv_growth_2011 ~ incent_change_bin + prop_use_change_bin + incent_change_bin*prop_use_change_bin,
                          data = comm_ind_temp),
  
  "2" = feols(fmv_growth_2011 ~ land_use + incent_prop + land_use*incent_prop,
              data = comm_ind_temp),
  
  "3" = feols(fmv_growth_2011 ~ land_use + incent_prop + land_use*incent_prop + fmv_2011,
              data = comm_ind_temp)

  )

modelsummary(pooled_ols_models)

```

**Pooled OLS with Winsorized FMV Growth:**

```{r pooled_ols-winsor}

pooled_ols_models <- list(
  
  "1" = feols(fmv_growth_2011_w ~ incent_change_bin + prop_use_change_bin + incent_change_bin*prop_use_change_bin,
                          data = comm_ind_temp),
  
  "2" = feols(fmv_growth_2011_w ~ land_use + incent_prop + land_use*incent_prop,
              data = comm_ind_temp),
  
  "3" = feols(fmv_growth_2011_w ~ land_use + incent_prop + land_use*incent_prop + fmv_2011,
              data = comm_ind_temp)
