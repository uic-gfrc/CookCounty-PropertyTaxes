...
# select variables
  ...
  select(pin, class, year, clean_name, 
         ...
         incent_prop, incent_indicator, gain_incent, incent_status, base_year_fmv_2011, 
         #base_year_fmv_2006, 
...
comm_ind_models <- comm_ind |>
  # Compare against all other land uses
  mutate(land_use = relevel(land_use, ref = "Other Land Use"))
...
name_map <- c(
  ...
  "gain_incent:landuse_changeChanges Land Use" = "Changes Land Use and Incentive Status"
)
...
incent_paper_models <- list(
  ...
  "I" = 
    ...
    feols(fmv_growth_2011_w ~ incent_prop + land_use +
            land_use*incent_change
                           | pin + year,
                           panel.id = c("pin", "year"),
                           vcov = "NW",
                           data = comm_ind_models),
  
  "II" = 
    
    feols(fmv_growth_2011_w ~ gain_incent + land_use +
            landuse_change*gain_incent
                           | pin + year,
                           panel.id = c("pin", "year"),
                           vcov = "NW",
                           data = comm_ind_models)
...
modelsummary(incent_paper_models,
             shape = "cbind",
             coef_map = name_map,
             stars = TRUE)
