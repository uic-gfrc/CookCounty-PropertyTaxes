...
pin_fe_models <- list(
      ...
      "3" = feols(fmv_growth_2011 ~ incent_prop + land_use + incent_prop*land_use 
                  | pin + year,
                       data = comm_ind_raw |> 
                         filter(land_use != "Rental")
                       )
...
muni_log_log_mods <- list(
     ...
     "1" = feols(fmv_growth_2011 ~ incent_change_bin + prop_use_change_bin + incent_change_bin*prop_use_change_bin
      ...
      "4" = feols(fmv_growth_2011 ~ incent_change_bin + prop_use_change_bin + land_use + incent_prop + land_use*incent_prop
                 | clean_name + year,
                   data = comm_ind_raw |>
                   filter(land_use != "Rental")
                       )
...
modelsummary(muni_log_log_mods,
             stars = TRUE,
             fmt = function(x) round(x, 2),
            # coef_omit= 'pin|year',
             gof_omit = "AIC|BIC|Log.Lik.|RMSE") |>
