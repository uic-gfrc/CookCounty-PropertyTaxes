...
reassessments_long <- reassessment_years %>% 
  pivot_longer(cols = c(`2006`:`2022`), names_to = "year", values_to = "reassessed_year") %>% mutate(year = as.numeric(year))
...
comm_ind_raw <- comm_ind_raw %>%
 ...
 # group_by(pin) %>%
 # mutate(treatment_year = as.numeric(ifelse(any(!is.na(status)), year, NA)))
...
mod1 <- fixest::feglm(incent_indicator ~ land_use + reassessed_year | pin + year,
                                          panel.id = c("pin", "year"),   
                      data = comm_ind_raw, 
...
muni_fe_models <- list(
      ...
      "1" = feols(fmv_growth_2011_w ~ land_use + year*reassessed_year + 
                   year*l(reassessed_year, 1:2) 
                  | pin  + year + clean_name ,
                panel.id = c("pin", "year"), 
                       data = comm_ind_raw ),
      ...
      "3" = feols(fmv_growth_2011_w ~ incent_prop*land_use + reassessed_year + l(reassessed_year, 1) + l(reassessed_year, 2)  | pin  + year+ clean_name ,
          ...
          "4" = feols(fmv_growth_2011_w ~ incent_prop*land_use + in_tif | pin + year + reassessed_year^year + clean_name ,
                    #    vcov = "threeway", 
                      panel.id = c("pin", "year"),
                       data = comm_ind_raw )
                        )
...
muni_fe_new_vars_mods <- list(
     ...
     "1" = feols(fmv_growth_2011_w ~ incent_prop
                 | clean_name + year,  vcov = "twoway",
                 data = industrial_subset
),
      ...
      "3" = feols(fmv_growth_2011_w ~ land_use
                 | clean_name + year,  vcov = "twoway",
                 data = industrial_subset
                       )
)
