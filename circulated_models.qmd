...
```{r} 
#| tbl-reassessinteractwithstatus
...
models <- list(
  "Land Use"  = fixest::feglm(incent_indicator ~ land_use | pin + clean_name + year,
                              panel.id = c("pin", "year"),   
                              data = comm_ind_df, 
                              family = binomial(link = "logit"),
                              vcov = ~ pin + year),
  
  "Land Use andFMV Growth" <- fixest::feglm(incent_indicator ~ land_use + fmv_growth_2011_w | pin + clean_name + year,
                                           panel.id = c("pin", "year"),   
                                           data = comm_ind_df, 
                                           family = binomial(link = "logit"),
                                           vcov = ~ pin + year),
  
  "Land Use Interact FMV" <- fixest::feglm(incent_indicator ~ land_use*fmv_growth_2011_w | pin + clean_name + year,
                                    panel.id = c("pin", "year"),   
                                    data = comm_ind_df, 
                                    family = binomial(link = "logit"),
                                    vcov = ~ pin + year)
