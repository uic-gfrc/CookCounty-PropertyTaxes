...
> A municipality that does not have properties gain or lose an incentive is dropped from the analysis when using muni-year fixed effects. (Similar to how a PIN is dropped from analysis if it does not gain or lose an incentive class when using PIN-Year level FE)
...
logitted_models <- list(
  ...
  "Land Use and FMV Growth" =  feglm(incent_indicator ~ land_use  + nonvacant | clean_name + year, 
                                     vcov = "newey_west",
...
# vcovs <- lapply(logitted_models_1, `[[`, "vcov")
...
#### Building or not Logit

```{r }
#| label: tbl-logit-buildingDV-noFE
#| tbl-cap: "No PIN Fixed Effects. Does have year and Muni Fixed Effects. DV is if a parcel has a building or not. 0 = no building, 1 = building. Does not have robust standard errors like previous logit model. Is not converted to odds."
#| warning: false
#| message: false
#| eval: false
#| include: false
...
models <- list(
    "By Incent Prop" = fixest::feglm(building ~ incent_prop | clean_name + year,
                                    panel.id = c("pin", "year"),   
                                    data = comm_ind_df, 
                                    family = binomial(link = "logit"),
                                    vcov = ~ pin),
    
     "By Incent&Use" = fixest::feglm(building ~ incent_prop + land_use |  clean_name + year,
                                    panel.id = c("pin", "year"),   
                                    data = comm_ind_df, 
                                    family = binomial(link = "logit"),
                                    vcov = ~ pin),
    
  "By Land Use"  = fixest::feglm(building ~ land_use | clean_name + year,
                              panel.id = c("pin", "year"),   
                              data = comm_ind_df, 
                              family = binomial(link = "logit"),
                              vcov = ~ pin ),
  
  "Land Use and\nFMV Growth" = fixest::feglm(building ~ incent_prop + land_use + fmv_growth_2011_w | clean_name + year,
                                           panel.id = c("pin", "year"),   
                                           data = comm_ind_df, 
                                           family = binomial(link = "logit"),
                                           vcov = ~ pin),
  
  "Land Use \nInteract FMV" = fixest::feglm(building ~ land_use*incent_prop | clean_name + year,
                                    panel.id = c("pin", "year"),   
                                    data = comm_ind_df, 
                                    family = binomial(link = "logit"),
                                    vcov = ~ pin)
