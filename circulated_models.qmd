...
```{r}
#| label: tbl-pinandmunimodels-notfiltered
#| tbl-cap: "PIN and Year FE compared with Municipality and Year FE models."

incent_paper_models <- list(
  
  "PIN-Level" = list(
    # landuse_change would be dropped (coded to be constant every year)
    # incent_status would be dropped (ditto)
    # landuse_change*incent_status dropped
    # land_use & incent_prop can change each year
  "I" = 
  ...
  "II" = 
    ...
    feols(fmv_growth_2011_w ~ incent_prop + land_use + land_use*incent_status + landuse_change*incent_prop
                           | pin + year,
                           panel.id = c("pin", "year"),
                           vcov = "NW",
                           data = comm_ind)
...
#| tbl-cap: "PIN and Year FE compared with Municipality and Year FE models. Drop observations with Excluded category in incent_status or landuse_change.  Winsorized."
...
incent_paper_models <- list(
  ...
  "PIN-Level" = list(
    ...
    feols(fmv_growth_2011_w ~ incent_prop + land_use
                           ...
                           vcov = "NW",
                           data = comm_ind |> 
            filter(incent_status != "Excluded" & landuse_change !="Excluded")),
...
```{r}
#| label: tbl-PINmodels
#| tbl-cap: "PIN and year fixed effects."
#| 
incent_paper_models <- list(
