...
#| tbl-cap: "Growth from 2011 to 2022 by aggregate incent_status FMV. Shows growth values as of 2022."
...
df_2011_v2_bal <- comm_ind_pins %>%
  ...
  mutate(            
    base_year_fmv = group_year_fmv[year == min(year)],
    fmv_group_growth = (group_year_fmv / base_year_fmv) -1) 
df_2011_v2_bal %>% filter(year == 2022)
...
df_2011_v2_bal %>%  
  filter(landuse_change != "Excluded") %>%
  mutate(year = as.factor(year)) %>%
  ggplot() + 
  geom_line(aes(x=year, y=fmv_group_growth, group = landuse_change, color = landuse_change)) +
  theme_bw()  +  
