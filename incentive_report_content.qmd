...
group_by( incent_status) %>%
  mutate(
    base_year_fmv = group_year_fmv[year == min(year)],
    fmv_group_growth = (group_year_fmv / base_year_fmv) -1) |> filter(year == 2022)  %>%
...
#  ones with missing incent_status appear to have not existed for many years.

comm_ind_pins %>%   mutate(incent_status = ifelse(years_existed < year_count, "Excluded", incent_status)) %>%
ungroup() %>% filter(is.na(incent_status)) %>% arrange(pin, year)
...
#| tbl-cap: "Includes all commercial and industrial PINs even if they did not exist for all years. (Unbalanced data for 2011 to 2022)"
...
df_2011 %>% 
  ...
  labs( title = "Commercial v Industrial FMV Growth - Not Balanced") + 
...
df_2011_v2 %>%  
  ...
  labs(title= "Growth from 2011 to 2022 - Unbalanced", 
...
df_2011_v2_bal %>%  
  ...
  scale_x_discrete(breaks = c(2012, 2022)) + 
  scale_y_continuous(breaks = c(1, 2, 3, 4), labels = scales::percent) +
  labs(title= "Growth from 2011 to 2022 - Balanced", 
       subtitle = "Incentive Classification Status by Land Use Change",
       y = "FMV Growth since 2011", x = NULL,
