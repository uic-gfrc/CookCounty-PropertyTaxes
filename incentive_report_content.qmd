...
#| tbl-cap: "Growth from 2011 to 2022. Includes all commercial and industrial PINs  if they exist for all years in sample timespan. (Balanced data for 2011 to 2022)"

df_2011_bal <- ptax_pins %>%
  filter(year >= 2011) %>%
  group_by(pin) %>% 
  mutate(n=n()) %>%
...
#| tbl-cap: "Growth from 2011 to 2022. Includes all commercial and industrial PINs even if they did not exist for all years. (Unbalanced data for 2011 to 2022)"
...
```{r}
ptax_pins %>% filter(is.na(incent_status))
...
#| tbl-cap: "Growth from 2011 to 2022 - Change in Land Use by Incentive Class Status using balanced data for 2011 to 2022. Not winsorized."
...
df_2011 %>% 
  select(year, land_use, incent_status, fmv_group_growth, n_group_growth) %>% 
  filter(year == 2022) %>% select(-year)
...
df_2011_bal %>% 
  filter(land_use == "Commercial" | land_use == "Industrial") %>% 
  ggplot() + geom_line(aes(x=year, fmv_group_growth, group = incent_status, color = incent_status)) + #scale_y_continuous(limits = c(0.8,2.5)) + 
  labs( title = "Commercial v Industrial FMV Growth-Balanced PINs",) + 
  theme_bw()  +  facet_wrap(~land_use)
...
df_2011_v2 %>% 
  select(year, landuse_change, incent_change, fmv_growth) %>% 
  filter(year == 2022) 
...
df_2011_v2 %>%  
  ...
  ggplot() + 
  geom_line(aes(x=year, y=fmv_growth, group = landuse_change, color = landuse_change)) +
  theme_bw()  +  
