...
cook_ratechange %>% 
select(contains("forgonerev") ) %>% kableExtra::kable_classic()
```
...
### Policy Scenario - Recaptured TIF EAV
...
df_2011_bal %>% select(year, land_use, incent_status, fmv_group_growth, n_group_growth) %>% 
filter(year == 2022) 
...
df_2011 %>% select(year, land_use, incent_status, fmv_group_growth, n_group_growth) %>% 
filter(year == 2022) 
