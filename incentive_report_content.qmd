...
ptax_pins_mvh <- ptax_pins |>
  ...
  mutate(lag_land = (ifelse(lag(land_use) == "Land", 1, 0)),
         lag_incent = ifelse(lag(incent_prop) == "Incentive", 1, 0),
         lag_lag_incent = lag(lag_incent)) |>
  ungroup() |>
  ...
  mutate(build_land = ifelse(lag_land == "1" & land_use != "Land", 1, 0),
         ...
         gain_incent_lag = ifelse(lag_lag_incent == "1" & incent_prop == "1", 1, 0)) |>
    # change to factors
  ...
  mutate(build_incent = ifelse((gain_incent == "1" & build_land == "1"), 1, 0),
         build_incent_lag = ifelse(gain_incent_lag == "1" & 
                                   build_land == "1", 1, 0)) |>
...
ptax_pins_mvh_final <- ptax_pins_mvh |>
  ...
  group_by(year) |>
  summarize("New Construction" = sum(build_land == "1", na.rm = T), "Gained Incentive" = sum(gain_incent == "1", na.rm = T), "Lagged gained incentive" = sum(gain_incent_lag == "1", na.rm = T), "New Construction w/ Incentive" = sum(build_incent == "1", na.rm = T), "New Construction w/ Lagged Incentive" = sum(build_incent_lag == "1", na.rm = T))
...
writexl::write_xlsx(tablelist, "Output/supporting_materials_incentivereport.xlsx")
```
