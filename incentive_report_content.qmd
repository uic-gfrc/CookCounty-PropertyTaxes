...
#| tbl-cap: "Percent of Industrial FMV within each Municipality that has incentive classification."
...
#| tbl-cap: "Growth from 2011 to 2022. Grouped by year, land_use, incent_prop."

year_count <- 12

comm_ind_pins %>%
  ungroup() %>% 
  filter(year == 2022 & years_existed == 12) %>%
...
#| tbl-cap: "Tables with \"Always Incent\" or \"Always Industrial\" Pin Counts and FMV Growth"
...
```{r}
#| include: false
#| label: tbl-growthcalculations-balanced
#| tbl-cap: "Unintuitive Table! Sums FMV for each combination of cateogories, but has a constnatly changing denominator depending due to pinschanging their land use or incentive type. This mostly shows just that more properties got incentives over time! Growth from 2011 to 2022 for all commercial and industrial PINs  if they exist for all years in sample timespan. (Balanced data for 2011 to 2022)"

df_2011_bal <- comm_ind_pins %>%
  filter(year >= 2011) %>%
  group_by(pin) %>% 
  mutate(n=n()) %>%
  filter(years_existed == year_count ) %>%
  ungroup() %>%
  group_by(year, land_use, incent_prop) |>
  summarize(
    group_pin_count = n(),
...
df_2011_bal %>% 
  select(year, land_use, incent_prop,  fmv_group_growth, n_group_growth, everything()) %>% 
  filter(year == 2022) %>% 
  select(-year)
...
```{r}
#| include: false
#| label: tbl-growthcalculations-unbalanced
#| tbl-cap: "Unintuitive Table! Sums FMV for each combination of cateogories, but has a constnatly changing denominator depending due to pinschanging their land use or incentive type. This mostly shows just that more properties got incentives over time! Growth from 2011 to 2022. Includes all commercial and industrial PINs even if they did not exist for all years. (Unbalanced data for 2011 to 2022)"
...
df_2011 %>% 
  select(year, land_use, incent_prop, fmv_group_growth, n_group_growth) %>% 
  filter(year == 2022)  %>% 
  select(-year)
...
```{r}
#| include: false
#| label: fig-indexedfmvgrowthbylanduse
#| fig-cap: "Unintuitive Figure! Use Always Commercial or Always Industrial if trying to make comparisons. Has same changing denominator problem."


df_2011 %>% 
  ggplot() + 
  geom_line(aes(x=year, y=fmv_group_growth, group = incent_prop, color = incent_prop)) + 
  #scale_y_continuous(limits = c(0.8,2.5)) + 
  labs( title = "Commercial v Industrial FMV Growth") + 
  theme_bw()  +  facet_wrap(~land_use)

df_2011_bal %>% 
  ggplot() + 
  geom_line(aes(x=year, y=fmv_group_growth, group = incent_prop, color = incent_prop)) + 
  #scale_y_continuous(limits = c(0.8,2.5)) + 
  labs( title = "Commercial v Industrial FMV Growth-Balanced PINs",) + 
  theme_bw()  +  facet_wrap(~land_use)

```

```{r}
comm_ind_pins %>% ungroup() %>% filter(is.na(incent_status)) %>% arrange(pin, year)


comm_ind_pins %>% ungroup() %>% filter(years_existed < 10) %>% arrange(pin, year)
...
comm_ind_pins %>%
  filter(year >= 2011) %>%
  group_by(pin) %>% mutate(years_existed = n()) |>
  filter(years_existed == year_count ) %>%
  ungroup() %>%
...
comm_ind_pins %>%
  ...
  mutate(
    ...
    fmv_group_growth = (group_year_fmv / base_year_fmv) -1) %>%
    filter(year == 2022) %>%   
  select(incent_status, fmv_group_growth, group_pin_count)
```

```{r}
#| label: tbl-growthcalculations-landusechange-incentstatus-balanced
#| tbl-cap: "Growth from 2011 to 2022 - Change in Land Use by Incentive Class Status using balanced data for 2011 to 2022. Not winsorized version of best table."
...
```{r}
#| label: tbl-growthcalculations-incentstatus-balanced
#| tbl-cap: "Growth from 2011 to 2022. Includes all commercial and industrial PINs even if they did not exist for all years. (Balanced data for 2011 to 2022)"
...
df_2011_bal <- comm_ind_pins %>%
  ...
  filter(years_existed == year_count ) %>%
  ungroup() %>%
  group_by(year, land_use, incent_status) |>
  summarize(
    group_pin_count = n(),
    group_year_fmv = sum(fmv, na.rm=TRUE)) |> 
  ungroup() |>
  group_by(land_use, incent_status) %>%
  mutate(
    base_year_fmv = group_year_fmv[year == min(year)],
    base_year_n = group_pin_count[year == min(year)],
    n_group_growth = group_pin_count/base_year_n -1,
...
df_2011_bal %>% 
  select(year, land_use, incent_status, fmv_group_growth, group_pin_count) %>% 
  filter(year == 2022) %>%   
  select(-year)
...
```{r}
#| include: false
#| label: tbl-growthcalculations-incentstatus-unbalanced
#| tbl-cap: "Unintuitive Table! Different numbers of PINs in different years. Doesn't actually show growth. Growth from 2011 to 2022. Includes all commercial and industrial PINs even if they did not exist for all years. (Unbalanced data for 2011 to 2022)"
...
df_2011 %>% 
  select(year, landuse_change, incent_status, fmv_group_growth, n_group_growth) %>% 
  filter(year == 2022) %>% select(-year)

df_2011 %>% filter(is.na(incent_status))
```
...
df_2011 %>% 
  ...
  ggplot() + 
  geom_line(aes(x=year, y=fmv_group_growth, group = incent_status, color = incent_status)) + #scale_y_continuous(limits = c(0.8,2.5)) + 
  labs( title = "Commercial v Industrial FMV Growth") + 
  theme_bw()  +  facet_wrap(~landuse_change)
...
df_2011_bal %>%
  filter(land_use == "Commercial" | land_use == "Industrial") %>%
  ggplot() + geom_line(aes(x=year, y=fmv_group_growth, group = incent_status, color = incent_status)) +# scale_y_continuous(limits = c(0.8,2.5)) +
  labs( title = "Commercial v Industrial FMV Growth-Balanced PINs",) +
  theme_bw()  +  facet_wrap(~land_use)
...
#| tbl-cap: "Growth from 2011 to 2022. grouped by new categorical variables. Includes all commercial and industrial PINs even if they did not exist for all years. Unbalanced."

df_2011_v2 <- comm_ind_pins %>%
...
df_2011_v2 %>% 
  select(year, landuse_change, incent_change, fmv_group_growth) %>% 
  filter(year == 2022) 
...
#| fig-cap: "**Growth from 2011 to 2022.** Faceted by if a PIN changed landuse during the sample period. Indexed to fair market value during 2011. Unbalanced and Balanced Charts."
#| layout-ncol: 2
...
df_2011_v2 %>%  
  ...
  theme_bw()  +  
  facet_wrap(~incent_change) +
  scale_x_discrete(breaks = c(2012, 2022)) + 
  scale_y_continuous(breaks = c(1, 2, 3, 4), labels = scales::percent) +
...
df_2011_v2_bal %>%  
  ...
  labs(title= "Growth from 2011 to 2022", 
       subtitle = "Incentive Classification Status by Land Use Change",
       y = "FMV Growth since 2011", x = NULL,
       caption =  "Values are indexed to 2011 FMV. Only uses PINs that existed all years of the sample. (Balanced Panel Data)") +
  theme(legend.title = element_blank(), legend.position = "bottom")
...
#| fig-cap: "**Growth from 2011 to 2022.** Faceted by if a PIN changed landuse during the sample period. Indexed to fair market value during 2011. Balanced data!"
...
df_2011_bal %>%  
  ...
  facet_wrap(~incent_status, nrow = 1) +
  scale_x_discrete(breaks = c(2012, 2022)) + 
  scale_y_continuous(breaks = c(-0.5, 1, 2, 3, 4), labels = scales::percent,) +
  labs(title= "Growth from 2011 to 2022", 
       subtitle = "Incentive Classification Status by Land Use Change",
       y = "FMV Growth since 2011", x = NULL,
       caption =  "Values are indexed to 2011 FMV") +
...
df_2011_bal %>%   
  ungroup() |>
  ggplot() + 
  geom_line(aes(x=year, y=fmv_group_growth, group = incent_status, color = incent_status)) + theme_bw() + 
  facet_wrap(~landuse_change, nrow=1) + 
