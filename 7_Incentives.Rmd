...
```{r}
nbh_res <- nbh_major_class %>% filter(major_class_code == 2) %>% select(nbhd_code, major_class_code, residential_PC = pin_count, res_EAV = Total_EAV, res_exemptions = Exempt_EAV) %>%
  mutate(nbhd_code = as.character(nbhd_code))
