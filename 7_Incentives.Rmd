...
::: {.callout-note appearance="minimal"} Assessor neighborhoods are 5 digits: first 2 for the township, last 3 are for the neighborhood within the township. These do not resemble the neighborhoods that many of us may think of but are instead of groups of properties with similar characteristics **(that are taxed in similar rates? double check)** :::
...
```{r}
nbh_pincounts <- parcel_data %>% 
  group_by(nbhd_code, class) %>%
  dplyr::summarize(pin_count = n(),
            av = sum(av)) %>%
  mutate(class_1dig = str_sub(class, 1, 1))

nbh_pincounts_majorclass <- parcel_data %>% 
...
parcel_data %>% 
  mutate(major_class_fct = recode_factor(
  class_1dig,
  "1" = "1 - Vacant",
  "2" = "2 - Residential",
  "3" = "3 - Multifamily",
  ...
  group_by(major_class_fct, nbhd_code) %>%
  summarize(nbh_av = sum(av),
            pin_count = n()) 
