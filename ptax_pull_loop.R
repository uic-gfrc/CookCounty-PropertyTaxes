...
for(i in years){
  ...
  muni_level_summary2 <- joined_pin_data %>%
    ...
    summarize(
      av = sum(av, na.rm = TRUE),
      eav = sum(eav, na.rm = TRUE),
      equalized_av = sum(equalized_av, na.rm = TRUE),          # all EAV in Muni before removing TIF increment or exemptions
      pins_in_muni = n(),
  ...
  tc_mc_summaries2 <- joined_pin_data %>%
    ...
    summarize(
      ...
      TC_MC_AV = sum(av),
      TC_MC_EAV = sum(eav),
      TC_MC_allexempssum = sum(exe_homeowner+exe_senior +exe_freeze + 
      ...
      TC_MC_exe_homeowner = sum(exe_homeowner, na.rm=TRUE),
      TC_MC_exe_senior = sum(exe_senior, na.rm=TRUE),
      TC_MC_exe_freeze = sum(exe_freeze, na.rm=TRUE),
      TC_MC_exe_longtime_homeowner = sum(exe_longtime_homeowner, na.rm=TRUE),
      TC_MC_exe_disabled = sum(exe_disabled, na.rm=TRUE),
      TC_MC_exe_vet_returning = sum(exe_vet_returning, na.rm=TRUE),
      TC_MC_exe_vet_dis_lt50 = sum(exe_vet_dis_lt50, na.rm=TRUE),
      TC_MC_exe_vet_dis_50_69 = sum(exe_vet_dis_50_69, na.rm=TRUE),
      TC_MC_exe_vet_dis_ge70 = sum(exe_vet_dis_ge70, na.rm=TRUE),
      TC_MC_exe_abate = sum(exe_abate, na.rm=TRUE),
      
      TC_MC_exe_vet_dis = sum(exe_vet_dis_lt50 + exe_vet_dis_50_69 + 
                                exe_vet_dis_ge70, na.rm=TRUE),  # all vet_dis variables added together
      zero_bills = sum(zero_bill),
      has_HO_exemp = sum(has_HO_exemp),
      has_SF_exemp = sum(has_SF_exemp),
  ...
  tc_class_summaries2 <- joined_pin_data %>%
    ...
    summarize(
      ...
      TC_C_AV = sum(av),
      TC_C_EAV = sum(eav),
      TC_C_allexempssum = sum(exe_homeowner + exe_senior + exe_freeze + 
                                exe_longtime_homeowner + exe_disabled + 
                                exe_vet_returning + exe_vet_dis_lt50 + 
                                exe_vet_dis_50_69 + exe_vet_dis_ge70 + 
                                exe_abate, na.rm = TRUE),
      
      TC_C_exe_homeowner = sum(exe_homeowner, na.rm=TRUE),
      TC_C_exe_senior = sum(exe_senior, na.rm=TRUE),
      TC_C_exe_freeze = sum(exe_freeze, na.rm=TRUE),
      TC_C_exe_longtime_homeowner = sum(exe_longtime_homeowner, na.rm=TRUE),
      TC_C_exe_disabled = sum(exe_disabled, na.rm=TRUE),
      TC_C_exe_vet_returning = sum(exe_vet_returning, na.rm=TRUE),
      TC_C_exe_vet_dis_lt50 = sum(exe_vet_dis_lt50, na.rm=TRUE),
      TC_C_exe_vet_dis_50_69 = sum(exe_vet_dis_50_69, na.rm=TRUE),
      TC_C_exe_vet_dis_ge70 = sum(exe_vet_dis_ge70, na.rm=TRUE),
      TC_C_exe_abate = sum(exe_abate, na.rm=TRUE),
      TC_C_exe_vet_dis = sum(exe_vet_dis_lt50 + exe_vet_dis_50_69 +
                               exe_vet_dis_ge70, na.rm=TRUE),  # all vet_dis variables added together
      zero_bills = sum(zero_bill),
      has_HO_exemp = sum(has_HO_exemp),
      has_SF_exemp = sum(has_SF_exemp),
