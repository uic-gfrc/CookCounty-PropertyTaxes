...
ptax_proj_summs <- ptax_proj_summs %>%
  ...
  mutate(land_use2 = factor(land_use2, levels = c("Commercial", "Industrial", "Multi Land Uses"), ordered = TRUE),
         incent_prop2 = factor(incent_prop, levels = c("Non-Incentive", "Incentive"), ordered = TRUE),
...
proj_panel <- ptax_proj_summs %>% 
  select(actual_keypin, year, incent_change2, tif_change2, incent_prop, landuse_change2, land_use2, in_tif2, triad, fmv_growth_2011, base_year_fmv_2011, project_pins, incent_pins, clean_name)  %>% 
...
feols(fmv_growth_2011 ~ land_use2*incent_prop + in_tif2 | actual_keypin, data = proj_panel)  %>% summary()
```
