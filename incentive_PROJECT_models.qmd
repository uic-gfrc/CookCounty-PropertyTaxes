...
ptax_proj_summs <- ptax_proj_summs %>%
  ...
  mutate(land_use2 = factor(land_use2, levels = c("Commercial", "Industrial", "Multi Land Uses"), ordered = TRUE)
         )

write_csv(ptax_proj_summs, "Output/project_summaries.csv")
```
...
proj_panel <- ptax_proj_summs %>% 
  select(actual_keypin, year, incent_change2, tif_change2, incent_prop, incent_prop2,
         landuse_change2, land_use2, in_tif2, triad, fmv_growth_2011, 
         base_year_fmv_2011, project_pins, incent_pins, clean_name)  %>% 
  mutate( fmv_growth_2011 = as.numeric(fmv_growth_2011)) %>% 
  arrange(actual_keypin, year) %>% 
...
class(proj_panel$land_use2)
class(ptax_proj_summs$incent_prop2)

table(ptax_proj_summs$land_use2)
table(proj_panel$land_use2)

table(ptax_proj_summs$incent_prop2)
table(ptax_proj_summs$incent_change2)
```


```{r}
projectlevel = feols(fmv_growth_2011 ~ land_use2 + incent_prop2 + land_use2*incent_prop2  + incent_prop2+ in_tif2 | actual_keypin + year, panel.id = c("actual_keypin", "year"), vcov = "twoway", data = proj_panel)

munilevel = feols(fmv_growth_2011 ~ land_use2*incent_prop2 + incent_prop2 + land_use2 + in_tif2 | clean_name + year, panel.id = c("actual_keypin", "year"), vcov = "twoway", data = proj_panel)
...
```{r}
projectlevel = feols(fmv_growth_2011 ~ land_use2*incent_prop2 + in_tif2  | actual_keypin + year, panel.id = c("actual_keypin", "year"), vcov = "twoway", data = ptax_proj_summs)

projectlevel2 = feols(fmv_growth_2011 ~ land_use2*incent_prop2 + in_tif2  | actual_keypin + clean_name+ year, panel.id = c("actual_keypin", "year"), vcov = "twoway", data = ptax_proj_summs)

munilevel = feols(fmv_growth_2011 ~ land_use2*incent_prop2 + incent_prop2 + in_tif2 | clean_name + year, panel.id = c("actual_keypin", "year"), vcov = "twoway", data = ptax_proj_summs)
