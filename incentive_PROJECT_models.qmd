...
ptax_proj_summs <- ptax_proj_summs %>%
  ...
  group_by(year) %>%
  mutate(fmv_w = DescTools::Winsorize(fmv, quantile(fmv, probs = c(0.03,0.97), na.rm=TRUE)),
         fmv_growth_2011_w = DescTools::Winsorize(fmv_growth_2011, quantile(fmv_growth_2011, probs = c(0.01,0.99), na.rm=TRUE)),
         fmv_2022_2011_diff_w = DescTools::Winsorize(fmv_2022_2011_diff, quantile(fmv_2022_2011_diff, probs = c(0.03,0.97), na.rm=TRUE))) |>
  ungroup() 
...
proj_panel <- proj_panel |> filter(years_existed > 7)
