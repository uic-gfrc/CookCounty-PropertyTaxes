...
proj_panel <- ptax_proj_summs %>% 
  select(actual_keypin, year, incent_prop,
  ...
  #mutate( fmv_growth_2011_w = as.numeric(fmv_growth_2011_2)) %>% 
...
#| tbl-cap: "Winsorized Growth and non-winsorized growth"
library(fixest)

models <- list(
  projectlevel_2FE = feols(fmv_growth_2011_w ~ land_use2 + incent_prop2 + land_use2*incent_prop2 | actual_keypin + year^triad, panel.id = c("actual_keypin", "year"), vcov = "twoway", data = proj_panel),
               
  projectlevel_3FE = feols(fmv_growth_2011_w ~ land_use2 + incent_prop2 + land_use2*incent_prop2  | actual_keypin + year^triad + clean_name, panel.id = c("actual_keypin", "year"), vcov = "threeway", data = proj_panel),
  
  munilevel = feols(fmv_growth_2011_w ~ land_use2 + incent_prop2 + incent_prop2*land_use2 + clean_name | actual_keypin + year^triad, panel.id = c("actual_keypin", "year"), vcov = "twoway", data = proj_panel)
)


modelsummary::modelsummary(model = models, output = "flextable",
                            coef_omit = "clean_name", stars = TRUE,
                           gof_omit = "AIC|BIC", include_reference = TRUE)


models <- list(
  projectlevel_2FE = feols(fmv_growth_2011 ~ land_use2 + incent_prop2 + land_use2*incent_prop2 | actual_keypin + year^triad, panel.id = c("actual_keypin", "year"), vcov = "twoway", data = proj_panel),
               
  projectlevel_3FE = feols(fmv_growth_2011 ~ land_use2 + incent_prop2 + land_use2*incent_prop2  | actual_keypin + year^triad + clean_name, panel.id = c("actual_keypin", "year"), vcov = "threeway", data = proj_panel),
  
  munilevel = feols(fmv_growth_2011 ~ land_use2 + incent_prop2 + incent_prop2*land_use2 + clean_name | actual_keypin + year^triad, panel.id = c("actual_keypin", "year"), vcov = "twoway", data = proj_panel)
)
...
#| tbl-cap: "Other variations of fixed effects and lagged reassessment years. "

models <- list(
  reassess_year = feols(fmv_growth_2011_w ~ land_use2 + incent_prop2 + land_use2*incent_prop2  + reassessed_year | actual_keypin + year, panel.id = c("actual_keypin", "year"), vcov = "twoway", data = proj_panel),
  
  triad = feols(fmv_growth_2011_w ~ land_use2 + incent_prop2 + land_use2*incent_prop2 + triad | actual_keypin + year, panel.id = c("actual_keypin", "year"), vcov = "twoway", data = proj_panel),
  
  lagged_assessment = feols(fmv_growth_2011_w ~ land_use2 + incent_prop2 + land_use2*incent_prop2 + in_tif2 + l(reassessed_year, 0:2) | actual_keypin + year, panel.id = c("actual_keypin", "year"), vcov = "twoway", data = proj_panel),
  
  yeartriad_FE = feols(fmv_growth_2011_w ~ land_use2 + incent_prop2 + land_use2*incent_prop2 + in_tif2 + l(reassessed_year, 0:2) | actual_keypin +year+ year^triad, panel.id = c("actual_keypin", "year"), vcov = "threeway", data = proj_panel)
)
...
models <- list(
  projectlevel = feols(fmv_growth_2011 ~ land_use2*incent_prop2 + in_tif2*incent_prop2 + incent_pins | actual_keypin + year, panel.id = c("actual_keypin", "year"), vcov = "twoway", data = proj_panel),

  yeartriad_FE = feols(fmv_growth_2011 ~ land_use2*incent_prop2 + land_use2*in_tif2| actual_keypin + year^triad, panel.id = c("actual_keypin", "year"), vcov = "twoway", data = proj_panel))

modelsummary::modelsummary(models, stars = TRUE, output = "flextable")
```
...
modelsummary::modelsummary(models, statrs = TRUE, output = "flextable")
