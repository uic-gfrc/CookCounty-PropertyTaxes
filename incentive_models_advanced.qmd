...
comm_ind_temp <- comm_ind_temp |> 
  select(pin, year, clean_name, fmv, fmv_growth_2011, fmv_2011, incent_prop, 
         land_use, major_class_code, # landuse_change, incent_change, 
         triad, 
...
plm(fmv_growth_2011 ~ incent_prop*land_use,
                       model = "within",
                       effect = "twoways",
                       data = comm_ind_panel_pin) %>% 
  summary()
  
plm(fmv_growth_2011 ~ lag(incent_prop) + land_use,
                       model = "within",
                       effect = "twoways",
                       data = comm_ind_panel_pin) %>% 
  summary()

plm(fmv_growth_2011 ~ lag(incent_prop) + land_use  + major_class_code,
                       model = "within",
                       effect = "twoways",
                       data = comm_ind_panel_pin) %>% 
  summary()
...
plm(fmv_growth_2011 ~ major_class_code + land_use,
                       model = "within",
                       effect = "twoways",
                       data = comm_ind_panel_muni) |> summary()
