...
change_interact_list <- list(
  ...
  model_1 <- plm(fmv_growth_2011 ~ landuse_change + incent_change,
                 model = "within",
                 effect = "twoways",
                 data = comm_ind_panel_muni),
  
  model_2 <- plm(fmv_growth_2011 ~ landuse_change + incent_change
                 + landuse_change*incent_change,
                 model = "within",
                 effect = "twoways",
                 data = comm_ind_panel_muni),
  
  model_3 <- plm(fmv_growth_2011 ~ prop_use_change_bin + incent_change_bin,
                 model = "within",
                 effect = "twoways",
                 data = comm_ind_panel_muni),
  
    model_4 <- plm(fmv_growth_2011 ~ prop_use_change_bin + incent_change_bin
...
```{r ind_vs_comm_models}
#| tbl-cap: "Land Use Dummy"

land_use_list <- list(
  
  "Land Use Change" <- plm(fmv_growth_2011 ~ prop_use_change_bin + land_use,
                           model = "within",
                           effect = "twoways",
                           data = comm_ind_panel_muni),
  
  "Incentive Class Change" <- plm(fmv_growth_2011 ~ incent_change_bin + land_use,
                                  model = "within",
                                  effect = "twoways",
                                  data = comm_ind_panel_muni)
)

modelsummary(land_use_list,
             stars = TRUE)

```

```{r only_ind_models}
#| tbl-cap: "Only Industrial"

ind_panel_muni <- comm_ind_panel_muni |>
  mutate(ind_bin = as.factor(
    ifelse(land_use == "Industrial", 1, 0))
    )|>
  group_by(pin) |>
  filter(ind_bin >= 1)
...
only_ind_list <- list(
  ...
  "Land Use Change" <- plm(fmv_growth_2011 ~ prop_use_change_bin,
                           model = "within",
                           effect = "twoways",
                           data = comm_ind_panel_muni),
