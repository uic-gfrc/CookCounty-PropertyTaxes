...
comm_ind_temp <- comm_ind |>
  # 1,187,124 obs. (i.e., lost no obs.)
  filter(!is.na(fmv)) |>
  # 1,118,232 obs. (lose 68892 obs.)
  filter(!is.na(fmv_growth_2011))

comm_ind_temp <- comm_ind_temp %>% select(pin, year, clean_name,  fmv_growth_2011, base_year_fmv_2011, incent_prop, land_use, landuse_change, incent_change, triad)

comm_ind_panel_pin <- pdata.frame(comm_ind_temp, index = c("pin", "year"))

comm_ind_panel_muni <- pdata.frame(comm_ind_temp, index = c("pin", "year", "clean_name"))
...
# fe_pin_model_list <- list(
#   
#   "PIN (1)" = plm(fmv_growth_2011 ~ land_use,
#                   model = "within",
#                   effect = "twoways",
#                   data = comm_ind_panel_pin),
#   
#   "PIN (2)" = plm(fmv_growth_2011 ~ incent_prop,
#                   model = "within",
#                   effect = "twoways",
#                   data = comm_ind_panel_pin)
#   
# )
# modelsummary(fe_pin_model_list)

plm(fmv_growth_2011 ~ base_year_fmv_2011 + land_use,
                  model = "within",
                  effect = "twoways",
                  data = comm_ind_panel_pin) %>% summary()
  
plm(fmv_growth_2011 ~ base_year_fmv_2011 + incent_prop,
                  model = "within",
                  effect = "twoways",
                  data = comm_ind_panel_pin) %>% summary()
...
plm(fmv_growth_2011 ~ base_year_fmv_2011 + land_use,
                  model = "within",
                  effect = "twoways",
                  data = comm_ind_panel_muni) %>% summary()
