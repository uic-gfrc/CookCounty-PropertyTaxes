...
comm_ind |>
  select(year, pin, clean_name) |>
  group_by(pin) |>
  summarize(n = n()) |>
  # 12 years in the data
  filter(n != 12) |>
  arrange(desc(n)) |>
  # 0 PINs have fewer than 12 observations.
  head(10)

# 1187124 obs.
comm_ind_temp <- comm_ind |>
  # 1187124 obs. (i.e., lost no obs.)
  filter(!is.na(fmv)) |>
  # 11118232 obs. (lose 68892 obs.)
  filter(!is.na(fmv_growth_2011))

comm_ind_panel_pin <- pdata.frame(comm_ind_temp, index = c("pin", "year"))

comm_ind_panel_muni <- pdata.frame(comm_ind_temp, index = c("agency_num", "year"))
...
fe_pin_model_list <- list(
  ...
  "PIN (1)" = plm(fmv_growth_2011 ~ lag(incent_prop),
                  model = "within",
                  effect = "twoways",
                  data = comm_ind_panel_pin),
  
  "PIN (2)" = plm(fmv_growth_2011 ~ lag(incent_prop) + land_use + land_use*lag(incent_prop),
                  model = "within",
                  effect = "twoways",
                  data = comm_ind_panel_pin)
