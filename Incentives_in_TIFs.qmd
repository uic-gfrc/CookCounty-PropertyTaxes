...
bor <- bor %>% mutate(
  class = str_sub(class, 1,3)) %>% filter(class > 399) 

bor <- bor %>% 
#  mutate(project_id = str_sub(appealid, 1, 7)) 
...
#| tbl-cap: "Board of Review Appeal Data - Grouped by apellant and project ID. Project ID is created from the `appealid` variable used by the BoR. The `appealid` appears to group PINs by the digits before the decimal point."
...
bor %>% 
  ...
  filter(pincount > 1)  %>% 
  arrange(desc(tax_year))  %>% datatable(rownames = FALSE)
```
...
#| tbl-cap: "Count of PINs within project ID from Board of Review. Uses appealid to create project ID."
...
bor <- bor %>% filter(project_id %in% projects$project_id)


bor_pins %>%
  group_by(project_id) %>%
  summarize(pincount = n()) %>%
  arrange(desc(pincount))  %>% datatable(rownames = FALSE)
...
82,957 distinct keypins from all_keypins file
...
proj_xwalk %>% distinct(keypin) # 82 distinct keypins from all_keypins file

# combine project IDs to PINs:
ptax_pins <- ptax_pins %>% left_join(proj_xwalk)

ptax_pins <- ptax_pins %>% left_join(bor_pins)
...
ptax_pins <- ptax_pins %>% 
  mutate(both_ids = ifelse(needs_keypin==1, project_id, keypin ),
         both_ids = ifelse(is.na(both_ids), pin, both_ids))

# ptax_pins %>% group_by(both_ids) %>% summarize(pincount = n()) 

ptax_pins %>% filter(class > 599) %>% group_by(both_ids) %>% summarize(pincount = n())  %>%
