...
puni_keypins <- parcuniverse_keypins %>% select(pin14, keypin, proration_rate) %>% rename(puni_keypin = keypin)
...
ptax_pins <- ptax_pins %>% left_join(puni_keypins, by = c("pin" = "pin14"))
...
ptax_pins <- ptax_pins %>% 
  mutate(both_ids = puni_keypin) %>%
  group_by(puni_keypin) %>% 
  mutate(puni_PC = n()) %>%
  ungroup() %>%
  mutate(
          no_punimatch = ifelse(is.na(puni_keypin) | puni_PC < 2, 1, 0),
          both_ids = ifelse(puni_PC < 2, NA, both_ids),
          both_ids = ifelse(is.na(both_ids), project_id, both_ids),
...
table2 %>% 
  left_join(nicknames) %>%
  filter(clean_name != "Chicago"& !is.na(clean_name)) %>%
...
#| fig-cap: "Number of projects with Incentive Classification. Areas with hatch marks are municipalities that do not use incentive property classes. A project indicates grouped PINs that belong together (based on sharing an appeal ID from the BOR, prjoect IDs from the commercial valuation data, or determined by some other way to belong together.). Excludes Chicago, which has 240 projects." 
...
table2 %>% 
  ...
  filter(incent_prop  == "Incentive" &!is.na(clean_name)) %>%
  full_join(muni_shp, by = c("shpfile_name" = "MUNICIPALITY")) %>%
