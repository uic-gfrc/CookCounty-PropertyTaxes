---
title: "Create Joined PIN Level Data for 2023"
author: "AWM"
date: "`r Sys.Date()`"
...
tif_distrib <- DBI::dbGetQuery(
  ...
  WHERE year = 2023'), .con = ptaxsim_db_conn)
...
Equalization factor for 2023 is 3.02 Use this to calculate the taxable AV from EAV. 

```{r}
# 2023 Tax Bills ---------------------------------------------------------------

# Create value for 2023 equalization factor
...
taxbills <- tax_bill(2023,  cook_pins$pin, simplify = FALSE)
...
pin14_bills <- taxbills %>%
  ...
  mutate(total_bill = final_tax_to_dist + final_tax_to_tif,
                     eq_av = av/eq2023) %>% # from each taxing agency
...
exemption_data <- lookup_pin(2023, cook_pins$pin) %>%
...
```{r}
write_csv(joined_pin_data, "./Output/Dont_Upload/0_joined_PIN_data_2023.csv")
```
