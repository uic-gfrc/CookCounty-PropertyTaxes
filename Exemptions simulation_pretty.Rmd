...
burden_shift %>% 
  ...
  labs(title = "New composite tax rates if exemptions were eliminated" ,    
         caption = "Data from CCAO & PTAXSIM. ")
```
...
burden_shift %>%  
  ...
  pivot_longer(c("current_burden", "burden_noexemps"), 
               names_to = "type", values_to = "pct_burden") %>% 
  inner_join(order) %>%
  ggplot(aes(x = pct_burden*100, y= reorder(agency_name, current_burden)))+
  geom_line(aes(group = agency_name))+ 
   geom_point(aes(color=type), size=3 )+
  theme_minimal() + 
...
dataset_names <- list('ResidentialEAV' = table_ResidentialEAV, 
                      'Burden Shift' = burden_shift_table,
