...
data <- data %>% 
  ...
  # ) %>%
  mutate(
    below_freeze = ifelse(HHINCOME >= 85000, "Not Eligible", "Eligible"),
...
```{r}
incometable <- svytable(~ YEAR + PUMA + below_freeze, design = HHdesign)

mapPUMAboth <- incometable %>% 
  as_tibble() %>%
  mutate(PUMA = str_pad(PUMA, 5, side= "left", pad = "0"),
        # below_freeze = ifelse(hhincdecile_w >= 5, "Not Eligible", "Eligible")
         ) |>
  group_by(YEAR, PUMA) |>
     mutate(
       Prop = n/sum(n)) %>%
     filter(below_freeze == "Eligible")|>
  left_join(pumas, by = c("PUMA" = "puma", "YEAR" = "YEAR"))
...
```{r}
incometable <- svytable(~ YEAR + PUMA + age_cat + below_freeze, design = HHdesign)
...
mapPUMAboth <- incometable %>% 
  ...
  group_by(YEAR, PUMA) |>
     mutate(
       Prop = n/sum(n)) %>%
     filter(below_freeze == "Eligible" & age_cat == "65+") %>%
