...
# individual level, we want household level. so this is probably NOT what we want to be using.
# dstrata <- survey::svydesign(id = ~CLUSTER, strata = ~STRATA, weights = ~PERWT, data = data) %>% 
#   as_survey() %>%
#   mutate(decile = ntile(INCEARN, 10),
#          hhdecile = ntile(HHINCOME, 10))
```

```{r}
# PUMA shapefiles

cook_words <- c("Cook", "Chicago")
...
#             owned = max(OWNERSHP),
#             hhinc = max(HHINCOME, na.rm=TRUE),
#             house_value = max(VALUEH),
#             oldest = max(AGE),
#             youngest = min(AGE), 
#             PUMA = mean(PUMA, na.rm=TRUE)) |> 
#   #filter(multi_peep > 1) |> 
#   arrange(desc(multi_peep) )
...
```{r}
obs_perPUMA<- data %>% 
   group_by(PUMA, age_cat) %>% 
   dplyr::summarize(weightedcount=sum(HHWT), #weighted 
                    unweightedcount = n()) %>%
  arrange(PUMA)
obs_perPUMA
obs_perPUMA |> select(-unweightedcount) |> pivot_wider(names_from = "age_cat", values_from = "weightedcount")
...
data <- data %>% 
  ...
  mutate(
    above_freeze = ifelse(hhincdecile_w >= 5, "Not Eligible", "Eligible"),
...
agetable <- svytable(~ PUMA+ age_cat, design = HHdesign)
...
mapPUMAboth <- agetable %>% 
  ...
  group_by(PUMA) %>% 
    ...
    left_join(pumasIL2020, by = c("PUMA" = "PUMACE20"))
...
```{r}
incometable <- svytable(~ PUMA + above_freeze, design = HHdesign)
...
mapPUMAboth <- incometable %>% 
  ...
  group_by(PUMA) |>
     mutate(
       Prop = n/sum(n)) %>%
     filter(above_freeze == "Eligible") %>%
    inner_join(pumasIL, by = c("PUMA" = "PUMACE20"))
...
figure5c <- ggplot(mapPUMAboth, aes(fill = Prop)) +
  geom_sf(aes(geometry = geometry), color = "black")+ 
  labs(title = "Share of ALL households below 77K") + 
  theme_void() + 
  theme(axis.ticks = element_blank(), axis.text = element_blank())+
     ...
     scale_fill_binned(high = "darkblue", low = "white", 
                       ...
                       nice.breaks=FALSE,
                       labels=scales::percent,
                        name = "% with HH Inc < 77K ")
figure5c
```

```{r}
incometable <- svytable(~ PUMA + above_freeze + age_cat, design = HHdesign)
...
mapPUMAboth <- incometable %>% 
  ...
  group_by(PUMA) |>
     mutate(
       Prop = n/sum(n)) %>%
     filter(above_freeze == "Eligible" & age_cat == "65+") %>%
    inner_join(pumasIL, by = c("PUMA" = "PUMACE20"))
...
figure5c <- ggplot(mapPUMAboth, aes(fill = Prop)) +
  geom_sf(aes(geometry = geometry), color = "black")+ 
  labs(title = "Share of all households 65+ w/ HH incomes < $77K") + 
  theme_void() + 
  theme(axis.ticks = element_blank(), axis.text = element_blank())+
...
```{r}
incometable <- svytable(~ PUMA + OWNERSHP + above_freeze + age_cat, design = HHdesign)
...
mapPUMAboth <- incometable %>% 
  ...
  group_by(PUMA) |>
     mutate(
       Prop = n/sum(n)) %>%
     filter(above_freeze == "Eligible" & age_cat == "65+" & OWNERSHP == 1) %>%
    inner_join(pumasIL, by = c("PUMA" = "PUMACE20"))


figure5c <- ggplot(mapPUMAboth, aes(fill = Prop)) +
  geom_sf(aes(geometry = geometry), color = "black")+ 
  labs(title = "Share of all Own. Occ. Households 65+ w/ HH incomes < $77K",
       subtitle = "Eligible for the Senior Freeze Exemption if Household Income increased to $77K") + 
  theme_void() + 
  theme(axis.ticks = element_blank(), axis.text = element_blank())+
