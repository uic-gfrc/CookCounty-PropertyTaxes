...
ratios<- munis_billchange %>% 
  ...
  summarize(GHE_0_bill_to_AV_25 = max(currbill_to_AV_25, na.rm=TRUE),
            GHE_0_bill_to_AV_75 = max(currbill_to_AV_75, na.rm=TRUE)) %>%
  mutate(muni_ratio_25to75 = GHE_0_bill_to_AV_25/GHE_0_bill_to_AV_75) #%>%filter(muni_ratio_25to75 > 0.01)


ggplot(data = ratios, aes(y = GHE_0_bill_to_AV_25, x = GHE_0_bill_to_AV_75, label = clean_name)) + 
  geom_abline(intercept = 0, slope = 1) +
  geom_point(aes(alpha = .5)) + 
  geom_point(data = (ratios %>% filter(clean_name %in% c("Park Forest", "Chicago", "Glencoe", "Winnetka", "Riverdale","Dolton", "Phoenix"))), aes(y = GHE_0_bill_to_AV_25, x = GHE_0_bill_to_AV_75, color = "red"), size = 3)+
    ggrepel::geom_text_repel(data = (ratios %>% filter(clean_name %in% c("Park Forest", "Chicago", "Glencoe", "Winnetka", "Riverdale", "Dolton", "Phoenix"))), aes(y = GHE_0_bill_to_AV_25, x = GHE_0_bill_to_AV_75 + 0.1, label = clean_name, size = 2))+
  ...
  theme(legend.position = "none") +
  labs(title = "Reduced GHE Amount by 10,000 EAV (0 EAV exempt from GHE)",
       subtitle = "Other exemptions still in place", x="", y="")
...
ggplot(data = new_ratios, aes(y = newbill_to_AV_25, x = newbill_to_AV_75, label = clean_name)) + 
  ...
  geom_point(data = (new_ratios %>% filter(clean_name %in% c("Park Forest", "Chicago", "Winnetka", "Riverdale", "Dolton"))), aes(y = newbill_to_AV_25, x = newbill_to_AV_75, color = "red", size = 2))+
  theme_classic() + 
  scale_y_continuous(limits = c(0, .6), expand = c(0,0))+
  scale_x_continuous(limits = c(0, .6), expand = c(0,0))+
  theme(legend.position = "none") + 
...
ggplot(data = new_ratios, aes(y = newbill_to_AV_25, x = newbill_to_AV_75, label = clean_name)) + 
  geom_abline(intercept = 0, slope = 1) +
  geom_point(aes(alpha = .5)) + 
  geom_point(data = (new_ratios %>% filter(clean_name %in% c("Park Forest", "Chicago", "Winnetka", "Glencoe", "Riverdale", "Dolton"))), aes(y = newbill_to_AV_25, x = newbill_to_AV_75, color = "red"), size = 3)+
  theme_classic() + 
    scale_y_continuous(limits = c(0, .6), expand = c(0,0))+
...
ggplot(data = new_ratios, aes(y = newbill_to_AV_25, x = newbill_to_AV_75, label = clean_name)) + 
  geom_abline(intercept = 0, slope = 1) +
  geom_point(aes(alpha = .5)) + 
  geom_point(data = (new_ratios %>% filter(clean_name %in% c("Park Forest", "Chicago", "Glencoe", "Winnetka", "Riverdale","Dolton"))), aes(y = newbill_to_AV_25, x = newbill_to_AV_75, color = "red"), size = 3)+
  theme_classic() + 
    scale_y_continuous(limits = c(0, .6), expand = c(0,0))+
