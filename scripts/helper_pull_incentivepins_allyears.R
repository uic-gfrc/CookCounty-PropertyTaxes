...
## 31+ million obs. (PIN-YEAR combos) if ALL PINs are pulled
## 45,724 obs if only incentive classes
...
incentive_pins %>% group_by(pin) %>% summarize(years_existed = n()) %>% filter(years_existed == 17)
# 4644 PINs existed every year where they were an incentive property at least once