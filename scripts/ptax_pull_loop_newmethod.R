...
for (i in years) {
  ...
  # Summarize ---------------------------------------------------------------
  ## for "./ptaxsim.db/ptaxsim-2023.0.0.db:
  ...
  pin_data <- pin_data |>
    ...
    mutate( # zero_bill = ifelse(eav <= all_exemptions, 1, 0),  ## old way of creating zero bill but has missing vet exemptions data problem
      ...
      has_WWII_exemp = ifelse(exe_wwii > 0, 1, 0),

      # includes missing disabled veteran exemption counts
      has_DV_exemp = ifelse(exe_vet_dis_lt50 + exe_vet_dis_50_69 + exe_vet_dis_ge70 + exe_vet_dis_100 + exe_missing_disvet > 0, 1, 0),
  ...
  pin_data <- pin_data |>
    ...
    mutate(
      ...
      untaxable_value_fmv = ifelse(is.nan(untaxable_value_fmv), 0, untaxable_value_fmv),
  ...
  county_sums2 <- pin_data %>%
    ...
    summarize(
      ...
      exempt_VET_eav = sum(exe_vet_returning + exe_vet_dis_lt50 + exe_vet_dis_50_69 + exe_vet_dis_ge70 + exe_vet_dis_100 + exe_missing_disvet, na.rm = TRUE),
      exempt_WWII_eav = sum(exe_wwii, na.rm = TRUE),
      ...
      has_WWII_exemp = sum(has_WWII_exemp, na.rm = TRUE),
  ...
  if (is.data.frame(county_sums)) {
    county_sums <- rbind(county_sums, county_sums2)
  } else {
    county_sums <- county_sums2
  }
  ...
  if (is.data.frame(county_MC_sums)) {
    county_MC_sums <- rbind(county_MC_sums, county_MC_sums2)
  } else {
    county_MC_sums <- county_MC_sums2
  }
  ...
  if (is.data.frame(muni_level_summary)) {
    muni_level_summary <- rbind(muni_level_summary, muni_level_summary2)
  } else {
    muni_level_summary <- muni_level_summary2
  }
  ...
  # bind muni level yearly data together
  if (is.data.frame(muni_MC_summary)) {
    muni_MC_summary <- rbind(muni_MC_summary, muni_MC_summary2)
  } else {
    muni_MC_summary <- muni_MC_summary2
  }
  rm(muni_MC_summary2)
...
write_csv(county_sums, "./Output/ptaxsim_cook_level_2006to2024.csv")
write_csv(county_MC_sums, "./Output/ptaxsim_cook_MC_level_2006to2024.csv")



write_csv(muni_level_summary, "./Output/ptaxsim_muni_level_2006to2024.csv")

write_csv(muni_MC_summary, "./Output/ptaxsim_muni_MC_2006to2024.csv")
