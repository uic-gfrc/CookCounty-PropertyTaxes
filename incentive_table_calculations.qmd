...
MVH to-do suggestions:
1. Rename "Alea_Cat" variable (at least to Nimbus)
2. Equalization factors --> create single variables
3. Conform to 'between' command
...
# Helpful for identifying projects because lawyers keep great records.
...
# MVH: IS IT THAT THE DATA IS UNRELIABLE OR JUST FIRST PASS?
...
# BOR shortfall: We only have the data if they appeal!

bor_pins <- bor %>% 
  group_by(pin) %>% 
  arrange(desc(tax_year)) %>%
  # Does this just grab the first PIN?
...
# MVH: NEW METHODOLOGY REQUIRES FURTHER EXPLANATION.
...
ptax_pins <- ptax_pins %>% 
  ...
  mutate(
    ...
    # MVH: For some reason I thought we had gotten rid of the "taxable" concept.
    taxable_fmv = taxable_av / loa,
    taxable_eav = taxable_av * eq2022,
...
## MVH: I DON'T KNOW WHAT THAT MEANS ^^
## Maybe case studies?
...
table_cook <- ptax_pins %>%
  summarize(cty_PC = n(),
            cty_projects = n_distinct(both_ids), # mostly for industrial and commercial properties...MVH: WHY?
            fmv_incentive = sum(ifelse(between(class, 600, 900), fmv, 0), na.rm = TRUE),
            fmv_taxed =  sum(taxed_fmv, na.rm=TRUE),
            fmv_incents_inTIFs = sum(ifelse(between(class, 600, 900) & in_tif == 1, fmv, 0), na.rm = TRUE),
...
# MVH: I DONT KNOW WHAT THAT MEANS ^^
...
# MVH: I THINK WE CAN REWRITE THIS SECTION...
...
## MVH: IS THIS THE MAJOR CLASS SECTION?
            ...
            cty_MC_fmv_incents_inTIFs = sum(ifelse(between(class, 600, 900) & in_tif == 1, fmv, 0), na.rm = TRUE),
            ...
            cty_avg_C2_bill_noexe = mean(ifelse(between(class, 200, 299) & all_exemptions == 0,
                (final_tax_to_dist+ final_tax_to_tif), NA), na.rm=TRUE),
            cty_avg_C2_bill_withexe = mean(ifelse(between(class,200, 299) & all_exemptions > 0, (final_tax_to_dist+ final_tax_to_tif), NA), na.rm=TRUE),
...
## MVH: I THINK WE JUST OMIT THOSE OBSERVATIONS.
...
munilevel <- ptax_pins %>% 
 ...
 # Move to creating muni-level data.
  mutate(
...
muni_effective_rate =  muni_levy / muni_fmv * 100
  ...
  ungroup() %>%
  select(clean_name, muni_effective_rate, starts_with("muni"))
...
table2 <- nonres_pins2022 %>% 
  ...
  summarize(pin_count = n(),
          ...
          fmv_incent = sum(ifelse(between(class, 600, 899), av/loa, 0), na.rm=TRUE),
          av_incent = sum(ifelse(between(class, 600, 899), (av/loa)*0.25, 0), na.rm=TRUE),
    # AV if level of assessment was 25% for everyone
    # MVH: DOES THIS INCLUDE RESIDENTIAL??
          av_adjusted = sum(ifelse(between(class, 600, 899), (av/loa)*0.25, av), na.rm=TRUE),
         fmv = sum(av/loa, na.rm=TRUE),
         av = sum(av, na.rm=TRUE)) %>%
  mutate(pct_av_incent = av_incent / av_adjusted,
         pct_fmv_incent = fmv_incent/fmv)  %>%
...
muni_class_AleaCats <- ptax_pins %>%
  ...
  summarize(classgroup_PC = n(),
            ...
            av = sum(av),
# eav_taxed
            current_taxable_eav = sum(taxed_av*eq2022),  ## taxable implies taxed in this code section. Need to update variable name and table references to the variable. 
eav_taxed = sum(taxed_av*eq2022),
eav_taxable = sum(taxable_eav),
...
eav_incents_inTIFs = sum(ifelse(class >=600 & class <=900 & in_tif == 1, eav, 0), na.rm = TRUE),
  ...
  mutate(
#eav_incents_inTIFs = fmv_incents_inTIFs / eq2022,
    ...
    forgone_EAV_overlap = ifelse(fmv_incents_inTIFs > 0, fmv_incents_inTIFs/loa
, 0)
  ) %>%
...
muni_forgone_eav_overlap_incents_inTIFs = sum(forgone_EAV_overlap),
 cur_munilevel_rate = mean(cur_munilevel_rate)
