...
table <- nonres_pins2022 %>% 
  ...
  select(both_ids, count, sum, pct_pins_w_inct, av, av_adjusted) %>% 
  arrange(desc(av))
...
datatable(table,
          rownames= FALSE,
          colnames = c('Project ID' = 'both_ids', 'PINs per Project' = 'count', 
                       'PINs with\nIncent Class' = 'sum',
