...
$$\mbox{Total Value = Taxed Value + Untaxable Value}$$ *where* $$\mbox{Untaxable Value = TIF Increment + Exemptions + Abatements + Reduced Taxable Value from Lower Incentive Class Assessment Ratios}$$ *where* $$\mbox{Reduced Taxable Value from Incentive Classification Levels of Assessments}$$ $$ \mbox{= (25% * EAV - ~10% * EAV)} $$
...
> CODE CHANGES July 10th 2024: No longer filter by commercial and incentive property types because it was dropping PINs that had changed property classes if they were not commercial or incentive. Not what was intended! - AWM

# Yearly Trends

## Change with Comm_Ind_PINs file

The file `comm_ind_PINs_2006-2022.csv` contains all PINs that had an incentive property class for at least 1 year. It includes all observations for a property during the years that it existed, even if it is not an incentive class property in that year.

```{r uses-comind-file}
ptax_pins <- read_csv("Output/comm_ind_PINs_2006-2022.csv") %>% 
  mutate(class = as.numeric(class)) 

table(ptax_pins$Alea_cat)
...
theme_classic() + 
labs(title = "FMV Over Time by Land Use", subtitle = "Uses com_ind_PINS... file.  Dashed line is AV")
...
labs(title = "FMV Over Time by Incentive Usage", subtitle = "Uses com_ind_PINS... file.")


```

## Yearly Change with comm_ind_PINs file

```{r}
#| label: tbl-growthcalculations5
#| tbl-cap: "Growth from 2006 to 2022. Removes PINs if they did not exist during all years in the data. Uses com_ind_PINS... file. "
...
df_1 %>% select(year, Alea_cat,  incent_prop, pins, fmv_growth, n_growth) %>% filter(year == 2022) 
...
#| tbl-cap: "Growth from 2011 to 2022."
...
df_1 %>% #select(year, Alea_cat,  incent_prop, pins, fmv_growth, n_growth) %>% 
filter(year == 2022) 
...
df_2011 <- ptax_pins %>%
  ...
  mutate(
         years_existed = n(),
       #  base_year_fmv = fmv[year == min(year)],
        # fmv_growth = fmv/base_year_fmv
         ) |>
  filter(years_existed >=11 ) %>%
  
  group_by(incent_prop, Alea_cat, year) |>
  summarize(
      group_pin_count = n(),
      group_year_fmv = sum(fmv),
...
df_2011 %>% select(year, Alea_cat, incent_prop, n_group_growth, fmv_group_growth, everything()) %>% 
filter(year == 2022) %>% select(-year)

```

## Change - Uses Muni-Class Summary File

> This is probably the wrong file to use! Does not account for when PINs were other classes before or after incentive classes! We accidently were dropping them some years if they were not commercial or industrial classes! 


```{r}
df <- read_csv("./Output/ptaxsim_muni_class_summaries_2006-2022.csv") %>%
  rename(pins_in_muni = muni_c_pins_in_muni,
         av = muni_c_av)
...
# filter to industrial and commercial, all years

### PROBLEMATIC LINE OF CODE BELOW!!! Was dropping the pins if they weren't between classes 500 and 899
ind_comm_cc <- df_all %>%
...
#   ggplot() + 
#   geom_line(aes(x=year, y=PINcount)) + theme_classic() + 
#   labs(title = "Number of PINs with Incentive Classifications")
...
ind_comm_cc %>%
  ...
  #  geom_line(aes(x=year, y = av, color = land_use),  lty = 2)+
  theme_classic() + labs(title = "FMV Over Time by Land Use", subtitle = "Uses ptaxsim_muni_class... file. DONT USE.")
...
#  geom_line(aes(x=year, y = av, color = incent_prop), lty=2)+
theme_classic() +  labs(title = "FMV Over Time by Incentive Usage", subtitle = "Uses ptaxsim_muni_class... file. DONT USE.")
...
#| tbl-cap: "Commercial and Industrial PINs in Cook County (2006 - 2022)"



table <- ptax_pins %>%
mutate(land_use = Alea_cat) %>% 
  group_by(year, land_use) %>%
  summarize(
    proptype_pincount = n(),
...
flextable(table) %>% 
  set_header_labels(year = "Year", land_use = "Land Use", proptype_fmv = 'Total', fmv_incent = "Incent. Class", fmv_nonincent = "No Incent. Class",
                    pct_incent = "% Incent. Class") %>%
  set_table_properties( layout = "autofit", width = .75)
```

```{r}
#| label: fig-cooktotals-allyears2
#| fig-cap: "Percent of PINs within each Property Type that had Incentive Property Classes"


table %>% ggplot() + geom_line(aes(x=year, y = pct_incent, group = land_use, color = land_use)) + labs(title = "Percent of PINs")
...
table <- ind_comm_cc %>%
  ...
  summarize(
    proptype_fmv = sum(fmv),
            n_incent = sum(ifelse(incent_prop == "Incentive", fmv, 0)),
            n_nonincent = sum(ifelse(incent_prop == "Non-Incentive", fmv, 0))) %>%
  mutate(pct_incent = n_incent / proptype_fmv) %>%
  arrange(desc(year)) %>%
  mutate(year = as.character(year))
```
...
table %>% ggplot() + 
geom_line(aes(x=year, y = pct_incent, group = land_use, color = land_use)) + labs(title = "Percent of Commercial and Industrial FMV with Incent. Classes")
...
df_1 <- df_all %>%
  ...
  summarize(n = sum(pins_in_muni), 
   ...
   total_final_levy = sum(muni_c_final_tax_to_dist ) ) |>

  mutate(      
   base_year_levy = total_final_levy[year == min(year)],
...
levy_growth = total_final_levy / base_year_levy
       ) |>
  filter(class != 0)

# This table is summarized at the County-Class. 
# df_1 
```

```{r}
#| label: tbl-growthcalculations
#| tbl-cap: "Growth from 2006 to 2022."
...
df_2011 %>% select(year, Alea_cat, incent_prop, fmv_growth, n_growth) %>% filter(year == 2022) 

```

```{r}
#| label: fig-growth-facetby-incentprop
#| fig-cap: "Growth from 2006 to 2022. Faceted by binary indicator for if a property had incentive classification or not. Indexed to FMV of properties during 2006."

#df_1 %>% filter(incent_prop == "Incentive") %>% ggplot() + geom_line(aes(x=year, y=fmv_growth, group = Alea_cat, color = Alea_cat))

df_1 %>% filter(!is.na(incent_prop) ) %>% 
ggplot() + geom_line(aes(x=year, y=fmv_growth, group = Alea_cat, color = Alea_cat)) + facet_wrap(~incent_prop) + labs(title= "Growth from 2006 to 2022")
...
ggplot() + geom_line(aes(x=year, y=fmv_growth, group = Alea_cat, color = Alea_cat)) + facet_wrap(~incent_prop) + labs(title= "Growth from 2011 to 2022")
...
df_1 %>% 
ggplot() + geom_line(aes(x=year, y=fmv_growth, group = incent_prop, color = incent_prop)) + theme_classic() + facet_wrap(~Alea_cat)
...
```{r}
df_1 %>% filter(Alea_cat == "Commercial" | Alea_cat == "Industrial") %>% ggplot() + geom_line(aes(x=year, y=n_growth, group = incent_prop, color = incent_prop)) + labs( title = "Commercial v Industrial Growth") + facet_wrap(~Alea_cat)
...
```{r}
df_2011 %>% 
filter(Alea_cat == "Commercial" | Alea_cat == "Industrial") %>% 
ggplot() + geom_line(aes(x=year, y=n_growth, group = incent_prop, color = incent_prop)) + scale_y_continuous(limits = c(0.8,2.5)) + labs( title = "Commercial v Industrial Growth") + facet_wrap(~Alea_cat)
