...
$$
$$
\mbox{Untaxable Value = TIF Increment + Exemptions + Abatements + Reduced Taxable Value from lower levels of assessments due to incentive classification}
$$
$$
\mbox{Reduced Taxable Value from Incentive Classification Decreased Levels of Assessments = (25% * EAV - ~10% * EAV)}
$$
...
$$
$$
\mbox{Fair Market Value} = {\frac{\mbox{final_tax_to_dist + final_tax_to_tif}}{\mbox{tax_code_rate}} + \mbox{Exemptions + Abatements}}
$$
```{r}
...
table_cook %>% 
  select(cty_fmv, cty_fmv_inTIF, cty_fmv_tif_increment, 
         cty_fmv_incentive, cty_fmv_incents_inTIFs) %>%
  flextable() %>% 
  set_header_labels(
                    cty_fmv = 'Total FMV', 
...
ungroup() %>%
  ...
  summarise(
    ...
    cur_munilevel_rate = mean(cur_munilevel_rate), # Was same value for each municpality. Took mean to keep variable in dataframe. (Could have included it in group() command) 
