...
table_cook %>% 
  ...
  select(cty_PC_comandind, cty_PC_incent_inTIF, cty_fmv_comandind, cty_pct_fmv_incentinTIF,
         cty_fmv, cty_pct_PC_both_incent, cty_pct_fmv_both_incent, cty_pct_PC_both_incent_inTIF) %>%
  flextable() %>% 
  FitFlextableToPage()
```
...
table_cook %>% 
  ...
  set_header_labels(
                    ...
                    cty_fmv_incents_inTIF = 'FMV with Incent. Class. in TIFs'
)%>%   FitFlextableToPage()
...
df %>%
  group_by(year) %>%
  summarize(n = n()) %>% ggplot() + geom_line(aes(x=year, y=n)) + theme_classic()
...
table %>% ggplot() + geom_line(aes(x=year, y = pct_incent, group = land_use, color = land_use))
...
table %>% ggplot() + 
geom_line(aes(x=year, y = pct_incent, group = land_use, color = land_use))
...
#df_1 %>% filter(Alea_cat == "Industrial") %>% ggplot() + geom_line(aes(x=year, y=n_growth, group = incent_prop, color = incent_prop)) + labs(title = "Industrial Growth")
```
