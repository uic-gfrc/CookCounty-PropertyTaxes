...
table_muni_mc %>% 
  select(-c(starts_with("muni"), muni_avg_C2_bill_noexe, muni_avg_C2_bill_withexe )) %>%
  ...
  datatable(rownames = FALSE,
      ...
      # 'Muni FMV' = 'muni_fmv'
     # 'Avg C2 Bills - Did not Claim Exe' = 'muni_avg_C2_bill_noexe',
      # 'Avg C2 Bill - Did Claim Exe' = 'muni_avg_C2_bill_withexe'

   )) %>%
  formatPercentage( c(  'Value not Taxed (%)', 'FMV w/ Incent. Class. (%)'  ), digits = 1) %>%
  formatCurrency(c('Class FMV', 'Taxed FMV', 'Value not Taxable for Levy', 'TIF Increment FMV', 'FMV with Incent.Class.', 'FMV with Incent. Class. in TIFs', 'FMV in TIFs'
#,'Avg C2 Bills - Did not Claim Exe',  'Avg C2 Bill - Did Claim Exe' 
), digits = 0)
...
```{r eval=FALSE}
table_muni_mc_percentages <- table_muni_mc %>%
...
```{r}
incentive_majorclasses <- c("6", "7-small", "7-big", "8A", "8B")
...
tablelist <- list(
"Muni Level Sums" = munilevel,
"Class Summaries" = muni_mc_table, "Class Percentages" = table_muni_mc_percentages, 
