...
ptax_pins <- ptax_pins %>% 
  ...
  mutate(
    ...
    # taxable AV = equalized assessed value net TIF increments, gross exemptions. Used for calculating untaxable value
...
ungroup() %>%
  ...
  mutate(
   ...
   # - muni_forgone_eav_overlap_incents_inTIFs
 #  ,
...
tbl3 <- alt_rates %>%
  select(clean_name, change_noExe, change_noInc, change_noC6, change_noC7, change_noC8, change_noAbate, change_noTIFs, 
         change_neither, change_max, change_vacants, change_error_dif) %>%
...
# "Class Summaries" = muni_class_AleaCats, 
"Tax Rate Hypotheticals" = alt_rates
)
