...
df_2011 %>% select(year, land_use, incent_prop, fmv_growth, n_growth) %>% filter(year == 2022) 
...
ggplot() + geom_line(aes(x=year, y=fmv_growth, group = land_use, color = land_use)) + facet_wrap(~incent_prop) + labs(title= "Growth from 2006 to 2022")
...
ggplot() + geom_line(aes(x=year, y=n_growth, group = incent_prop, color = incent_prop)) + scale_y_continuous(limits = c(0.8,2.5)) + labs( title = "Commercial v Industrial Growth") + facet_wrap(~land_use)
