...
$$\mbox{Total Value = Taxed Value + Untaxable Value}$$ *where* $$\mbox{Untaxable Value = TIF Increment + Exemptions + Abatements    
+ Reduced Taxable Value from Lower Incentive Class Assessment Ratios}$$

*where*

$$\mbox{Reduced Taxable Value from Incentive Classification Levels of Assessments = 25% X EAV - ~10% X EAV}$$
...
df_2006 %>% filter(!is.na(incent_prop) ) %>% 
...
df_2011_v2 %>%  
ggplot() + geom_line(aes(x=year, y=fmv_growth, group = landuse_change, color = landuse_change)) + facet_wrap(~incent_change) + labs(title= "Growth from 2011 to 2022")
...
ggplot() + geom_line(aes(x=year, y=fmv_growth, group = land_use, color = land_use)) + facet_wrap(~incent_prop) + labs(title= "Growth from 2011 to 2022")
...
ggplot() + geom_line(aes(x=year, y=n_growth, group = incent_prop, color = incent_prop)) + scale_y_continuous(limits = c(0.8,2.5)) + labs( title = "Commercial v Industrial PIN Count Growth") + facet_wrap(~land_use)
...
ptax_pins <- read_csv("./Output/comm_ind_2011-2022_balanced.csv")
...
df_2011 %>% filter(!is.na(incent_prop)) %>% 
ggplot() + geom_line(aes(x=year, y=fmv_group_growth, group = land_use, color = land_use)) + facet_wrap(~incent_prop) + labs(title= "Growth from 2011 to 2022")
...
ptax_pins <- read_csv("./Output/comm_ind_PINs_2006to2022_existallyears.csv")
...
labs(title= "Growth from 2006 to 2022", subtitle = "Change in Incentive Use by Land Use Change", caption = "Indexed to 2006 FMV", x = "", y = "FMV Growth from 2006") 
```
