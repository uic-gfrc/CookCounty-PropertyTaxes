...
ptax_pins <- ptax_pins %>% 
  mutate( both_ids = project_id,
          ...
          both_ids = ifelse(is.na(both_ids) & (class > 299 & class < 900), pin, both_ids))
...
### JUST DOUBLE CHECKING THAT WE KNOW CLASSES 400 AND STUFF GET TAXED AT THE EMPTY LAND VALUE?
...
ptax_pins <- ptax_pins %>% 
  mutate(class_group = str_sub(class, 1,1),
         class_group = case_when(
          ...
          ## MVH: I THINK THIS IS TOO MUCH DATA FOR THEM ##
  ...
  mutate(
    ...
    taxed_eav = final_tax_to_dist / tax_code_rate*100, # MVH: simplified formula
    ...
    taxable_av = (final_tax_to_dist / tax_code_rate *100 + all_exemptions + abatements)/ eq2022, # MVH: SAME
    ...
    untaxable_value_eav = all_exemptions + abatements + 
      ...
      (final_tax_to_tif /  tax_code_rate*100 +
         ## difference between 25% and reduced level of assessment for incentive class properties 
         ifelse(incent_prop=="Incentive", 
               ((taxable_av - taxed_av)*eq2022), 0)
       ) ,
    untaxable_value_eav = ifelse(class == 239, equalized_av-taxed_eav, untaxable_value_eav), 
  ...
  select(tax_code, class, pin, taxed_fmv, # taxable_fmv, 
...
> What is class 239? That is the class that is consistently not equalling 0 in the test above.

-   239 is some form of subsidized farm land. Using the assessment ratio to back out the value does not work.\
-   Could include something like: `untaxable_value = ifelse(class == 239, equalized_av-taxed_value, untaxable_value)` - COMPLETED
...
stopifnot(
 # (ptax_pins$equalized_av - ptax_pins$eav <= 1000),
...
## I don't think this variable means anything... delete?
...
- Ignore stats for these Municipalities. Simple rounding errors may cause bizarre results for rate changes & other calculations.
...
munilevel <- ptax_pins %>% 
  ...
  summarize(
    ...
    muni_PC_withincents = sum(ifelse(class >=600 & class <=900, 1, 0), na.rm = TRUE),
    muni_PC_incents_inTIFs = sum(ifelse(class >=600 & class <=900 & in_tif == 1, 1, 0), na.rm = TRUE),
    ...
    muni_fmv_abates_inTIF = sum(ifelse(class >=600 & class <=900 & in_tif == 1 & abatements >0 , fmv, 0), na.rm = TRUE),
...
table_mc_percentages <- table_muni_mc %>%
  mutate(pct_PC_proptype = proptype_pincount/muni_PC_total,
         ...
         pct_fmv_taxed = fmv_taxed / muni_fmv_taxable,
...
table_proptype <- ptax_pins %>% 
  ...
  mutate(pct_fmv_untaxable = untaxable_value / fmv) %>%
   select(Alea_cat, proptype_pincount, fmv, fmv_taxed, untaxable_value, pct_fmv_untaxable, fmv_tif_increment, fmv_inTIF, fmv_incentive, incents_inTIFs, everything() )


flextable(table_proptype) %>% 
  set_header_labels(Alea_cat = "Property Type", proptype_pincount = 'PINs',# projects = "Project IDs", 
                    fmv = 'Total FMV', fmv_taxed = 'Taxed FMV', untaxable_value = 'Value not Taxable for Levy', fmv_incentive = "FMV with Incent.Class.", fmv_inTIF = 'FMV in TIFs', fmv_tif_increment = 'TIF Increment FMV' , pct_fmv_untaxable = 'Value not Taxed (%)', incentive_fmv = 'FMV with Incent. Classification', incents_inTIFs = 'FMV with Incent. Class. in TIFs' )
...
table <- nonres_pins2022 %>% 
  group_by(Alea_cat) %>%
      ...
      summarize(proptype_pincount = n(),
          #  projects = n_distinct(both_ids), # mostly for industrial and commercial properties
            ...
            taxed_fmv = sum(taxed_fmv, na.rm=TRUE),
  ...
  mutate(pct_fmv_untaxable = untaxable_value / fmv,
pct_fmv_taxed = taxed_fmv / fmv,
         pct_incents = fmv_incentive / fmv) %>%
   select(Alea_cat, proptype_pincount, fmv, taxed_fmv, taxable_fmv, untaxable_value, pct_fmv_untaxable, fmv_tif_increment, fmv_inTIF, fmv_incentive, incents_inTIFs, everything() )


flextable(table) %>% 
  set_header_labels(Alea_cat = "Property Type", proptype_pincount = 'PINs',# projects = "Project IDs", 
                    fmv = 'Total FMV', taxed_fmv = 'Taxed FMV', untaxable_value = 'Value not Taxable for Levy', fmv_incentive = "FMV with Incent.Class.", fmv_inTIF = 'FMV in TIFs', fmv_tif_increment = 'TIF Increment FMV' , pct_fmv_untaxable = 'Value not Taxed', incentive_fmv = 'FMV with Incent. Classification', incents_inTIFs = 'FMV with Incent. Class. in TIFs', pct_incents = 'FMV with Incent. Class' )
...
datatable(table1,
          rownames= FALSE,
          colnames = c('Municipality' = 'clean_name',   'Property Type' = 'Alea_cat', 'PIN Count' = 'proptype_pincount', 'Project Count' = 'projects', 'Currently Taxed AV' = 'av', 'FMV' = 'fmv',
       'FMV Untaxed (%)'  = 'pct_fmv_untaxable', 'FMV with Incent. Class. (%)' =  'pct_incents', 'FMV in TIF (%)' = 'pct_inTIF'),
       #   extensions = 'RowGroup',
          options = list(rowGroup = list(dataSrc = 0))) %>%
  formatCurrency(c('Currently Taxed AV', 'FMV'), digits = 0) %>%
...
muni_class2022 <- read_csv("./Output/ptaxsim_muni_class_summaries_2006-2022.csv") %>% 
  ...
  mutate(
    ...
    # taxing district revenue = taxable eav * tax rate so rearrange the formula:
    taxed_eav = (final_tax_to_dist/(cur_comp_muni_rate/100)),
    taxed_av =  taxed_eav / eq2022, # current value that taxing agencies can tax for their levies
    
    # taxable AV = equalized assessed value net TIF increments, gross exemptions
    taxable_av = ((final_tax_to_dist/(cur_comp_muni_rate/100)) + (all_exemptions + exe_abate) ) / eq2022,
    ...
    untaxable_value_eav = all_exemptions + exe_abate + (final_tax_to_tif/(cur_comp_muni_rate/100) +                                                  ifelse(incent_prop=="Incentive", 
                                                                                                                                                            ((taxable_av - taxed_av)*eq2022), 0)  ) ,
    
    untaxable_value_av = untaxable_value_eav / eq2022,
    untaxable_value_fmv = untaxable_value_av / loa,
    
    exempt_eav= (all_exemptions + exe_abate),
    exempt_fmv = exempt_eav / eq2022 / loa, 
    
    fmv_inTIF = ifelse(final_tax_to_tif >0, av/loa, 0),
    fmv_tif_increment = ifelse(final_tax_to_tif > 0, ((final_tax_to_tif / (cur_comp_muni_rate/100)) / eq2022 ) / loa, 0),
    fmv = av/loa
  ) %>%
  
  select(clean_name, class, taxed_fmv, # taxable_fmv, 
         untaxable_value_fmv, fmv_inTIF, fmv_tif_increment, fmv, final_tax_to_dist, final_tax_to_tif, cur_comp_muni_rate, eav, equalized_av, av, everything())
...
muni_class_AleaCats <- muni_class2022 %>%
  ...
  summarize(classgroup_PC = n(),
            ...
            levy = sum(final_tax_to_dist),
            current_rate_avg = mean(cur_munilevel_rate),
            
            avg_C2_bill_noexe = mean(ifelse(between(class,200,300) & all_exemptions == 0, (final_tax_to_dist+ final_tax_to_tif), NA), na.rm=TRUE),
            avg_C2_bill_withexe = mean(ifelse(between(class,200,300) & all_exemptions > 0, (final_tax_to_dist+ final_tax_to_tif), NA), na.rm=TRUE),
            av_taxed = sum(taxed_av, na.rm = TRUE),
            untaxable_value_av = sum(untaxable_value_av, na.rm=TRUE),
            av = sum(av),
            current_taxable_eav = sum(av_taxed*eq2022), 
            pins_in_muni = sum(pins_in_muni, na.rm=TRUE),
...
## How tax rates were originally calculated:
...
```{r}
muni_class_AleaCats <- ptax_pins %>%
  left_join(MuniLevel) %>%
  group_by(clean_name, 
           class_group, incent_prop, 
           Alea_cat
  ) %>%
  summarize(classgroup_PC = n(),
            fmv_incentive = sum(ifelse(class >=600 & class <=900, fmv, 0), na.rm = TRUE),
            ...
            untaxable_value_av = sum(untaxable_value_av, na.rm=TRUE),
            av = sum(av),
            current_taxable_eav = sum(taxed_av*eq2022), 
            pins_in_class = n(),
            all_exemptions = sum(all_exemptions),
            abatements = sum(exe_abate),
  ...
  mutate(
    ...
    new_TEAV_noIncents = ifelse(incent_prop ==  "Incentive", (current_taxable_eav/loa)*0.25, current_taxable_eav),
    new_TEAV_noC6 = ifelse(class_group == "6", (current_taxable_eav/loa)*0.25 , current_taxable_eav),
    new_TEAV_noC7 = ifelse(class_group %in% c("7-big", "7-small"), (current_taxable_eav/loa)*0.25, current_taxable_eav),
    new_TEAV_noC8 = ifelse(class_group %in% c("8A", "8B"), (current_taxable_eav/loa)*0.25, current_taxable_eav),
    ...
    # amount of EAV from taxing an additional 15% of the AV if incentive properties didn't exist
    forgone_EAV_incent = ifelse(incent_prop == "Incentive", 
                                new_TEAV_noIncents - (current_taxable_eav), 0),
...
alt_rates <- alt_rates %>% 
  mutate(change_noExe =  muni_rate_noExe - muni_current_rate,
...
> TAX RATE CALCULATIONS DO NOT SEEM RIGHT

-   potentially from TIF calculation. when calculating the forgone EAV to TIFs for the new tax rate, formula had used final_tax_to_tif instead of the calculated TIF increment variable.
...
datatable(tbl1,
          ...
          colnames = c('Municipality' = 'clean_name', 
                       'PIN Count' = 'muni_PC', 'Current Composite Rate' = 'muni_current_rate', 
