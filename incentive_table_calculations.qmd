...
#| fig-cap: "Percent of FMV Incentivized"
...
#| fig-cap: "Commercial and Industrial FMV in Cook County (2006 - 2022)"
...
```
```{r}
#df_1 %>% filter(Alea_cat == "Commercial") %>% ggplot() + geom_line(aes(x=year, y=fmv_growth, group = incent_prop, color = incent_prop))
...
```{r}
df_1 %>% filter(Alea_cat == "Commercial") %>% ggplot() + geom_line(aes(x=year, y=n_growth, group = incent_prop, color = incent_prop))

df_1 %>% filter(Alea_cat == "Industrial") %>% ggplot() + geom_line(aes(x=year, y=n_growth, group = incent_prop, color = incent_prop))
```
