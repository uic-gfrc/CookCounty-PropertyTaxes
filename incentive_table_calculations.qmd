...
#| tbl-cap: "Number of PINs associated with an apellant each tax year. Excludes property classes 0-399. Data from Board of Review Appeals dataset on Cook Data Portal. "
...
table_cook <- ptax_pins %>%
  mutate(EAV_untaxable_value = case_when(
  class >= 200 & class <= 299 ~ all_exemptions,
  class > 299 ~ abatements,
TRUE ~ 0  
...
table_MC <- ptax_pins %>%
  ...
  summarize(proptype_pincount = n(),
            projects = n_distinct(keypin), # mostly for industrial and commercial properties
            fmv_w_incent_class = sum(ifelse(class >=600 & class <=900, fmv, 0), na.rm = TRUE),
...
table_proptype <- ptax_pins %>% 
 ...
 summarize(proptype_pincount = n(),
            projects = n_distinct(keypin), # mostly for industrial and commercial properties
            incentive_fmv = sum(ifelse(class >=600 & class <=900, fmv, 0), na.rm = TRUE),
...
table1 <- nonres_pins2022 %>% 
  ...
  summarize(pin_count = n(),
         project_count = n_distinct(keypin), 
                     av_adjusted=sum(ifelse(between(class, 600, 899), av*2.5, av), na.rm=TRUE),
...
table2 <- nonres_pins2022 %>% 
  ...
  summarize(pin_count = n(),
          project_count = n_distinct(keypin), 
          av_incent = sum(ifelse(between(class, 600, 899), av*2.5, 0), na.rm=TRUE),
          av_adjusted = sum(ifelse(between(class, 600, 899), av*2.5, av), na.rm=TRUE),
