...
ptax_pins %>% filter(year == 2022) %>% reframe(count = n(), .by = incentive_years) %>% arrange(incentive_years) %>% filter(incentive_years > 0) %>% ggplot() + geom_col(aes(x=incentive_years, y = count)) + scale_x_continuous(limits = c(0,18) )
...
df_1 %>% select(year, land_use,  incent_prop, pins, fmv_growth, n_growth) %>% filter(year == 2022) 
...
df_2011 <- ptax_pins %>%
  ...
  group_by(incent_prop, land_use, year) |>
  summarize(
    group_pin_count = n(),
    group_year_fmv = sum(fmv),
...
#| fig-cap: "Percent of PINs within each Land Use that had Incentive Property Classes"
...
#| tbl-cap: "Growth from 2006 to 2022. Uses muni_class summary file. DONT USE. Includes all commercial and industrial PINs even if they did not exist for all years."
...
#| tbl-cap: "Growth from 2011 to 2022. Uses muni_class summary file. DONT USE. Includes all commercial and industrial PINs even if they did not exist for all years."
...
#| fig-cap: "Growth from 2006 to 2022. Faceted by binary indicator for if a property had incentive classification or not. Indexed to FMV of properties during 2006. DONT USE."
...
#| fig-cap: "**Growth from 2011 to 2022.** Faceted by binary indicator for if a property had incentive classification or not. Indexed to fair market value during 2011. DONT USE"
...
#| fig-cap: "Growth from 2006 to 2022. Faceted by land use category. DONT USE"
...
```{r}
df_1 %>% filter(Alea_cat == "Commercial" | Alea_cat == "Industrial") %>% ggplot() + geom_line(aes(x=year, y=n_growth, group = incent_prop, color = incent_prop)) + 
labs( title = "Commercial v Industrial Growth") + facet_wrap(~Alea_cat)
