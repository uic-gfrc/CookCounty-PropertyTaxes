...
bor_pins <- bor %>% 
  ...
  summarize(pin = first(pin),              # grabs first occurance of unique PIN
...
table_cook %>% 
  select(cty_fmv_comandind, #cty_fmv, 
         ...
         cty_pct_PC_both_incent, cty_PC_comandind, cty_PC_incent_inTIF, cty_pct_PC_both_incent_inTIF) %>%
  flextable() %>% 
    set_header_labels(
                          cty_fmv_comandind = 'Com. & Ind. FMV',
                        #  cty_fmv = 'Total FMV in Cook',
...
table_cook %>% 
  select(cty_fmv, cty_fmv_inTIF, cty_fmv_tif_increment, 
         cty_fmv_incentive, cty_fmv_incent_inTIF) %>%
  flextable() %>% 
  set_header_labels(
                    cty_fmv = 'Total FMV', 
...
filter(incent_prop == "Incentive") %>%
  group_by(year) %>%
  summarize(PINcount = sum(pins_in_muni)) %>% ggplot() + geom_line(aes(x=year, y=PINcount)) + theme_classic() + labs(title = "Number of PINs with Incentive Classifications")
...
```{r}
#| label: fig-cooktotals-allyears-fmv
