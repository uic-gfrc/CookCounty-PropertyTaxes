...
df_2011_incentchange %>% select(year, landuse_change, fmv_group_growth, base_year_fmv, group_year_fmv, everything()) %>% 
filter(year == 2022) %>% select(-year)
...
comm_ind_PINs_2011to2022_existallyears
...
df_2006 %>% select(year, land_use, incent_prop, n_group_growth, fmv_group_growth, everything()) %>% 
filter(year == 2022) %>% select(-year)
