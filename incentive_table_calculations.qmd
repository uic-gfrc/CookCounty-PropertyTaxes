...
MVH to-do suggestions: 1. Rename "Alea_Cat" variable (at least to Nimbus) 2. Equalization factors --\> create single variables 3. Conform to 'between' command
...
::: callout-note
MVH: I DON'T KNOW WHAT THAT MEANS. Maybe case studies?
:::
...
\<span style="color:green;"\>MVH: I THINK WE CAN REWRITE THIS SECTION</span>
...
write_csv(table_muni_percentages, "Output/incentive_table_calcs_munipercentages.csv")
...
#eav_taxed2 = sum(final_tax_to_dist / (tax_code_rate/100), na.rm=TRUE),
...
eav_incents_inTIFs = sum(ifelse(class >=600 & class <=900 & in_tif == 1, eav, 0), na.rm = TRUE),
  ...
  mutate(
#eav_taxed_dif = eav_taxed-eav_taxed2,
#eav_incents_inTIFs = fmv_incents_inTIFs / eq2022,
   ...
   # new_av = ifelse(incent_prop == "Incentive", (av/loa)*0.25, av),   # for incentive properties
    ...
    forgone_EAV_overlap = ifelse(fmv_incents_inTIFs > 0, fmv_incents_inTIFs*loa*eq2022, 0)
  ) %>%
...
muni_eav_max = sum(eav_max)
  ...
  mutate(
    ...
    muni_TEAV_max = muni_current_TEAV + muni_exemptions + muni_abatements + muni_forgoneTIF + muni_forgone_EAV_incent - muni_forgone_eav_overlap_incents_inTIFs,
...
write_csv(alt_rates, "Output/incentive_table_calculations.csv")
```
...
tablelist <- list("Muni Level Sums" = munilevel, "Class Summaries" = muni_class_AleaCats, 
"Class Summaries" = table_muni_mc, "Class Percentages" = table_mc_percentages, 
"Muni Percentages" = table_muni_percentages, "Tax Rate Hypotheticals" = alt_rates
)
