...
comm_ind <- comm_ind |>
  ...
  ungroup() |>
  group_by(year) |>
         mutate(base_year_fmv_2011_w = DescTools::Winsorize(base_year_fmv_2011,
...
gain_lose_pins %>% group_by(year, type) %>% summarize(n = n()) %>%
  pivot_wider(names_from = "type", values_from = n)
```

```{r convert_panel_data}

comm_ind_panel <- panel(comm_ind_raw, panel.id = ~pin + year)
...
comm_ind_raw  <- panel(comm_ind_raw, panel.id = c("pin", "year"))
...
# comm_ind_raw <- comm_ind_raw |>
#   mutate(incent_prop = as.numeric(incent_prop)) |>
#   mutate(incent_prop = incent_prop - 1)
...
logit_med_list <- list(
    ...
    "Landuse-Year Interact." = feglm(incent_prop ~ land_use + land_use*year | pin + clean_name + year,
      data = comm_ind_raw,
      family = binomial(link = "logit"),
      vcov = "threeway",
      panel.id = ~ pin + year),
    
    "Reassess?" = feglm(incent_prop ~ land_use + reassessed_year | pin + clean_name + year,
      data = comm_ind_raw,
      family = binomial(link = "logit"),
      vcov = "threeway",
      panel.id = ~ pin + year),
