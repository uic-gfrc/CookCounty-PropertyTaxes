...
# write_csv(pin_data, "3_Exemption_Details_output-all_cook_pin_exemptions_2021_actual.csv")
...
# write_csv(exemptions_inCook_perTC, "3_Exemption_Details_output-Cook_Exemps_byClassandTaxcode.csv")
```
...
exemptions_by_class_per_TC <- read_csv("3_Exemption_Details_output-Cook_Exemps_byClassandTaxcode.csv") %>%
  ...
  mutate(tax_code_num = as.character(tax_code_num),
         class_2dig = str_sub(class_code, 1,1)) %>%
  ...
  mutate(exempt_EAV = (exe_homeowner + exe_senior + exe_freeze + exe_longtime_homeowner + 
        ...
        ResidentialProps = ifelse(class_2dig %in% c("2", "3", "9"), "Residential", "Non-Residential"),
        PropType = case_when(
...
tif_table <-exemptions_by_class_per_TC %>% 
  group_by(clean_name, agency_name, in_TIF, PropType) %>%
  summarize(av = sum(av),
            eav = sum(eav)) %>% 
  ungroup() %>%
  left_join(muni_totals) %>% 
  pivot_wider(id_cols = c(clean_name, agency_name, muni_av, muni_eav), names_from = c(in_TIF,PropType), values_from = eav)
tif_table

tif_table %>% select(clean_name:muni_eav, `1_Non-Residential`, `0_Non-Residential`) %>% 
...
tif_table %>% mutate(Residential_inTIF = `1_Rental`+`1_Owner Occupied`+`1_Other Residential`,
                     Residential_outsideTIF = `0_Rental`+`0_Owner Occupied`+`0_Other Residential`,
...
```{r}
pivottable <- table %>% 
  pivot_wider(id_cols = c(clean_name, agency_name, muni_av, muni_eav), names_from = PropType, values_from = c(eav)) %>%   
...
pivot_table <- pivottable %>%   
  mutate(perc_rental = `Rental`/muni_eav,
         ...
         perc_nonres = `Non-Residential`/muni_eav,
         perc_residential = 1-perc_nonres) %>% 
  mutate_all( ~coalesce(.,0))

pivot_table %>% arrange(desc(perc_residential))
pivot_table %>% arrange(desc(perc_rental))

write_csv(pivot_table, "3_exemption_details_output-residential_percent.csv")
...
grouped_exemptions <- exemptions_by_class_per_TC %>% 
  ...
  group_by(clean_name, ResidentialProps, PropType, agency_name) %>%
...
# write_csv(exemptions_per_ClassTaxcode, "3_Exemptions_Details_output-MajorClassTaxcodeExemps.csv")
