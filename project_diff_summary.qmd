...
```{r}
#| tbl-cap: "Commercial or industrial PINs that existed in 2022 in PTAXSIM"
...
```{r}
#| tbl-cap: "Commercial or industrial PINs that existed in 2022 in PTAXSIM and are in manually coded project crosswalk file `projects_checked_MAINFILE.xlsx`"

exists_in2022 |> filter(pin %in% keypins$pin) |> count()
```
```{r}
exists_in2022 |> select(pin) |> 
  left_join(keypins) |> 
  group_by(main_keypin) |>
  summarize(n = n(),
            n_incent = sum(incent_prop == "Incentive") )|> 
  ungroup() |>
  summarize(avg_pins_perproj = mean(n),
            project_count = n(),
            incent_pins = sum(n_incent, na.rm=TRUE))
...
```{r}
#| tbl-cap: "Commercial or industrial PINs in valuation worksheets that existed in 2022"
...
exists_in2022 |> select(pin) |> left_join(worksheet_keypins) |> left_join(keypins) |>
  group_by(keypin) |> 
  summarize(n = n(),
                        n_incent = sum(incent_prop == "Incentive") )|> 
  ungroup() |>
...
# 4. PIN-level join: comm_ind x parcel_comparison
  ...
  keypin_info_summary <- pin_info |>
    ...
    summarize(
      ...
      fmv            = round(sum(fmv,    na.rm = TRUE)),
      incentive_pins = round(sum(incent, na.rm = TRUE)),
      exempt         = sum(exempt, na.rm = TRUE),
    ) |>
    arrange(desc(incentive_pins), desc(n)) |>
   # ungroup() |>
    mutate(avg_pin = round(mean(n), digits = 2),
           median_pincount= median(n), 
      avg_pin_perproj = round(mean(n[n > 1 ]), digits = 2),
           median_pin_perproj = round(median(n[n > 1 ]), digits = 2))
...
muni_data$keypin_info_summary|> DT::datatable(rownames = FALSE)
muni_data$plot

#muni_data$parcel_comparison
muni_data$parcel_keypin_summary|> DT::datatable(rownames = FALSE)
#muni_data$pin_info
muni_data$class_summary |> DT::datatable(rownames = FALSE)

muni_data$parcel_comparison |> as.data.frame() |>  left_join(muni_data$pin_info) |> distinct(pin) |> count()
...
parcel_comparison |>
 ...
 # ungroup() |>
  mutate(avg_pin = round(mean(n), digits = 2),
         median_pincount = median(n),
    avg_pin_perproj = round(mean(n[n > 1]), digits = 2),
         mean_pin_perproj = median(n[n > 1 ])) |> 
  DT::datatable(rownames = FALSE)

parcel_comparison |> as.data.frame() |>  left_join(muni_data$pin_info) |> distinct(pin) |> count()
```
...
#     arrange(desc(incentive_pins), desc(n)) |>
#     ungroup() |>
#     mutate(avg_pin_perproj = round(mean(n[n>1]), digits = 2),
#            median_pin_perproj = median(n[n>1])
#   )
#   
#   # Return everything you might want to look at
#   list(
...
# join actual pins to spatial data based on parcel identifier
parcel_comparison <- left_join(parcels, worksheet_keypins, by = "pin10") %>%
  select(pin10, keypin, everything()) |> 
  mutate(keypin = ifelse(is.na(keypin), pin, keypin))
