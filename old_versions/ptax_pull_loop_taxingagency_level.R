...
#  muni_tax_codes<- dbGetQuery(ptaxsim_db_conn, glue_sql(query, .con = ptaxsim_db_conn))
 ...
 # tc_muninames <- tax_codes %>%
  ...
  agency_summary2 <- taxbills %>%
    ...
    mutate(
      taxed_eav = final_tax_to_dist / agency_tax_rate,    # backed out amount billed, compare to agency_total_eav
    ...
    arrange(fmv) |>
    summarize(
      median_fmv_c2 = median(ifelse(c2_prop == 1, fmv, NA), na.rm = TRUE),
     # min_fmv_c2 = min(ifelse(c2_prop == 1, fmv, NA), na.rm = TRUE),
     # quant25_c2_fmv = round(quantile(ifelse(c2_prop == 1, fmv, NA), probs = q[1])),
      mean_fmv_c2 = mean(ifelse(c2_prop == 1, fmv, NA), na.rm = TRUE),
     # quant75_c2_fmv = round(quantile(ifelse(c2_prop == 1, fmv, NA), probs = q[3])),
      fmv_sd_c2 = sd(ifelse(c2_prop == 1, fmv, NA), na.rm=T),

      mean_c2fmv_taxed = mean(ifelse(c2_prop == 1, taxed_fmv, NA), na.rm = TRUE),
      median_c2fmv_taxed = median(ifelse(c2_prop == 1, taxed_fmv, NA), na.rm=T),
      #min_c2fmv_taxed = min(ifelse(c2_prop == 1, taxed_fmv, NA)),
      #quant25_c2_fmv_taxed = round(quantile(ifelse(c2_prop == 1, taxed_fmv, NA), probs = q[1])),
      #quant75_c2_fmv_taxed = round(quantile(ifelse(c2_prop == 1, taxed_fmv, NA), probs = q[3])),
      fmv_taxed_c2_sd = sd(ifelse(c2_prop == 1, taxed_fmv, NA), na.rm=T),
