...
#write_json(muni_shp, "muni_shp.json")
...
`MuniLevy` comes from the `final_tax_to_dist` is from tax_bill() output (and ultimately comes from tif_distrib table). Amount that was billed to all pins within a municipality (based on tax code level sums that are taxed by each Municipality). the `tif_distrib` table had tax_code level EAV values that were POST exemptions.

`district_rev_collected` came from exemptions data from the lookup_pin() output.
...
The tax rate shown is the composite tax rate for the Municipality. The composite tax rate is the aggregate of the tax rate for each taxing agency X the EAV within the taxing jurisdiction. This was calculated at a tax code level first and then added together for all taxcodes taxed by a municipal taxing agency (such as the Village of \_\_\_\_).
