...
# muni_shp %>% st_crs() #  WGS 84

#muni_shp <- st_transform(muni_shp, crs = 26916)
...
15 cross into other counties. All 15 that cross county lines do use incentive classifications within Cook County.
...
![](images/clipboard-3675640920.png)

![](images/clipboard-2707126706.png)

![](images/clipboard-2892269521.png)

![](images/clipboard-2484422157.png)
...
## Newest Addition for April 30th Presentation --------------------
class_8_df_outofCook %>% filter(incent_prop == "Incentive") %>%
              group_by(year) %>%
              summarise(incent_perc = sum(FMV/year_ind_comm_FMV))
...
> Change scale of combined incentive class map. Change color too so it looks different than the one that splits them apart. (maybe combining the scale will do that by darkening areas with different incentive types.)

![](images/clipboard-2197304119.png)


### FMV Out of Municipality Total FMV
...
# Slide 9: Impact on Composite Tax Rate
...
muni_class_AleaCats <- muni_class2022 %>%
  ...
  mutate(
    ...
    forgone_TIF_EAV = final_tax_to_tif / cur_munilevel_rate    # revenue that went to TIF comes from tax rate  X the EAV above the frozen EAV amount (aka the TIF increment) 
    )     
...
tbl2 <- alt_rates %>% 
  select(clean_name, change_noExe, change_noInc, change_neither, change_max, change_vacants) %>%
  distinct() %>%
...
```{r}
#| label: fig-incent-and-exempt-ratechange-map-samescales
#| column: page 
#| layout-ncol: 2
#| fig-subcap:
...
# limits = c(0,15),
 ...
 na.value="gray",
                    n =4,
                       name = "Change in Composite Tax Rate\n(Percentage Points)",)+
   geom_sf_pattern(data = no_incentives, aes(geometry = geometry), pattern = 'crosshatch', pattern_spacing = 0.015,pattern_scale = 0.1, fill = "white", alpha = .5, color = 'gray40')
...
```{r}
#| label: fig-incent-and-exempt-ratechange-map-difscales
#| layout-ncol: 2
#| column: page 
#| fig-subcap:
#|      - "Change in composite tax rate from assessment ratio for incentive properties set at 25% instead of 10%"
...
joined <- muni_class_AleaCats %>%   
  left_join(MuniLevel) %>%
  select(-cur_munilevel_rate) %>% 
  left_join(tbl1, by = "clean_name")
...
burden <- joined %>% 
  ...
  mutate(
    ...
    vacant_burden = (new_TEAV_vacant_noIncents * (muni_rate_vacant/100) ) / munilevy
      ) %>%
     mutate(change_noExe = cur_munilevel_rate - muni_rate_noExe,
         change_noInc = cur_munilevel_rate - muni_rate_noInc,
         change_neither = cur_munilevel_rate - muni_rate_neither,
         change_max_decrease = cur_munilevel_rate - muni_rate_lowest,
         change_max_increase = cur_munilevel_rate - muni_rate_vacant
         )
...
```{r muni-levies, include = FALSE}
levies <- burden %>%
  reframe(munilevy, tif_revenue = sum(final_tax_to_tif), .by = clean_name) %>%
  select(clean_name, munilevy, tif_revenue) %>% distinct()  %>% arrange(desc(munilevy))
levies
```
...
#| fig-height: 6
#| fig-cap: "Tax burden for properties with incentive classifications. Excludes municipalities that had a rate change less than 1 percentage point. Ordered by amount of change in the composite tax rate."
...
```{r}
#| label: fig-dotplot-ratechange-extradot
#| fig-height: 6
#| fig-cap: "Tax burden for Properties with Incentive Classifications. Excludes municipalities that had a rate change less than 0.5 percentage points."
...
order <- burden %>%  
  ...
  arrange(cur_munilevel_rate) %>%
  select(clean_name, cur_munilevel_rate) %>% distinct()

# make dot graph
burden %>% 
  filter(incentive == "Incentive" & clean_name != "NA") %>%
    filter(clean_name != "Willow Springs") %>%

  select(clean_name, cur_munilevel_rate, muni_rate_noInc, muni_rate_vacant) %>% 
  distinct() %>%
  arrange(cur_munilevel_rate) %>%
  pivot_longer(c("cur_munilevel_rate", "muni_rate_noInc", "muni_rate_vacant"), 
               names_to = "type", values_to = "tax_rate") %>% 
  inner_join(order) %>%
  ggplot(aes(x = tax_rate, y= reorder(clean_name, cur_munilevel_rate)))+
  geom_line(aes(group = clean_name))+ 
  ...
  scale_fill_manual(labels = c("Current Rate", "If LoA 25%", "If Vacant"),
    #values = c("#A6CEE3", "#1F78B4", "#08519C")  
    values = c("#EFF3FF", "#6BAED6", "#08519C")
...
```{r}
#| label: fig-dotplot-ratechange-fourdots
#| fig-height: 6
#| fig-cap: "Composite Tax Rate for Scenarios. Excludes municipalities that had a rate change less than 1 percentage point."
...
burden %>% 
 # filter(incentive == "Incentive" & clean_name != "NA") %>%
  select(clean_name, cur_munilevel_rate, muni_rate_noInc, muni_rate_neither, muni_rate_vacant, muni_rate_noExe) %>% 
  distinct() %>%
  arrange(cur_munilevel_rate) %>%
  pivot_longer(c("cur_munilevel_rate", 
                 "muni_rate_noInc", 
                 "muni_rate_vacant", 
                 "muni_rate_noExe",
                  "muni_rate_neither"), 
...
```{r}
#| label: fig-justmarkham
#| fig-cap: "Zoom in on Markham: Composite Tax Rate for Scenarios"


levies %>% filter( clean_name =="Markham" | clean_name == "Matteson")
...
burden %>% 
  ...
  pivot_longer(c("cur_munilevel_rate", "muni_rate_noInc", 
                 ...
                 "muni_rate_vacant", "muni_rate_noExe",
                  "muni_rate_neither"), 
               names_to = "type", values_to = "tax_rate") %>% 
  inner_join(order) %>%
  ggplot(aes(x = tax_rate, y= reorder(clean_name, cur_munilevel_rate)))+
...
#| fig-height: 8
#| fig-cap: "Tax Burden for Owner-Occupied Properties for 3 Tax Scenarios: With and without the reduced assessment ratio for commercial and industrial property (10% vs 25%). The Tax burden if homeowners were not granted the GHE. And the tax burden if residential owners did not get their exemption and incentive properties were assessed at 25% instead of 10%. This graph is made using the owner-occupied sub-category of Class 2 Properties for tax year 2022. Property tax data from the CCAO."
...
#  filter(clean_name != "Willow Springs") %>%
 ...
 # filter(incentive == "Incentive" & Alea_cat == "Industrial"  & clean_name !="NA") %>%
  ...
  scale_fill_brewer(palette = "RdGy", labels = c("Current Burden", "No Exemps or Incents",  "No Exemptions", "No Incentives"), direction = 1)+
...
```{r}
#| label: fig-residentialburden-scenarios-extradots
#| fig-height: 8
#| fig-cap: "Tax Burden for Owner-Occupied Properties for 3 Tax Scenarios: With and without the reduced assessment ratio for commercial and industrial property (10% vs 25%). The Tax burden if homeowners were not granted any exemptions. And the tax burden if residential owners did not get their exemption and incentive properties were assessed at 25% instead of 10%. This graph is made using the owner-occupied sub-category of Class 2 Properties for tax year 2022. Property tax data from the CCAO."
...
#  filter(clean_name != "Willow Springs") %>%
 ...
 # filter(incentive == "Incentive" & Alea_cat == "Industrial"  & clean_name !="NA") %>%
  ...
  scale_fill_brewer(palette = "RdGy", 
                    labels = c("Current Burden", "No Exemps or Incents",  
                               "No Exemptions", "No Incentives",
                               "If Vacant"
                               ), direction = 1)+
...
#| fig-height: 8
#| fig-cap: "Tax Burden for Owner-Occupied Properties for 3 Tax Scenarios: With and without the reduced assessment ratio for commercial and industrial property (10% vs 25%). The Tax burden if homeowners were not granted any exemptions. And the tax burden if residential owners did not get their exemptions and incentive properties were assessed at 25% instead of 10%."
...
burden %>% 
  ...
  scale_fill_brewer(labels = c("Current Tax Burden" , "Tax Burden without Exemptions\nor Incentives" , "Burden without Incentive\nClassifications"), direction = 1)+
...
#| fig-height: 10
#| fig-cap: "Tax Burden for Owner-Occupied Properties for 3 Tax Scenarios: With and without the reduced assessment ratio for commercial and industrial property (10% vs 25%). The Tax burden if homeowners were not granted any exemptions. And the tax burden if residential owners did not get their exemption and incentive properties were assessed at 25% instead of 10%."
...
burden %>% 
  ...
  scale_fill_brewer(labels = c("Current Tax Burden",  "Tax Burden without Exemptions\nor Incentives", "Burden without Incentives" ), direction = 1)+
