...
> Code above were all gradual steps to create the projects_checked_mainfile.xlsx
...
# 4. PIN-level join: comm_ind x parcel_comparison
  ...
  class_summary <- pin_info |>
    filter(!is.na(keypin) & !is.na(class)) |>
    group_by(class) |>
    summarize(
      pins    = n(),
...
There are `r paste(parcel_comparison %>% group_by(sale_document_num) %>% summarize(pins_ingroup = n()) |> filter(pins_ingroup > 1) |> n_distinct())` projects with more than 1 pin in them and  `r paste(parcel_comparison %>% group_by(sale_document_num) %>% summarize(pins_ingroup = n()) |> n_distinct())` projects overall based on the sale document number.
...
```{r}
township = "Bedford Park"
in_manually_coded <- keypins|> filter(Township == township) |> count()
in_wrkshts <- worksheet_keypins |> filter(pin %in% keypins$pin[keypins$Township==township]) |> count()
```

There are `r in_manually_coded - in_wrkshts` pins identified in the manually coded file than in the valuation worksheets for this township. 
...
```{r}
pin_info <- comm_ind |> right_join(parcel_comparison)
...
pin_info |> 
  ...
  arrange(desc(pins_ingroup), desc(incentive_pins))

```


```{r}
township = "South Holland"
in_manually_coded <- keypins|> filter(Township == township) |> count()
in_wrkshts <- worksheet_keypins |> filter(pin %in% keypins$pin[keypins$Township==township]) |> count()
```
...
pin_info |> 
  ...
  summarize(pins_ingroup = n_distinct(pin),
            parcels_ingroup = n_distinct(pin10),
            incent_pins = sum(incent, na.rm=TRUE),
            #av_mailed = sum(av_mailed, na.rm=TRUE),
            #av_clerk = sum(av_clerk, na.rm=TRUE),
            fmv = sum(fmv, na.rm=TRUE),
            ) |>
