...
```{r}
# made this before bedfork park, berwyn, and south holland, and matteson were released in the newest methodology worksheets
...
parcel_comparison <- left_join(parcels_sf, keypins_df, by = "pin10") %>%
  select(pin10, keypin, everything())
...
```{r eval = FALSE}
keypins <- readxl::read_xlsx("Output/projects_checked_MAINFILE.xlsx") |>
  mutate(keypin = main_keypin)
