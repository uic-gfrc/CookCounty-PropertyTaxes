...
> Pull triad_name variable (North, South, ) from Assessor Neighborhood boundaries? Only for township codes though, BUT that indicates when they are reassessed. 


# The 6 Communities

```{r}
six_areas <- c("OAK PARK", "BRIDGEVIEW", "DOLTON", "HOFFMAN ESTATES", "MIDLOTHIAN", "CHICAGO")

```



Including `pin`, `class` and `tax_code` in the `$select` command creates duplicate rows. 
...
Using simplify = FALSE: Gives you eav and `final_tax`
