...
## aggregated straight to county level

```{r}
#| label: tbl-county-sums
...
county_summary %>% filter(year == 2021)
```


```{r}
#| label: tbl-county-incentive-sums
#| tbl-cap: "County level stats aggregated from PIN level data. Includes ALL commercial and industrial PINs."
...
```
```{r}
#writexl::write_xlsx(muni_incentive_data, "muni_incentive_values.csv")
```

```{r}
#| label: tbl-create-county-trends
#| tbl-cap: "**Industrial and Commercial Values** Aggregated from PIN level data. Includes ALL commercial and industrial PINs."
...
county_trends %>% filter(year == 2021)
```


## Aggregated to Muni level, then find average muni

```{r}
#| label: tbl-muni-sums
#| tbl-cap: "Industrial and Commercial Properties aggregated to Municipality level. Includes all industrial and commercial properties, both with and without incentives."
...
muni_data <- df_temp |>
  ...
  summarize(n_pins = sum(n_pins, na.rm=TRUE), 
            ...
            fmv_mean = mean(fmv, na.rm=TRUE),
            fmv = sum(fmv, na.rm=TRUE))
muni_data %>% filter(year == 2021)
```


```{r}
#| label: tbl-muni-landuse-sums
#| tbl-cap: "Industrial and Commercial Properties aggregated to Municipality level. Includes all industrial and commercial properties, both with and without incentives."
...
muni_landuse_data %>% filter(year == 2021)
```


```{r}
#| label: tbl-muni-incentive-sums
#| tbl-cap: "Incentive Class Industrial and Commercial Properties aggregated to Municipality level. Includes all industrial and commercial properties with incentives property classes (including 400-level properties)"
...
muni_incentive_data %>% filter(year == 2021)
```

### Find "Average" Municipality


```{r }
#| label: tbl-countyfrommuni
#| tbl-cap: "For all commercial and industrial PINs. County level stats created from the aggregated municipalities. "
...
county_total_from_muni_data %>% filter(year == 2021)
```
...
county_incent_from_munis %>% filter(year == 2021)
```

```{r }
#| label: tbl-countyfrommuni-incentiveprops
#| tbl-cap: "For all commercial and industrial PINs. County level stats created from the aggregated municipalities. "
...
county_landuse_incentives %>% filter(year == 2021)
```

```{r }
#| label: tbl-countyfrommuni-incentiveprops-together
...
write_csv(sums_table, "sums_table2021.csv")
