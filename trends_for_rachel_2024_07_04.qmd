...
vars_4fn <- c("n_pins", "min_fmv_all", "quant25_all_fmv", "quant50_all_fmv", "quant75_all_fmv", "max_fmv_all", "fmv_mean", "fmv", "cty_pins")

labels_4fn <- c("PIN Count", "Min. FMV", "25th Quantile", "50th Quantile", "75th Quantile", "Max. FMV", "Avg. FMV", "Total FMV", "PIN Count")
...
county_trends <- df_temp |> #see df_temp created above
  ...
  ungroup() |>
  mutate(incent_prop_use = paste(land_use, incent_prop)) |>
  arrange(year) 

county_trends <- county_trends |>
  group_by(incent_prop_use) |>
  arrange(year) |>
  ...
  select(year, incent_prop_use, n_pins, lag_n_pins, av, lag_av, 
         fmv, lag_fmv) |>
  # Calculate YoY deltas
  group_by(year, incent_prop_use) |>
  mutate(yoy_pin_delta = round((n_pins - lag_n_pins) / lag_n_pins, digits =2),
         yoy_av_delta = round((av - lag_av) / lag_av, digits = 2),
         yoy_fmv_delta = round((fmv - lag_fmv) / lag_fmv, digits = 2)) %>% ungroup() 

county_trends %>% filter(year == 2022)  %>% 
  select(-year) %>%
  pivot_longer(cols = n_pins:yoy_fmv_delta, names_to = "Variables")  %>%
  pivot_wider(id_cols = "Variables", names_from = incent_prop_use ) %>%
  #  mutate(Variables = ifelse(Variables %in% df_labels$vars_4fn, df_labels$labels_4fn, "Need Name")) %>% 
...
county_trends |> 
  filter(year == 2021) %>%  pivot_longer(cols = n_pins:yoy_fmv_delta, names_to = "Variables")  %>%
  pivot_wider(id_cols = "Variables", names_from = incent_prop_use ) %>%
  #  mutate(Variables = ifelse(Variables %in% df_labels$vars_4fn, df_labels$labels_4fn, "Need Name")) %>% 
...
```{r}
#| label: tbl-countyfrommuni
#| tbl-cap: "Municipality summary statistics include **all commercial and industrial PINs** within each municipality. County level \"median municipality\" stats created from the aggregated municipalities."
#| layout-ncol: 2
...
county_total_from_muni_data %>% filter(year == 2022) %>% 
  pivot_longer(cols = cty_pins:fmv, names_to = "Variables") %>% select(-year) %>%
   mutate(Variables = ifelse(Variables %in% df_labels$vars_4fn, df_labels$labels_4fn, "Need Name")) %>% 
   flextable() %>%
   set_header_labels(Variables = "Median Municipality Values", value = "2022 Values") %>%
   FitFlextableToPage()
...
county_total_from_muni_data |>
  ...
  pivot_longer(cols = cty_pins:fmv, names_to = "Variables") %>% select(-year) %>%
   mutate(Variables = ifelse(Variables %in% df_labels$vars_4fn, df_labels$labels_4fn, "Need Name")) %>% 
   flextable() %>%
   set_header_labels(Variables = "Median Municipality Values", value = "2021 Values") %>%
   FitFlextableToPage()
```


```{r}
#| label: tbl-munis_to_county
#| tbl-cap: "Median Municiaplity. Values aggregated to the Municpality level and then summary statistics are calcuated for the Municiaplities. **Only includes incentive class properties** when calculating summary values."
#| layout-ncol: 2
#| 
...
county_incent_from_munis %>% filter(year == 2022) %>%  pivot_longer(cols = cty_pins:fmv, names_to = "Variables") %>% select(-year) %>%
   mutate(Variables = ifelse(Variables %in% df_labels$vars_4fn, df_labels$labels_4fn, "Need Name")) %>% 
   flextable() %>%
   set_header_labels(Variables = "Median Municipality Values", value = "2022 Values") %>%
   FitFlextableToPage()
...
#| tbl-cap: "For all commercial and industrial **incentive** PINs. County level stats created from the aggregated municipalities. 98 Municipalities used incentive property classes in 2021 and 99 municipalities did so in 2022."
#| layout-ncol: 2
...
county_landuse_incentives %>% filter(year == 2022) %>%   pivot_longer(cols = cty_pins:fmv, names_to = "Variables") %>% select(-year) %>%
  pivot_wider(id_cols = "Variables", names_from = land_use) %>%
   mutate(Variables = ifelse(Variables %in% df_labels$vars_4fn, df_labels$labels_4fn, "Need Name")) %>% 
   flextable() %>%
   set_header_labels(Variables = "2022 Median Municipality Values", value = "2022 Values") %>%
   FitFlextableToPage()



county_landuse_incentives |>
  filter(year == 2021) %>%   pivot_longer(cols = cty_pins:fmv, names_to = "Variables") %>% select(-year) %>%
    pivot_wider(id_cols = "Variables", names_from = land_use) %>%

   mutate(Variables = ifelse(Variables %in% df_labels$vars_4fn, df_labels$labels_4fn, "Need Name")) %>% 
   flextable() %>%
   set_header_labels(Variables = "2021 Median Municipality Values", value = "2021 Values") %>%
   FitFlextableToPage()
...
#| tbl-cap: "For all **incentive** commercial and industrial PINs. County level stats created from the aggregated municipalities."
#| layout-ncol: 2
...
county_landuse_incentives %>% filter(year == 2022) %>%   
  ungroup%>%
  pivot_longer(cols = cty_pins:fmv, names_to = "Variables") %>% 
   mutate(Variables = ifelse(Variables %in% df_labels$vars_4fn, df_labels$labels_4fn, "Need Name")) %>%   select(-year) %>%

   flextable() %>%
  
   set_header_labels(Variables = "Median Municipality Values, 2022", value = "2022 Values") %>%
   FitFlextableToPage()
...
county_landuse_incentives |>
  ...
  pivot_longer(cols = cty_pins:fmv, names_to = "Variables") %>% select(-year) %>%
   mutate(Variables = ifelse(Variables %in% df_labels$vars_4fn, df_labels$labels_4fn, "Need Name")) %>% 
   flextable() %>%
   set_header_labels(Variables = "Median Municipality Values. 2021", value = "2021 Values") %>%
   FitFlextableToPage()
...
muni_medians_2021 <- muni_landuse_data |>
  filter(year == 2021) |>
  filter(incent_prop == "Incentive") |>
  filter(land_use == "Industrial") |>
  group_by(clean_name, land_use) |>
...
sum_pins_2021 <- muni_landuse_data |>
  filter(year == 2021) |>
  filter(incent_prop == "Incentive") |>
  group_by(clean_name) |>
  mutate(pins = sum(n_pins),
...
sum_fmv_2021 <- muni_landuse_data |>
  filter(year == 2021) |>
  filter(incent_prop == "Incentive") |>
  group_by(clean_name) |>
  mutate(pins = sum(n_pins),
...
sum_ind_pin_2021 <- muni_landuse_data |>
  filter(year == 2021) |>
  filter(incent_prop == "Incentive") |>
  filter(land_use == "Industrial") |>
  group_by(clean_name) |>
...
sum_ind_fmv_2021 <- muni_landuse_data |>
  filter(year == 2021) |>
  filter(incent_prop == "Incentive") |>
  filter(land_use == "Industrial") |>
  group_by(clean_name) |>
...
sum_comm_pin_2021 <- muni_landuse_data |>
  filter(year == 2021) |>
  filter(incent_prop == "Incentive") |>
  filter(land_use == "Commercial") |>
  group_by(clean_name) |>
...
sum_comm_fmv_2021 <- muni_landuse_data |>
  filter(year == 2021) |>
  filter(incent_prop == "Incentive") |>
  filter(land_use == "Commercial") |>
  group_by(clean_name) |>
...
sums_table_2021 <- bind_cols(sum_pins_2021, sum_fmv_2021, sum_ind_pin_2021, sum_ind_fmv_2021, sum_comm_pin_2021, sum_comm_fmv_2021)

write_csv(sums_table_2021, "sums_table2021.csv")
sums_table_2021
...
perc_table2021 <- bind_cols(pin_perc, perc_ind_pin, perc_comm_pin, fmv_perc, fmv_ind_perc, fmv_comm_perc)

write_csv(perc_table2021, "perc_table2021.csv")
perc_table2021
```
...
comm_ind <- comm_ind %>% 
  ...
  mutate(
    land_use = case_when(
      Alea_cat == "Commercial" ~ "Commercial",
      Alea_cat == "Industrial" ~ "Industrial",
      TRUE ~ "Other"),
    landuse_change = ifelse())


table(comm_ind$years_existed, comm_ind$major_class_code)

table(comm_ind$years_existed, comm_ind$incent_prop)

table(comm_ind$years_existed, comm_ind$Alea_cat)
