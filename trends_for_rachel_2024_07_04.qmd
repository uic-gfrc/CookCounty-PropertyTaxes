...
county_muni <- muni_data |>
  ungroup() |>
  group_by(year) |>
  summarize(
         cty_pins = sum(n_pins, na.rm=TRUE),
            min_fmv_all = min(min_fmv_all, na.rm = TRUE),
            quant25_all_fmv = round(quantile(quant25_all, probs = q[1])),
            quant50_all_fmv = round(quantile(quant50_all, probs = q[2])),
            quant75_all_fmv = round(quantile(quant75_all, probs = q[3])),
