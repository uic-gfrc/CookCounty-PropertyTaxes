...
> Observations where `fmv_growth_2011` were NAs or NaNs were dropped. This means the dataset is not longer a square panel data dataset. 
...
> Changed `filter(!is.na(fmv_2011))` to `filter(!is.na(fmv_growth_2011))` since that is the DV that is used below.
...
#### Comparison with Muni dummy out of curiosity 

```{r}
#| label: plm-munidummy-comparison


model_plm <- plm(fmv_growth_2011 ~ change_incent_bin + fmv_2011 + Triad 
                   + change_prop_use_bin + land_use + in_tif,
    model = "within",
    effect = "individual",
                    data = df_2022) 
...
```{r include=FALSE}
# breaks code, still don't know why.

# modelnames <- c("Base", "1", "2", "3",  "4")
# 
# muni_dummies_table <-
#   export_summs(muni_dummy_1, muni_dummy_2, muni_dummy_3, muni_dummy_4, muni_dummy_5,
#                model.names = c("Base", "1", "2", "3",  "4"),
#                                 statistics = c(N = "nobs", R2 = "r.squared", adj.R2 = "adj.r.squared"))
# 
# 
