...
```{r}
parcel_comparison <- readxl::read_xlsx("Output/projects_bedfordpark_checked.xlsx")  |>
...
```{r}
parcel_comparison <- readxl::read_xlsx("Output/projects_southholland_checked.xlsx")  |>
  mutate(pin10 = str_pad(pin10, 10, "left", "0"),
         pin = str_pad(pin, 14, "left", "0"),
         keypin = str_pad(keypin, 14, "left", "0"))
...
```{r}
parcel_comparison <- readxl::read_xlsx("Output/projects_franklinpark_checked.xlsx")  |>
  mutate(pin10 = str_pad(pin10, 10, "left", "0"),
         pin = str_pad(pin, 14, "left", "0"),
         keypin = str_pad(keypin, 14, "left", "0"))
...
```{r}
parcel_comparison <- readxl::read_xlsx("Output/projects_matteson_checked.xlsx")  |>
  mutate(pin10 = str_pad(pin10, 10, "left", "0"),
         pin = str_pad(pin, 14, "left", "0"),
         keypin = str_pad(keypin, 14, "left", "0"))
...
There are `r paste(parcel_comparison2 %>% group_by(keypin) %>% summarize(pins_ingroup = n()) |> filter(pins_ingroup > 1) |> n_distinct())` projects with more than 1 pin in them and  `r paste(parcel_comparison2 %>% group_by(keypin) %>% summarize(pins_ingroup = n()) |> n_distinct())` projects overall based on manually grouped keypins.
