...
comm_ind |>
 ...
 # select(year, pin, fmv, incent_prop, Alea_cat) |>
...
comm_ind |>
  ...
  ggplot() + 
  geom_line(aes(x=year, y=av_clerk, color = Alea_cat))
```
...
djia <- read_csv("G:/My Drive/PhD/Cook County Property Taxes/djia_timetrend.csv") |>
  rename(year = Year, djia = djia_avg_close)

early <- as.numeric(2006:2010)

late <- as.numeric(2011:2022)

comm_ind <- df_2006_2022

dumb_table <- comm_ind |>
  filter(year %in% c("2006", "2011", "2022")) |>
  group_by(year) |>
  mutate(fmv_2006_sum = sum(base_year_fmv_2006, na.rm = TRUE),
         fmv_2011_sum = sum(base_year_fmv_2011, na.rm = TRUE)
         ) |>
  ungroup() |>
  filter(Alea_cat == "Commercial" & incent_prop == "Incentive") |>
  group_by(year) |>
  summarize(year_fmv = sum(fmv), year_fmv/fmv_2006_sum, year_fmv/fmv_2011_sum)
  

summarize(fmv_2006 = sum(base_year_fmv_2006, na.rm = TRUE))
  
  mutate(year_county_fmv = sum(fmv),
         base_2006 = sum(base_year_fmv_2006),
         base_2011 = sum(base_year_fmv_2011)
         )|>
  ungroup() |>
  #filters down to 1.2mil obs.
  filter(Alea_cat == "Commercial") |>
  #filters to 8948 obs
  filter(incent_prop == "Incentive") |>
  select(year, pin, year_county_fmv, fmv, base_2006, base_2011) |>
  group_by(year) |>
  summarize(PINs = n(), "Total Industrial/Commercial FMV" = first(year_county_fmv), 
            "Incentive Class Commercial FMV" = sum(fmv))

kable(dumb_table, format = "html", escape = FALSE) |>
  kable_styling(full_width = FALSE) |>
  column_spec(1, width = "3cm")

dumb_table |>
  ggplot(aes(x = year)) +
  geom_line(aes(y = sum_fmv_mils), color = "purple") +
  geom_line(aes(y = exp(djia)), color = "orange") +
  geom_vline(aes(xintercept = 2011), color = "blue") +
  theme_minimal()
...
# 2006 vs. 2022 Incentivized Commercial:

3.68/.18

# 2011 vs. 2022 Incentivized Commercial:
...
vars_4fn <- c(
  #"n_pins", "min_fmv_all", "quant25_all_fmv", "quant50_all_fmv", "quant75_all_fmv", "max_fmv_all", "fmv_mean", "fmv", "cty_pins", 
            #  "term", 
              "av_clerk", "years_existed","fmv", "base_year_fmv_2006", "base_year_fmv_2011" )

labels_4fn <- c(
  #"PIN Count", "Min. FMV", "25th Quantile", "50th Quantile", "75th Quantile", "Max. FMV", "Avg. FMV", "Total FMV", "PIN Count",
              #  "Variables", 
                "AV",  "PIN<br>Age", "FMV", "FMV<br>2006", "FMV<br>2011"
                )
