...
The block range for mutate() is from the Clerk's Office document for EPA and buyer of property.
has pins listed on a [pdf on the website](https://crs.cookcountyclerkil.gov/Document/Detail?dId=Mjk4Njg0Mjk1&hId=OWQ1Y2RhZWY2NmRmNzM5YTZkZjUyOGE2YjUyZTgwZmQxYmYwZDlkNjQwYzczMmNjYmI0NWQxOWQ4MWUxZTU5Nw2)
...
incentpins_2022 <- unique_ptax_w_class %>%
  ...
  mutate(keypin_concat = ifelse((parcel >= 2824201015 & parcel <= 2824211032) | (parcel >=2919100002 & parcel <= 2919105056), "Amazon", keypin_concat)) %>%
  mutate(
    keypin_concat = ifelse(parcel >= 2919107001 & parcel <= 2919120018, "Amazon", keypin_concat )
  )
# 4385 obs
...
amazon_map <- ggplot() +
  ...
  theme_void() +
    labs(title = "Amazon PINs in 2022 by Class: Markham and Harvey") +
  theme(
