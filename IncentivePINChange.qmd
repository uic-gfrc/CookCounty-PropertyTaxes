...
There were 4,385 incentive PINs in tax year 2022. Of those, 1,483 were missing key pins. 
...
incentpins_2022 <- unique_ptax_w_class %>%
  ...
  mutate(keypin_concat = ifelse(block >= 2919107 & block <= 2919120, "Amazon", keypin_concat ),
...
# 1483 unique PINs still # 1149 3/21/2024 AWM
...
```{r}
# there are 345 project identifiers for the 716 pins that
project_pins_accessDB %>% group_by(keypin_concat) %>% summarize(pincount = n())
```
