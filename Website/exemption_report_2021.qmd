...
<!--- Note to self: Inline Code cannot be used in YAML so you cannot include the tax year in the title using the paramters. ---> 
...
#| tbl-cap: "**Summary of Homestead Exemption Use in Cook County (Tax Year 2021)** Eq_AV is the 2021 equalization factor multiplied by the assessed value of all property and can be thought of as EAV pre-exemption. The general homestead exemption accounts for the vast majority of exempt EAV. EAV shown in millions."
...
median_exempt <- muni_sums |>
  filter(year==i)|>
  mutate(muni_pct_fmv_exempt = (muni_eav - muni_eav_taxed) / muni_eav) |> 
  select(muni_pct_eav_taxed, clean_name)


midpoint = scales::percent(median(median_exempt$muni_pct_eav_taxed), accuracy = 0.01)

median_muni = median_exempt |>
  arrange(muni_pct_eav_taxed) |>
  filter(muni_pct_eav_taxed == median(muni_pct_eav_taxed)) |>
...
print(muni_sums |>
 ...
 full_join(muni_shp, by = c("agency_name" = "AGENCY_DESC")) |>
  ggplot(aes(fill = muni_pct_eav_taxed)) + 
 ...
 +
    scale_fill_stepsn(colors = c("#ffffcc","#a1dab4" ,"#41b6c4","#2c7fb8", "#253494"),
                        show.limits=TRUE, 
                    limits = c(0, max(muni_sums$muni_pct_eav_taxed)),
                  breaks = c(0.075, 0.15, 0.225, 0.30),
                       na.value = "gray50",
                  #  nice.breaks = FALSE,
...
```{r}
#| label: fig-exemptiontotals_for2021
#| fig-cap: "**Exempt EAV in Cook County, Tax Year 2021** Exemption use varies between suburban Cook County ($9.7B EAV) and the City of Chicago ($7.0B EAV), which may point to broader socioeconomic patterns as well as residents' eligibility and update. Note: Our current calculations undervalue the disabled veterans exemption."
