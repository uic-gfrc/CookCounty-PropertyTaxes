...
pin_data %>% filter(class == 239) %>% mutate(difference = round(eq_av - (taxed_eav + untaxable_value_eav)), .before = taxed_fmv ) %>% arrange(desc(difference)) %>% head()
...
```{r ptax2022-codebook, eval=FALSE, include = FALSE}
library(dataReporter)
check(pin_data$fmv)  # Individual check of events
check(pin_data$both_ids)  # Individual check of events
reporter_summary <- summarize(pin_data)

check(pin_data) # Check all variables at once
...
```{r}
pin_data <-  read_csv("../Output/comm_ind_PINs_2006to2022_timeseries.csv") %>% 
