...
```{r}
#| label: fig-income-distributions-2018-2023
#| fig-cap: "Weighted household and earnings distributions used to evaluate Senior Freeze income thresholds"
#| fig-subcap: ["Household income, 2018", "Household income, 2023", "Individual earnings, 2018", "Individual earnings, 2023"]
#| layout-ncol: 2
...
#| tbl-cap: "Change in owner-occupied senior households from 2018 to 2023 by PUMA area"
...
mapPUMAboth |> 
  ...
  select(`Change in households` = change, `PUMA area` = puma_area# Prop, puma_area
  ) |> 
  arrange(desc(`Change in households`)) |> 
  DT::datatable(rownames = FALSE)
```

```{r}
#| label: tbl-currenteligible-change
#| tbl-cap: "Change from 2018 to 2023 in owner-occupied senior households eligible under the $65,000 Senior Freeze income threshold"
...
mapPUMAboth |> 
  ...
  pivot_wider( id_cols = c(puma_area), names_from = "YEAR", values_from = "n") |> 
  mutate(change = `2018`-`2023`) |>
  select(`Change in eligible households` = change, `PUMA area` = puma_area# Prop, puma_area
  ) |> 
  arrange(desc(`Change in eligible households`)) |> 
  DT::datatable(rownames = FALSE)
```
...
```{r}
#| label: fig-senior-freeze-pin-count
#| fig-cap: "PINs receiving the Senior Freeze Exemption by tax year; the vertical line marks the 2017 income-limit increase"
pins |> 
  group_by(year) |> 
