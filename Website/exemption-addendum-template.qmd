...
muni_ratechange <- read_csv(muni_ratechange, paste0("../Output/muni_ratechange_", i, "_test.csv"))

muni_rates <- muni_ratechange |> # keep just a couple variables to avoid join variable name conflicts later
  select(clean_name, current_rate_avg, rate_noExe, rate_current)
...
tax_bill_change_HO <- c2pins %>% 
  ...
  mutate(eq_av = av_clerk*eq_factor,
         ...
         bill_noexemps =  tc_hyp_taxrate*(taxed_eav+exe_homeowner),
