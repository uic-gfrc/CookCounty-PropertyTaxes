...
C2_munistats_adjusted <- c2pins %>% 
  ...
  arrange(av) %>%
  summarize(
    median_eav = round(median(taxed_eav_adj)), 
    median_av = round(median(av)), 
    avg_av = round(mean(av)),
    avg_eav = round(mean(taxed_eav_adj)),
    C2_pins_in_muni = n(),
    C2_current_exemptions = sum(exe_total_adj, na.rm = TRUE),
    C2_HO_exemps = sum(exe_homeowner, na.rm = TRUE),
  ) 
...
tax_bill_change_HO_adjusted <- c2pins %>% 
  ...
  arrange(av) %>%
  mutate(#eq_av = av_clerk*eq_factor,
         bill_current = (taxed_eav_adj*tax_code_rate),
         bill_noexemps =  rate_noGHE*(taxed_eav_adj+exe_homeowner),
         bill_change = bill_noexemps - bill_current) %>%
  group_by(clean_name, has_HO_exemp) %>% 
  summarize(median_AV = median(av),
            median_EAV = median(taxed_eav_adj),
            median_bill_cur = round(median(bill_current)),
            median_bill_new = round(median(bill_noexemps)),
            median_change = round(median(bill_change)),
            pincount=n(),
