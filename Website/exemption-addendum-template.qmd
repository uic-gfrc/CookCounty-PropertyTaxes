...
Many of the figures and tables in the Exemption Addendum refer to the "Alternative Scenario" and the "Status Quo". The status quo is simply Cook County's current property tax regime. The alternative scenario, by way of contrast, sets the *general* homestead exemption (and only general homestead exemption) to zero.
...
### Table 1. Comparison of Tax Bills for Median Assessed Value (AV) Class 2 Properties in Selected Municipalities under the Status Quo and Alternative Scenario

```{r}
#| label: tbl-taxbillchange
#| tbl-cap: "**Change in Tax Bill for Properties with the median Assessed Value for a property within a given municipality** AV stands for \"assessed value.\" \"Tax calculator reduction\" refers to the exemption values shown on the tax bill received by taxpayers. Values are estimated using an average of the properties with an AV within $1,000 of its municipality's median AV to enlarge the number of observations. Thus, our calculations do not precisely represent the median." 
...
pin_data <- pin_data %>% 
  ...
  mutate(
    ...
    # Use the gross increment EAV. For transit TIFs, final_tax_to_tif is only
    # the retained share and cannot be converted back to the full increment.
    tif_increment_eav = tif_increment_eav_bill_allocated,
...
scenario_calcs <- pin_data %>%    
  group_by(clean_name) %>%
    ...
    summarize(MuniLevy = round(sum(final_tax_to_dist, na.rm = TRUE)), # amount billed by munis with current exemptions in place
            current_nonTIF_EAV_post_exemps = sum(taxed_eav, na.rm = TRUE),
            current_TIF_increment_EAV = sum(tif_increment_eav_bill_allocated, na.rm=TRUE),
            current_Exempt_EAV = sum(all_exemptions, na.rm=T), 
            current_GHE = sum(exe_homeowner, na.rm=TRUE),
            Total_EAV = sum(all_exemptions + total_taxed_eav_AWM, na.rm = TRUE),
...
pin_data %>% 
  ...
  summarize(
    ...
    tif_increment_eav = sum(tif_increment_eav_bill_allocated)
  ) %>%
...
There are `r scales::comma(mil_homes_total$million_dollar_homes)` homes worth $1 million in Cook County, IL.
