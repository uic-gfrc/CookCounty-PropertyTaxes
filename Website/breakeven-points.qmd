...
**Steps for prepping data for graphs below:**
...
current tax rate = aggregate levy for all taxing agencies within the geography being used for comparison / taxable EAV

$\frac{Levy}{EAV_{Taxable}} = TaxRate$

$current\_taxable\_eav = \frac{final\_tax\_to\_dist}{(\frac{(cur\_comp\_muni\_rate)}{100})}$

taxable EAV = non-TIF increment EAV after exempt EAV has been subtracted
...
The "breakeven point" is where the current taxbill for a property is the same amount as it would be in the hypothetical scenario. In this case, the hypothetical involves eliminating the GHE, which adds all of the `exe_homeowner` to the taxable EAV and decreases the tax rate.

*hypothetical tax bill = current tax bill*

*EAV \* tax rate w/o the GHE = EAV \* current tax rate*

$$
EAV * \tau_{wo} = EAV - exemptEAV_{homeowner} * \tau_w
$$

which simplifies to:

$$
EAV_{breakeven} = - (\frac{\tau_w}{\tau_{wo} - \tau_w} * exemptEAV_{homeowner})
$$

$$
tax code tax rate = \frac{\sum{final\_tax\_to\_dist}}{\sum{CurrentTaxableEAV}} $$

Aggregate Municipal Levy $\approx$ $\sum{final\_tax\_to\_dist}$ because it is the amount billed by the taxing agency. It technically is based on the amount billed.

Homeowners think they are saving the equivalent of their exempt EAV \* their current tax rate. This amount also appears on the tax bill and is the variable `tax_amt_exe` in the PTAXSIM database.

$\sum{tax\_amt\_exe} = EAV_{exempt}*\tau_{current}$

$naive\_cost = \sum(bill\_change)$\

But if the EAV that was exempt became taxable, everyone's tax rate would decrease to some extent. The change in tax rate varies on a tax code to tax code level and depends on the amount of EAV that becomes taxable in comparison to total EAV within the tax code.

For the GHE scenarios, the hypothetical tax rate and amount of EAV that is exempt changes in each scenario.

**Perceived vs. Actual Savings:** Homeowners think they are saving $EAV_{exempt}* taxrate_{current} = \sum{tax\_amt\_exe}$ but their taxable EAV would be multiplied by a lower tax rate due to the increase in taxable EAV within the geographic region.

At the county level, the "cost" of exemptions is \\\$1.385 billion.[^breakeven-points-1]

[^breakeven-points-1]: $\sum(EAV_{exempt} * taxrate_{current})$ = \$1.579 billion

    $\sum(EAV_{exempt} * taxrate_{hyp})$ = \$1.385 billion
...
#| fig-cap: "Excludes outliers. The breakeven point is the assessed values at which a major class 2 property would not have their taxbill change if the GHE were eliminated. Excludes outliers: University Park, Bedford Park, McCook, Hodgkins, and Rosemont. Class 2 properties that had claimed the GHE would have lower taxbills even if the GHE were removed if the AV is above the breakeven point."
...
```{r}
#| label: fig-breakevenpoint-flippedaxes
#| fig-cap: "Municipalities' Median AV & Breakeven Point - Class 2 Properties Only"
#| 
nochange %>%
  filter(nochange_av < 300000) %>%
  ...
  geom_point(data = (nochange %>% filter(clean_name %in% c("Park Forest", "Chicago", "Winnetka", "Riverdale","Dolton"))), aes(x = nochange_av, y = C2median_av, color = "red"), size = 3) +
  ggrepel::geom_label_repel(data = (nochange %>% filter(clean_name %in% c("Park Forest", "Chicago", "Winnetka", "Riverdale", "Dolton"))), aes(x = (nochange_av), y = (C2median_av)), size = 3)+
  theme_classic() +
  scale_x_continuous(labels = scales::dollar, expand = c(0,0), limits = c(0,170000) ) +
  scale_y_continuous(labels = scales::dollar, expand = c(0,0), limits = c(0,170000) ) +
    theme(legend.position = "none") +
...
#| fig-subcap: "The breakeven point is the assessed values at which a major class 2 property would not have their taxbill change if the GHE were eliminated. Excludes outliers: University Park, Bedford Park, McCook, Hodgkins, and Rosemont. Class 2 properties that had claimed the GHE would have lower taxbills even if the GHE were removed if the AV is above the breakeven point."
