...
comm_ind_pins %>%
  ...
  ggplot() +
    scale_color_manual(values = c("#008FD5", "#ADE0EE", "#D0E4A4"))+
...
df_2011_bal %>% 
  ...
  theme(    panel.background = ggplot2::element_blank(),
   ...
   scale_x_continuous(limits = c(2005, 2024),                                    
                      breaks = c(2006, 2010, 2015, 2020, 2023)) + 
...
#| fig-cap: !expr paste0("Growth from 2011 to ", params$year,  ". Faceted by if a PIN changed landuse during the sample period. Indexed to fair market value during 2011. **Only for comparison to show what has been dropped in the next image.**")
...
df_2011_bal %>% 
  ...
  geom_line(aes(x=year, fmv_group_growth, group = incent_status, color = incent_status)) + 
  labs( title = "FMV Growth Since 2011 by Land Use",) + 
  theme_bw()  +    scale_color_manual(values = cmap_colors) + 
...
df_2011_bal %>%  
  ...
  theme_bw()  +  
  facet_wrap(~incent_status, nrow = 1) +
  scale_x_discrete(limits = c(min_year-1, max_year+1)
  #  breaks = c(2012, params$year)
                   ) + 
