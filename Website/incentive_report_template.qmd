...
Municipalities that are dropped due to having EAV outside of Cook County: Frankfort, Homer Glen,  Oak Brook, East Dundee, University Park,  Bensenville, Hinsdale, Roselle, Deer Park, Deerfield, Elgin,
  Buffalo Grove,  Bartlett,  Burr Ridge, Hanover Park, Steger. 
...
pin_data <- pin_data |>   
  select(-incent_prop) |> 
  left_join(proj_xwalk)
```

<!--- BE CAREFUL and fix this in the future: pin_data had incent_prop be binary numeric until this point but then the variable is dropped and rejoined by a different incent_prop variable that uses "Incentive" and "Non-Incentive" as the categories  --->
...
cook_MC_sums <- read_csv("../Output/ptaxsim_cook_MC_level_2006to2023.csv") 
...
no_incentives <-  pin_data |> 
  group_by(clean_name) |> 
  filter(sum(incent_prop == "Non-Incentive" ) == 0) |> 
  distinct(clean_name)
...
```{r}
#| label: fig-altFigure7-shareofcomindFMV
#| fig-cap: "Share of Commercial and Industrial FMV that has an incentive property class."
...
(taxed_av*eq_factor/loa)*0.25, taxed_av*eq_factor), na.rm=TRUE),
    ...
    pins_withincents = sum(incent_prop == "Incentive"),
    fmv_incentive = sum(ifelse(incent_prop == "Incentive", fmv, 0), na.rm = TRUE),
    #fmv_taxed =  sum(taxed_fmv, na.rm=TRUE),
    fmv_incents_inTIFs = sum(ifelse(incent_prop == "Incentive" & final_tax_to_tif > 0, fmv, 0), na.rm = TRUE),
 ...
 #   fmv_untaxable_value = sum(untaxable_value_fmv , na.rm=TRUE),
    ...
    eav_incents_inTIFs = sum(ifelse(incent_prop == "Incentive" & in_tif == 1, taxed_eav, 0), na.rm = TRUE),
...
```{r}
#| label: tbl-Table1_JD_paper
#| tbl-cap: "**Composite Tax Rate Change** from hypothetical scenario of taxing property that is current exempt due to exemptions."
...
```{r}
#| label: fig-Figure9-dotplot3dots-focusonbigchanges
#| fig-cap: "**Figure 9 in Incentive Report.** Only shows municipalities that had more than 1/2 percentage point change in tax rate. Municipalities with less than X amount are in grayscale."
#| fig-height: 6
#| fig-width: 6
...
figure9_data <- muni_ratechange |> 
  ...
  mutate(dotcolor = case_when(
      ...
      dotlabel = ifelse(lowchange, 
                              paste0("<span style='color:gray70;'>", clean_name, "</span>"), 
                              clean_name)
...
```{r}
#| label: fig-JoshRachelPaper-dotplot3dots-focusonbigchanges
#| fig-cap: "**Image for Academic Paper.** Only shows municipalities that had more than 1/2 percentage point change in tax rate. Municipalities with less than X amount are in grayscale."
#| fig-height: 10
#| fig-width: 6
