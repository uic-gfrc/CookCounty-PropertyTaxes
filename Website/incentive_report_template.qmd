...
class_dict <- read_csv("../Necessary_Files/class_dict_expanded.csv") %>%
  select(class_code, comparable_props, land_use=Alea_cat, incent_prop)
...
pin_data <- pin_data %>% 
  mutate(
     incent_prop = ifelse(class >= 600 & class < 900, "Incentive", "Non-Incentive"),
...
cook_ratechange <- read_csv("../Output/ptaxsim_cook_level_2006to2023.csv") |> 
  filter(year == params$year) 
...
(taxed_av*eq_factor/loa)*0.25, taxed_av*eq_factor), na.rm=TRUE),
    ...
    eav_incents_inTIFs = sum(ifelse(class >=600 & class <=900 & in_tif == 1, taxed_eav, 0), na.rm = TRUE),
