...
cross_county_lines <- c(
  ...
  "030440000", "030585000", "030890000", "030320000", "031280000",
...
#| tbl-cap: "Municipalities with the largest share of Commercial and Industrial property with incentive classification. Uses values obtained from PTAXSIM, and levels of assessment from CCAO's Github. There are 27 municipalities that do not use incentives and have a majority of their taxable EAV within Cook County."
...
muni_incentshare_slice |>
  ...
  add_footer_lines(top = FALSE, values = c("There are 27 municipalities that do not use incentives and have a majority of their taxable EAV within Cook County.") ) |>
  bold(i = 8) |>
  set_table_properties( layout = "autofit")
```
...
muni_rate <- read_csv("../Output/ptaxsim_muni_level_2006to2023.csv") |>
  ...
  rename(cur_munilevel_rate = muni_current_rate_avg) |> filter(!clean_name %in% cross_county_line)

muni_ratechange <- read_csv(paste0("../Output/muni_ratechange_", params$year, "_test.csv")) |> filter(!clean_name %in% cross_county_line)
```
...
muni_count <- n_distinct(muni_ratechange_sliced$clean_name)
no_incents <- n_distinct(muni_ratechange_sliced$clean_name[muni_ratechange_sliced$change_noInc == 0])
median = round(muni_count/2)
median_5 = c( (median-2) : (median+2) )
bottom_5 = c( (muni_count-4) : muni_count )

muni_ratechange_sliced <- muni_ratechange_sliced|>
...
muni_ratechange_sliced |>
  ...
  bold(i = 8) |>
  add_footer_lines(paste0("There are ", muni_count,  " municipalities that do not use incentives and have a majority of their taxable EAV within Cook County."), top = FALSE) |>
  set_table_properties( layout = "autofit")
```
