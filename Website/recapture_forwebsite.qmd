...
![](images/clipboard-3118867582.png)
...
![](images/clipboard-3072202614.png)
...
agency_funds |> filter(fund_type_num==408) |>
  ...
  select(
    `Tax year` = year,
    `Taxing agency` = agency_name_short,
    `Final levy` = final_levy,
    `Fund 408 rate` = final_rate,
    `Fund number` = fund_num
  )  |> DT::datatable(rownames = FALSE)
```

```{r}
#| label: fig-fund-408-tax-rates
#| fig-cap: "Tax-rate additions associated with levy recapture Fund 408 by taxing agency and agency type"
#| 
agency_funds |> filter(fund_type_num==408) |>
...
filtered_df |>
  select(
    `Tax year` = year,
    `Taxing agency` = agency_name,
    `Fund 408 rate` = final_rate,
    `Final levy` = final_levy,
    `Agency number` = agency_num,
    `Fund number` = fund_num,
    `Agency type` = major_type
  ) |>
  DT::datatable(rownames = FALSE)
```
...
```{r}
#| label: fig-fund-408-rate-distribution
#| fig-cap: "Distribution of tax-rate additions associated with Fund 408 by tax year. Source: PTAXSIM agency_fund table."
#| 
agency_funds |> 
  filter(fund_type_num==408) |>
  ggplot() + 
...
```{r}
#| label: fig-fund-408-nonrecapture-share
#| fig-cap: "Fund 408 revenue as a share of non-recapture levy by taxing agency and agency type"
agency_fund408_sums |> 
  filter(year > 2019 & pct_inCook > 0.75) |>
...
agency_fund408_sums |> 
  ...
  select(
    `Tax year` = year,
    `Taxing agency` = agency_name,
    `PTAXSIM Fund 408 amount` = fund408,
    `Treasurer Fund 408 amount` = FundExtension,
    Difference = difference,
    `Final levy` = total_final_levy,
    `Total extension` = total_ext,
    `Share of EAV in Cook County` = pct_inCook,
    `Agency number` = agency_num
  ) |>
  DT::datatable(rownames = FALSE)
```
...
adjust_all |> 
  select(
    ...
    `Recapture eligibility` = adjust_eligible,
    `Aggregate refunds` = aggregate_refunds,
    `Levy adjustment` = levy_adjustment,
    `Final levy` = total_final_levy,
    `Recaptured share of levy` = recap_levyshare,
    `Share of EAV in Cook County` = pct_inCook
  ) |>
  DT::datatable(rownames = FALSE)
```
...
```{r}
#| label: fig-recapture-levy-share-distribution
#| fig-cap: "Distribution of the share of levy recaptured among eligible non-home-rule taxing agencies, by year"
adjust_all |>  
  filter(
...
refund_all |> 
  select(
    `Tax year` = year,
    `Taxing agency` = agency_name,
    `Refund share of levy` = refund_levyshare,
    `Total refunds` = total_refunds,
    `Final levy` = total_final_levy,
    `Final tax rate` = total_final_rate,
    `Home-rule indicator` = home_rule_ind,
    `Share of EAV in Cook County` = pct_inCook,
    `Agency number` = agency_number
  ) |>
  DT::datatable(rownames = FALSE)
...
refund_outliers |>
  rename(
    ...
    `Final tax rate` = total_final_rate,
    `Refund share of levy` = refund_levyshare,
    `Home-rule status` = homerule_label
  ) |>
  DT::datatable(rownames = FALSE)
```
...
#aes(x=year, y = share, label = agency_name), nudge_y =.05 ) + 
  ...
  scale_y_continuous(limits = c(0, 0.25), 
                     breaks = c(0, .1, .25), labels = scales::percent) + 
  labs(title = "Refunds / Levy Amount",
...
refund_all |> 
  ...
  select(
    `Taxing agency` = agency_name,
    `Refund share of levy` = refund_share,
    `Tax year` = year,
    `Total refunds` = total_refunds,
    `Final levy` = total_final_levy
  ) |>
...
#| tbl-cap: "Joined recapture, refund, and levy-adjustment records from the Cook County Clerk and Treasurer"
...
adjust_recap_all |>
  select(
    ...
    `Aggregate refunds in levy-adjustment file` = aggregate_refunds,
    `Levy adjustment` = levy_adjustment,
    `Final levy` = total_final_levy,
    `Recapture eligibility` = adjust_eligible,
    `Share of EAV in Cook County` = pct_inCook
...
final_df |> 
  ...
  select(
    ...
    Difference = diff,
    `PTAXSIM Fund 408 amount` = fund408,
    `Clerk levy adjustment` = levy_adjustment,
    `Total refunds` = total_refund
  ) |>
  DT::datatable(rownames = FALSE)
```
...
impacted_taxcodes <- taxcodes |> 
  ...
  select(year, agency_num, fund_num, fund_type_num, levy, final_levy, final_rate, tax_code_num, tax_code_rate) |>
  #filter(final_levy > 0) |>
  left_join(muni_names, by = "tax_code_num") |>
  group_by(year, tax_code_num, tax_code_rate) |>
  summarize(
    muni_name = first(agency_name),
    rate_408 = sum(ifelse(fund_type_num==408, final_rate, 0), na.rm=TRUE),
         rate_non408 = sum(ifelse(fund_type_num!=408, final_rate, 0), na.rm=TRUE)
  ) |> 
...
```{r}
#| label: tbl-municipal-fund-408-rates
#| tbl-cap: "Average Fund 408 and non-recapture tax rates by municipality and year"
...
impacted_munis |>
  select(
    ...
    `Fund 408 rate` = rate_408,
    `Other-fund rate` = rate_non408,
    `Fund 408 to other-fund ratio` = rate_ratio
  ) |>
  DT::datatable(rownames = FALSE)
...
```{r}
#| label: tbl-high-fund-408-rates
#| tbl-cap: "Municipalities with an average Fund 408 tax-rate addition greater than 0.5 percentage points"
impacted_munis |>
  filter(rate_408 > .5) |>
  select(
    `Tax year` = year,
...
```{r}
#| label: fig-agency-recapture-eligibility-counts
#| fig-cap: "Number of taxing agencies with positive levies by recapture eligibility and year"
final_df |>
...
```{r}
#| label: fig-recapture-eligibility-comparisons
#| fig-cap: "Levy and refund comparisons for recapture-eligible and ineligible taxing agencies"
#| fig-subcap: ["Total levy", "Average levy", "Total refunds", "Refunds as a share of levy"]
#| layout-ncol: 3
...
not_eligible |>
  filter(levy_adjustment!=0 | fund408!=0) |>
  select(
    `Tax year` = year,
    `Taxing agency` = agency_name,
    `Levy adjustment` = levy_adjustment,
...
```{r}
#| label: tbl-eligible-agencies-without-adjustments
#| tbl-cap: "Recapture-eligible agencies with a positive levy and no levy adjustment"

final_df |>
  filter(levy_adjustment == 0 & total_levy > 0 & adjust_eligible == "Eligible") |>
  select(
    `Tax year` = year,
    `Taxing agency` = agency_name,
    `Final levy` = total_levy,
    `Levy adjustment` = levy_adjustment,
    `Recapture eligibility` = adjust_eligible
  ) |>
  DT::datatable(rownames = FALSE)
...
```{r}
#| label: tbl-recapture-eligibility-minor-type-counts
#| tbl-cap: "Counts of taxing agencies by minor agency type and recapture eligibility in tax year 2021"
final_df |>
  ...
  rename(`Agency type` = minor_type) |>
  DT::datatable(rownames = FALSE)
```

```{r}
#| label: fig-recapture-eligibility-minor-type
#| fig-cap: "Counts of park, library, and elementary-school agencies by recapture eligibility and year"
final_df |>
...
```{r}
#| label: fig-recapture-eligibility-major-type
#| fig-cap: "Counts of taxing agencies by recapture eligibility, major agency type, and year"
final_df |>
  filter(final_levy>0) |>
...
```{r}
#| label: fig-recapture-change-ratio
#| fig-cap: "Annual recapture amount relative to the prior year's recapture amount by taxing agency"
final_df |>
  filter(year > 2020) |>
