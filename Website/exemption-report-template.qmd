*Current and Future Uses of Homestead Exemptions in Cook County*
...
tbl <- muni_cl_sums |>
    ...
    summarize(
      ...
      'Other Exemps.' = sum(muni_c_all_exemptions) - sum(muni_c_exe_senior) - sum(muni_c_exe_freeze)
      ) |>
...
ft <- tbl |>
    ...
    add_header_row(value = c("", "Total*"), top = TRUE, colwidths = c(1, 1)) |>  # Add header with colspan
    footnote(i = 1, j = 2, value = as_paragraph("EAV in billions."), ref_symbols = "*") |>  # Footnote in new header
    delete_part(part = "header")  # Remove the default header
...
#| tbl-subcap: "**Supporting Table for Table 2** Current tax burden (levy paid by group / total levy billed by municipality) for all municipalities in Cook County, IL"
#| tbl-subcap: "**Supporting Table for Table 2** Change in tax burden if EAV that is current tax exempt due to homestead exemptions became taxable. Measured in percentage point change."
...
> pct_taxbase and pct_taxburden_current should not be the same
...
pin_data |> 
  ...
  summarize(
            median_bill = median(tax_bill_total, na.rm=TRUE),
            median_AV = median(av, na.rm= TRUE),
            eav = median(taxed_eav, na.rm=TRUE),
            # class = median(class),
            type = "Current Exemptions",
            N = n()) |>
  rename(Municipality = clean_name,
         `Claims GHE` = has_HO_exemp,
        `Median Bill` =median_bill,
        `Median AV` = median_AV,
        `Median Taxed EAV` = eav)
```
