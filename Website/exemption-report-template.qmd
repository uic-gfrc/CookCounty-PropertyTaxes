...
Exemption use varies between suburban Cook County (`r geom_sums$ExemptEAV[[1]]` EAV) and the City of Chicago (`r geom_sums$ExemptEAV[[2]]` EAV) , which may point to broader socioeconomic patterns as well as residents' eligibility and uptake.
...
## Delete if chunk above does both tables successfully.
...
## can cause weird results in tax bill tables if properties that receive other exemption types are kept in the data frame
...
#| label: tbl-Table3
#| column: page
#| tbl-cap: "Table 3. Estimated Bill Change if exempt EAV from the GHE were added to back to the tax base."
...
```{r}
# naive cost:  uses current tax rate!
cost <- pin_data %>% 
  summarize(forgone_rev = sum(tax_amt_exe, na.rm = TRUE),
            exempt_EAV = sum(tax_amt_exe/(tax_code_rate), na.rm = TRUE)
  )
```

There is `r scales::comma(cost$exempt_EAV)` EAV that is exempt from property taxes.

It would take `r scales::dollar(cost$forgone_rev)` to meet taxing agencies' levies and prevent the tax rate from changing.
...
naive_cost = taxcodes_current |> summarize(naive_cost = sum(cur_comp_TC_rate*taxed_eav))
better_cost_est = taxcodes_current |> summarize(better_cost_est = sum(new_comp_TC_rate*(current_taxable_eav + HO_exemps)))

```


There is `r scales::comma(naive_cost$naive_cost)` EAV that is exempt from property taxes.

It would take `r scales::dollar(better_cost_est$better_cost_est)` to fund the levies without the tax rate increasing from the hypothetical rate to the current rate.
