...
_Report went up to 2021, this image includes values through 2023._ 
...
for(i in year_examples){
 ...
 MuniLevel_CompRates <- muni_cl_sums %>%  
      filter(year == i) %>% 
      group_by(clean_name) %>% 
   ...
   MuniLevel_CompRates %>% datatable(rownames = FALSE,
...
_CMAP used a donut chart, we will use a  bar chart that represents 100% on the x a axis._
...
_Table 2: Change in the share of property tax burden due to exemptions for single-family, multi-family,
and commercial and industrial properties, tax year 2021
