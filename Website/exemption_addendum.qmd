...
tax_bill_change_HO <- cook_pins %>% 
 ...
 # filter(clean_name %in% c("Park Forest", "Phoenix", "Winnetka", "Riverdale")) %>%
  arrange(av) %>%
  #mutate(all_exemptions = sum(across(starts_with("exe_")), na.rm=T)) |>
...
<!-- ```{r} -->
<!-- chosen3_billchange <- tax_bill_change_HO %>% filter(clean_name %in% c("Dolton", "Chicago", "Glencoe")) -->
<!-- chosen3_billchange -->
...
<!-- #  -->
<!-- # chosen3_ranked <- cook_pins %>% -->
<!-- #   left_join(tc_muninames)|> -->
<!-- #   inner_join(chosen3, by = "clean_name") %>% -->
<!-- #    arrange(av) %>% -->
<!-- #   summarize( -->
<!-- #     count_pins = n(),  -->
<!-- #             min = min(av), -->
...
<!-- #  -->
<!-- chosen3_billchange <-  munis_ranked %>% -->
...
<!--          bill_noexemps =  tc_hyp_taxrate/100*(equalized_AV-all_exemptions+exe_homeowner), -->
<!--          bill_change = bill_noexemps - bill_current) %>% -->
...
muni_taxrates <- cook_pins %>% 
  ...
  summarize(
    ...
    equalized_AV = sum(av*eq_factor, na.rm = TRUE),
    pins_in_muni = n(),
...
muni_breakeven_points <- cook_pins |>
 ...
 filter(exe_homeowner == 10000) %>% 
  ...
  group_by(clean_name) %>%
  summarize(median_AV = median(av),
            bill_current = mean(final_tax_to_dist), # money needed by non-TIF agencies
   ...
   # rate_change_muni = mean(cur_comp_muni_rate - new_comp_muni_rate), # muni level, not tax code level
    rate_change_TCs = mean(tax_code_rate - tc_hyp_taxrate),
    nobillchange_propertyEAV = round(mean(10000 * ((tax_code_rate/100) / (rate_change_TCs/100)))),
#   nobillchange_propertyEAV_munirates = round(mean(exe_homeowner * ((cur_comp_muni_rate/100) / (rate_change_muni/100)))),
 ...
 nochange_av_TCrates = round(nobillchange_propertyEAV_TCrates / eq_2021),
  ...
  select(clean_name, median_AV, ratio_av_to_breakeven, bill_change, 
         #nobillchange_propertyEAV_munirates, nochange_av_munirates,   rate_change_muni, 
         rate_change_TCs)
```
