...
> Not done. Has current shares with exemptions but not change in property tax burden. Need to create from PIN level data I think. 
...
burden_shift <- ptax_pins %>%
  ...
  mutate(muni_levy = sum(final_tax_to_dist, na.rm = T),
         muni_TEAV = sum(taxed_eav, na.rm = T),
         muni_hyp_TEAV = sum(taxed_eav + all_exemptions, na.rm=T)) %>%
  ungroup() %>%
 # mutate(hyp_TEAV = eav_taxed + all_exemptions)%>%
  ...
  summarize(
         hyp_amt_todist = sum(((rate_noExe/100) * (taxed_eav + all_exemptions)), na.rm=TRUE),
    amt_todist = sum(final_tax_to_dist, na.rm = T) ) %>%
  arrange(desc(amt_todist))

burden_shift %>%  
  filter(Group == "Single-family") %>%
    select(clean_name, hyp_burden, burden) 

burden_shift %>%  
    select(clean_name, Group, hyp_burden, burden) %>% 
pivot_wider(names_from = Group, values_from = burden) %>%
...
mc_burden <- muni_mc_sums %>% 
  ...
  summarize(levy = sum(final_tax_to_dist)) %>%
    mutate(burden = levy / muni_levy ) %>% arrange(desc(burden))

mc_burden %>% 
  select(clean_name, Group, burden) %>% 
  pivot_wider(names_from = Group, values_from = burden) %>%
  arrange(desc(`Single-family`))
```
...
muni_taxrates <- ptax_pins %>% 
  ...
  mutate(rate_change = cur_comp_TC_rate - new_comp_TC_rate,
    ...
    nochange_av = round(nobillchange_propertyEAV / eq2022),
...
muni_mc_sums %>%
  filter(major_class_code== 2) %>% 
  group_by(year) %>%
  summarize(zerodollar_count = sum(zero_bill)) %>% ggplot(aes(x=year, y = zerodollar_count)) + 
