...
muni_cl_sums %>% filter(year == 2023) %>% select(clean_name, muni_c_exe_homeowner:muni_c_exe_vet_dis) %>%
  mutate(Geography = ifelse(clean_name == "Chicago", "City", "Suburbs")) %>%
  group_by(Geography) %>%
  summarize("GHE" = sum(muni_c_exe_homeowner, na.rm=TRUE),
            "Senior" = sum(muni_c_exe_senior, na.rm=TRUE),
