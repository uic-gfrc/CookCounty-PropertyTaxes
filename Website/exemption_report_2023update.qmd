...
smallbills <- ptax_pins |> 
  ...
  select(clean_name, class, eav, equalized_av, av, fmv, total_billed, final_tax_to_dist, pin, everything()) |>
...
smallbills |> select(clean_name, class, eav, equalized_av, av, fmv, total_billed, final_tax_to_dist, pin)
```
...
zerobills %>% 
  reframe(n = n(), exe_homeowner = sum(exe_homeowner), exe_senior = sum(exe_senior), 
          exe_freeze = sum(exe_freeze), taxed_eav = sum(eav, na.rm=TRUE), 
          total_eav = sum(taxed_eav + exempt_eav, na.rm=TRUE), .by = clean_name) |> 
  arrange(desc(exe_freeze))
```
