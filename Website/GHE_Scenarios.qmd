---
title: "Exemption Progressivity & GHE Scenarios"
author: "Alea Wilbur"
date: "`r Sys.Date()`"
format: 
  html:
...
tc_muninames <- tax_codes %>% 
  ...
  select(-agency_rate) %>% 
  left_join(nicknames) %>% 
  select(-c(minor_type, short_name, agency_number))
...
## Ranked Properties and Muni Quartiles - "Single-family Homes"
...
### Case Studies in Red - 25 v 75 Percentile Homes

```{r}
#| label: fig-currenttaxsystem
#| fig-cap: "25th & 75th Percentile AV Properties (Class 2 Only): Ratio of Tax Bill to AV"
#| fig-subcap: "Current GHE Amount (up to 10,000 EAV exempt per property)"
...
ggsave("./addendum-images/eav0.svg")
ggsave("./addendum-images/eav0.png", height = 6, width = 6, units = "in")
...
ggsave("./addendum-images/eav10k.svg")
ggsave("./addendum-images/eav10k.png", height = 6, width = 6, units = "in")
...
ggsave("./addendum-images/eav20k.svg")
ggsave("./addendum-images/eav20k.png", height = 6, width = 6, units = "in")
...
ggplot(data = new_ratios, aes(y = newbill_to_AV_25, x = newbill_to_AV_75, label = clean_name)) + 
  ...
  labs(title = "GHE = $30,000 EAV",
                                        x = "", y = "")
ggsave("./addendum-images/eav30K.svg")
ggsave("./addendum-images/eav30k.png", height = 6, width = 6, units = "in")
...
ggplot(data = new_ratios, aes(y = newbill_to_AV_25, x = newbill_to_AV_75, label = clean_name)) + 
  ...
  labs(title = "GHE = $40,000 EAV", 
                                     x= "", y = "")
ggsave("./addendum-images/eav40K.svg")
ggsave("./addendum-images/eav40k.png", height = 6, width = 6, units = "in")
...
ggplot(data = new_ratios, aes(y = newbill_to_AV_25, x = newbill_to_AV_75, label = clean_name)) + 
  ...
  theme(legend.position = "none") + labs(title = "GHE = $50,000", 
                                      x = "", y = "")
ggsave("./addendum-images/eav50k.svg")
ggsave("./addendum-images/eav50k.png", height = 6, width = 6, units = "in")

```

### Redone using Major Class 2 instead of Single-family homes
...
```{r ratios-without-labels, eval=FALSE, include=FALSE}
#| layout-ncol: 3
#| layout-nrow: 2
#| column: page
...
ggplot(data = ratios, aes(y = GHE_0_bill_to_AV_10, x = GHE_0_bill_to_AV_90, label = clean_name)) + 
  geom_abline(intercept = 0, slope = 1, lty = 2, alpha = .4) +
  geom_point(aes(alpha = .5)) +
      ggrepel::geom_label_repel(data = (ratios %>% filter(clean_name %in% c("Park Forest", "Chicago", "Winnetka", "Glencoe", "Riverdale", "Dolton", "Markham", "Chicago Heights", "Hazel Crest", "Phoenix"))), aes(y = GHE_0_bill_to_AV_10, x = GHE_0_bill_to_AV_90), size = 2, max.overlaps = Inf, point.padding = 0, # additional padding around each point
    min.segment.length = 0, # draw all line segments
    ) +
  geom_point(data = (ratios %>% filter(clean_name %in% c("Park Forest", "Chicago", "Winnetka", "Glencoe", "Riverdale", "Dolton", "Markham", "Chicago Heights", "Hazel Crest", "Phoenix"))), 
                     aes(y = GHE_0_bill_to_AV_10, x = GHE_0_bill_to_AV_90), color = "red", size=2) + 
...
ggplot(data = ratios, aes(y = currbill_to_AV_10, x = currbill_to_AV_90, label = clean_name)) + 
  geom_abline(intercept = 0, slope = 1, lty = 2, alpha = .4) +
  geom_point(aes(alpha = .5)) +
  ggrepel::geom_label_repel(data = (ratios %>% filter(clean_name %in% c("Park Forest", "Chicago", "Winnetka", "Glencoe", "Riverdale", "Dolton", "Markham", "Chicago Heights", "Hazel Crest", "Phoenix"))), aes(y = currbill_to_AV_10, x = currbill_to_AV_90), size = 2, max.overlaps = Inf, point.padding = 0, # additional padding around each point
    min.segment.length = 0, # draw all line segments
    ) +
  geom_point(data = (ratios %>% filter(clean_name %in% c("Park Forest", "Chicago", "Winnetka", "Glencoe", "Riverdale", "Dolton", "Markham", "Chicago Heights", "Hazel Crest", "Phoenix"))), 
                     aes(y = currbill_to_AV_10, x = currbill_to_AV_90), color = "red", size=2) + 
  theme_classic() + 
  scale_y_continuous(limits = c(0, .6), expand = c(0,0))+
  scale_x_continuous(limits = c(0, .6), expand = c(0,0))+
  theme(legend.position = "none") +
...
ggplot(data = new_ratios, aes(y = newbill_to_AV_10, x = newbill_to_AV_90, label = clean_name)) + 
  geom_abline(intercept = 0, slope = 1, lty = 2, alpha = .4) +
  geom_point(aes(alpha = .5)) + 
  ggrepel::geom_label_repel(data = (new_ratios %>% filter(clean_name %in% c("Park Forest", "Chicago", "Winnetka", "Glencoe", "Riverdale", "Dolton", "Markham", "Chicago Heights", "Hazel Crest", "Phoenix"))), aes(newbill_to_AV_10, x = newbill_to_AV_90), size = 2, max.overlaps = Inf, point.padding = 0, # additional padding around each point
    min.segment.length = 0, # draw all line segments
    ) +
  geom_point(data = (new_ratios %>% filter(clean_name %in% c("Park Forest", "Chicago", "Winnetka", "Glencoe", "Riverdale", "Dolton", "Markham", "Chicago Heights", "Hazel Crest", "Phoenix"))), 
                     aes(newbill_to_AV_10, x = newbill_to_AV_90), color = "red", size=2) + 
  theme_classic() + 
  scale_y_continuous(limits = c(0, .6), expand = c(0,0))+
  scale_x_continuous(limits = c(0, .6), expand = c(0,0))+
  theme(legend.position = "none") +
...
ggplot(data = new_ratios, aes(y = newbill_to_AV_10, x = newbill_to_AV_90, label = clean_name)) + 
  geom_abline(intercept = 0, slope = 1, lty = 2, alpha = .4) +
  geom_point(aes(alpha = .5)) +
  ggrepel::geom_label_repel(data = (new_ratios %>% filter(clean_name %in% c("Park Forest", "Chicago", "Winnetka", "Glencoe", "Riverdale", "Dolton", "Markham", "Chicago Heights", "Hazel Crest", "Phoenix"))), aes(y = newbill_to_AV_10, x = newbill_to_AV_90), size = 2, max.overlaps = Inf, point.padding = 0, # additional padding around each point
    min.segment.length = 0, # draw all line segments
    ) +
  geom_point(data = (new_ratios %>% filter(clean_name %in% c("Park Forest", "Chicago", "Winnetka", "Glencoe", "Riverdale", "Dolton", "Markham", "Chicago Heights", "Hazel Crest", "Phoenix"))), 
                     aes(newbill_to_AV_10, x = newbill_to_AV_90), color = "red", size=2) + 
...
ggplot(data = new_ratios, aes(y = newbill_to_AV_10, x = newbill_to_AV_90, label = clean_name)) + 
  ...
  scale_x_continuous(limits = c(0, .6), expand = c(0,0))+
  theme(legend.position = "none") +
  labs(title = "Increase GHE by 40,000 EAV (for up to 50,000 EAV exempt) ", 
                                         y = "10th percentile of homes, taxbill:AV",
                                         x= "90th percentile of homes, taxbill:AV")
```
...
```{r eval=FALSE, include=FALSE}
#| label: progressivity-90to10-expanded
#| layout-ncol: 3
#| layout-nrow: 2
#| column: page
...
# write_csv(munis_billchange, "../Output/5b_munis_billchange_1025507590.csv")
```
