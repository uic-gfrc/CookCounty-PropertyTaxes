...
#| fig-cap: "**Share of FMV that is tax exempt due to Homestead Exemptions**"
...
> auto sum totals for inline text
...
#| fig-cap: "**Exempt EAV in Cook County** Exemption use varies between suburban Cook County ($9.7B EAV) `{r} geo_sums$`Exempt EAV`[Geography==Suburbs]` EAV and the City of Chicago ($7.0B EAV) `{r} geo_sums$`Exempt EAV`[Geography==Suburbs]` EAV, which may point to broader socioeconomic patterns as well as residents' eligibility and update. Note: Our current calculations undervalue the disabled veterans exemption."
...
### Figure 6. Composite property tax rates with and without homestead exemptions, tax year `{r} params$year`
...
#    
#    # values that appear on tax bill 
#     tax_amt_exe = sum(all_exemptions*tax_code_rate, na.rm = TRUE), 
#     tax_amt_pre_exe = sum(eq_av*tax_code_rate, na.rm = TRUE), 
#     tax_amt_post_exe = sum(taxed_eav*tax_code_rate, na.rm = TRUE)
#   ) |>
#   
...
_Table 2: Change in the share of property tax burden due to exemptions for single-family, multi-family, and commercial and industrial properties, tax year `{r} params$year`_
...
burden_shift <- pin_data |>
  ...
  mutate(
     #    pct_eav_current = group_taxbase / muni_taxbase,
         pct_taxburden_current = round(group_taxes_current / muni_levy, digits = 4),
         hyp_pct_taxburden = round(hyp_group_taxes / muni_levy, digits =4 )  )|> 
...
pin_data <- pin_data |> 
  ...
  mutate(
    ...
    bill_noGHE = new_comp_TC_rate/100*(eq_av-all_exemptions+exe_homeowner),
    bill_noexemps = new_comp_TC_rate/100*(eq_av),
...
muni_median_summarytable <- pin_data |> 
  ...
  mutate(bill_current = (final_tax_to_dist + final_tax_to_tif),
         bill_noexemps =  new_comp_TC_rate/100*(eq_av-all_exemptions+exe_homeowner),
