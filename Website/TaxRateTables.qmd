---
title: "Whitepaper Figures and Supporting Tables"
---
...
datatable(MuniLevel_CompRates, rownames = FALSE,
          colnames = c('Municipality' = 'clean_name', 'Composite Tax Rate Change'='rate_change', 'Current Comp. Rate' = 'cur_comp_muni_rate', 'Hypothetical Rate' = 'new_comp_muni_rate', 
...
MC_burden <- MC_sums %>% 
  ...
  mutate(muni_eav = sum(eav),
         ...
         hyp_muni_taxableEAV = sum(hyp_taxable_eav)
  ) %>%
  ungroup() %>%
...
```{r}
MC_burden <- MC_sums %>% 
  mutate(hyp_taxable_eav = current_taxable_eav + all_exemptions,
         class_taxes_current = current_taxable_eav * (cur_comp_muni_rate/100)
     ) %>%
  group_by(clean_name) %>%
  mutate(muni_eav = sum(eav),
  ...
  mutate(new_comp_rate = muni_levy / hyp_muni_taxableEAV,
         ...
         # pct_taxburden_ghe0 = new_taxable_eav / muni_levy,
         pct_taxburden_current = class_taxes_current / muni_levy,
         pct_taxburden_noexemps = class_taxes_hyp / muni_levy,
         burden_shift = (pct_taxburden_current - pct_taxburden_noexemps)*100)


current_burden_c2 <- MC_burden %>% 
  filter(major_class_code == 2) %>% 
  select(clean_name, pct_eav, burden_shift, pct_taxburden_current, pct_taxburden_noexemps) %>%
  arrange(pct_eav)


datatable(current_burden_c2, rownames = FALSE,
          colnames = c('Municipality' = 'clean_name', "C2 EAV/Muni EAV" = 'pct_eav','Burden Shift, Pct Pt Change' = 'burden_shift',  'Current Tax Burden\nC2 Tax Collected / Muni Levy' = 'pct_taxburden_current', "Hypothetical Tax Burden\nHyp. C2 Tax Collected / Muni Levy" = 'pct_taxburden_noexemps'),
          caption = "Table 3: Tax Burden Shifted by Residential Exemptions
          \n(All Exemption types except abatements)."
           ) %>%
  formatPercentage(c(2,4,5), digits = 2) %>%
  formatRound(c(3), digits = 2)
```
